...
begin
   ...
   QrChequeStatus.AsInteger := -1; //ÈÏæä æÖÚíÊ íÇ ÎÇáí
   if ChequeType2 = 1 then //ÝÞØ ÈÑÇí ˜åÇí ÏÑíÇÝÊí ÔÊ äãÑå äíÇÒ ÇÓÊ
...
Var
   ...
   FacTypeFa : String;
begin
   //ÌáæíÑí ÇÒ ÍÐÝ ˜åÇíí ˜å ÏÑ ÝÇ˜ÊæÑåÇ ÇÓÊÝÇÏå ãí ÔæäÏ
   //ÈÇÔÏ íÚäí Çíä ÝÑã ÇÒ ØÑÞ ÝÑã ÝÇ˜ÊæÑ ÈÇÒ ÔÏå ÇÓÊ Factor_ID2 > 0 ÇÑ ãÞÏÇÑ
   ...
   begin
      ...
      if FacType = 'F' then
         FacTypeFa := 'ÝÇ˜ÊæÑ ÝÑæÔ'
      else
         if FacType = 'KH' then
            FacTypeFa := 'ÝÇ˜ÊæÑ ÎÑíÏ'
         else
            {if FacType = 'P' then
               FacTypeFa := 'íÔ ÝÇ˜ÊæÑ'
            else}
               if FacType = 'BF' then
                  FacTypeFa := 'ÝÇ˜ÊæÑ ÈÑÔÊ ÇÒ ÝÑæÔ'
               else
                  if FacType = 'BKH' then
                     FacTypeFa := 'ÝÇ˜ÊæÑ ÈÑÔÊ ÇÒ ÎÑíÏ'
                  else
                     if FacType = 'A' then
                        FacTypeFa := 'ÝÇ˜ÊæÑ ÇãÇäí'
                     else
                        if FacType = 'E' then
                           FacTypeFa := 'ÝÇ˜ÊæÑ ÇÕØåáÇ˜';
      if FacNu > 0 then
      begin
         ShowMyMessage('íÛÇã',' ÈÏáíá ÇÓÊÝÇÏå Çíä ˜ ÏÑ ' + FacTypeFa + ' ÈÇ ÔãÇÑå '+ IntToStr(FacNu) + ' Çã˜Çä ÍÐÝ Âä æÌæÏ äÏÇÑÏ¡ ' +#13+
                        ' ÈÑÇí ÍÐÝ Çíä ˜ ÈÇíÏ ÇÒ ØÑíÞ ÝÇ˜ÊæÑ ãÑÈæØå ÇÞÏÇã äãÇííÏ ' ,[mbOK],mtInformation);
         Abort;
      end;
   end;
...
begin
   ...
   QrCheque.Parameters.ParamByName('Status1').Value := Status_; //ÌåÊ ÝíáÊÑ ˜ÑÏä
   QrCheque.Parameters.ParamByName('Status2').Value := Status_; //ÌåÊ ÝíáÊÑ ˜ÑÏä
end;

procedure TDMCheque.QrChequeBeforePost(DataSet: TDataSet);
begin
   Qry_IFFieldIsEmpty(QrChequeNumber , 'ÔãÇÑå ˜');
   Qry_IFFieldIsEmpty(QrChequeAcountNumber , 'ÔãÇÑå ÍÓÇÈ');
   Qry_IFFieldIsEmpty(QrChequeExportDate , 'ÊÇÑíÎ ÕÏæÑ');
   Qry_IFFieldIsEmpty(QrChequeED_Bank_Ref , 'äÇã ÈÇä˜');
   //if Not Date_IsFaDate(QrChequeExportDate.AsString,'ÊÇÑíÎ ÕÏæÑ') then
      //Abort;
   //if Not Date_IsFaDate(QrChequeDueDate.AsString,'ÊÇÑíÎ ÓÑÑÓíÏ') then
      //Abort;
end;

procedure TDMCheque.QrChequeCalcFields(DataSet: TDataSet);
begin
   QrChequePriceBeHorof.AsString := AdadBeHorof(QrChequePrice.AsInteger)+ ' ÑíÇá ';
end;

procedure TDMCheque.QrChequeAfterScroll(DataSet: TDataSet);
begin
   //Cheque áíÓÊ ÝíáÏåÇí ÝÑÚí ÌÏæá
