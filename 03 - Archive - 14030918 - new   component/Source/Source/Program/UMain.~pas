...
Uses shellapi, windows,forms, BaseUnit, ExtCtrls, Menus, ExtActns, BandActn, Classes,SysUtils,StdCtrls,Dialogs,
  ...
  dbimageen,yShamsiDate, AcroPDFLib_TLB, mssdbgrid, Planner, AdvAlertWindow,
  TntDBGrids, xpWindow;
...
uses Dmu , FixedTableF, EnteredLetterInputF, ExitedLetterInputF,
  ...
  ULetterImport, ULetterNoSettingForWord;
...
begin
   ...
   begin
      Exec_SendLetter_To_Other_Sec(FselectSec.SelectedSecID,aLetterID);
...
//   lettersDbGrid.SelectedField := colNum.Field;
...
begin
   ...
   Dm.qtemp.Close;
   Dm.qtemp.SQL.Clear;
   Dm.qtemp.SQL.Add('SELECT IsCopy_In_Dabir FROM ReCommites WHERE LetterID='+IntToStr(dm.Get_All_LetterLetterID.AsInteger));
   Dm.qtemp.Open;
   if Dm.qtemp.FieldByName('IsCopy_In_Dabir').AsString <> '' then
      _AllowToEditWordFiles:= Dm.qtemp.FieldByName('IsCopy_In_Dabir').AsBoolean;
...
begin
   ...
   begin
      ...
      if not Dm.ExecGet_LetterWordFile(Dm.Get_All_LetterLetterID.AsInteger,false,False) then
      begin
         messageShow(86,FALSE);
         exit;
      end;
   ...
   {Ranjbar 89.08.04 ID=165}
   //fmAssignLetterIndicator.lblMessage.Caption:=Get_sysAppMessage(34)+' '+dm.Get_All_LetterLetterID.AsString+' '+
   fmAssignLetterIndicator.lblMessage.Caption := Get_sysAppMessage(34)+' '+Trim(dm.Get_All_LetterIndicatorID.AsString)+' '+
   //---
      ...
      begin
            ...
            (*
             //Comment By Sanaye 940918
              //Õ· „‘ò· ¬ÅœÌ  ‘„«—Â œ»Ì— Œ«‰Â œ— ÊÊ—œ
            with TADOQuery.Create(nil)do
            begin
               Connection := dm.YeganehConnection;
               SQL.Text := 'select dbo.InnerNo('+Dm.Get_All_LetterLetterID.AsString+')';
               Open;
               if not IsEmpty then
...
begin
   inherited;
   ISMessageShow := GetUserSetting('cbAlarmMessage');
...
//   ShowMessage('11'+lettersDbGrid.SortedFieldName);
   ...
   {$ELSE}
      Caption := '‰—„ «›“«— œ»Ì—Œ«‰Â Ê « Ê„«”ÌÊ‰ «œ«—Ì Ìê«‰Â - ‘—ò  ›‰«Ê—«‰ «ÿ·«⁄«  Ìê«‰Â';
   {$ENDIF}
...
begin
   ...
   begin
      ...
      //SpinEdit.Text := IntToStr(Dm.MaxLetterToShow);
