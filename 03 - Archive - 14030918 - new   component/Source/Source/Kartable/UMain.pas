...
uses shellapi,forms, BaseUnit, ExtCtrls, Menus, ExtActns, BandActn, Classes,SysUtils,StdCtrls,Dialogs,
  ...
  Buttons, uCiaTrayIcon,Registry,Math ,
  StdStyleActnCtrls,  frxClass, wwriched, RpDefine, RpRender, RpRenderPDF,
  OleCtnrs, OleCtrls,U_SHOW_ATF_PEYRO_FRM,U_VERSION_INFO_FRM,U_SEE_PAYVAST_ERJA_FRM,
  {Myowner }
  frmmain;
...
begin
       ...
       begin
          ...
          if (FInbox.SBHome.Down) or (FInbox.SBArchive.Down)  then ParamByName('@OrgID').Value:=_ActiveSemat
          else
...
//   ADraftToLetter.Visible:=false;
   ...
   FInbox.lblYear.Caption:=' ”«· : '+IntToStr(1300+dm.CurrentMYear);
...

{
  ...
  //‰„«Ì‘ ‰«„Â Â«Ì ”«·Â«Ì ﬁ»· »— «”«”  «—ÌŒ
  //ATAIE 1400
  //if dm.CurrentMyear <> StrToInt(Copy(_Today,3,2)) then
  if dm.CurrentMyear <> StrToInt(LeftStr(_Today,4)) then
...
begin
   ...
   begin
      ...
      if not Dm.ExecGet_LetterWordFile(FieldByName('LetterID').AsInteger,Dm.Sp_InboxIsCopy.AsBoolean) then
      begin
