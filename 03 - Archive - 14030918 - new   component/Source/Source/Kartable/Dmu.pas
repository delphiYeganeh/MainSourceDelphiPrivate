...
type
  ...
  public
    ...
    Function  FileName(indicatorID:integer;Myear:byte):string;
...
Const
   _Kartable = True;
   _Showlogo = True;
   _SoftLastUpdate = '1398/05/19';//'1396/06/22';
   _SoftVersion = '10.0.0.0';//'7.0.0.2';

{$R *.dfm}

implementation

uses yShamsiDate, Uconnect, LogOn, Settingsu, UMain,   UsearchStno, USearchTitle, YeganehDemo,
...
begin
   ...
   _ArchiveCenterID:=-1;// »—«Ì ò«— «»· »«Ìœ Â„Ì‘Â „‰›Ì Ìò »«‘œ çÊ‰ »—«Ì ò«— «»· „—ò“ »«Ìê«‰Ì „⁄‰Ì ‰œ«—œ
end;
...
begin
   ...
   DefaultIsCopy := GetUserSetting('DefaultisCopy'); //œ— œ»Ì—Œ«‰Â  ⁄ÌÌ‰ «—Ã«⁄ ‰«„Â »’Ê—  ›ﬁÿ ŒÊ«‰œ‰Ì
...
begin
  ...
  if SetThreadLocale(1065) then
   ...
   //ATAIE 1400
   //_Year  := strtoint(Copy(_Today,3,2));
...
//    on E : Exception do
//     begin
//     ShowMessage('«” ›«œÂ «“ ﬁ›· ﬁœÌ„Ì ...' + chr(13) + e.Message);
//     end;
//   END;
//   
//   end
...
//    end;
   ...
   begin
      File_FindAndDelete(_UserFolder,'*.*'); //Õ–›  „«„Ì ›«Ì·Â«Ì „ÊÃÊœ œ— «Ì‰ ÅÊ‘Â
...
function TDm.FileName(indicatorid:integer;myear:byte):string;
begin
...
begin
 ...
 {  with TADOQuery.Create(nil)do
   begin
      ...
      SQL.Text := 'SELECT IsCorrectedImage FROM LetterData WHERE LetterID='+IntToStr(Global_LetterID)+' AND extention=3 ';
      Active := true;
      if (Fields[0].AsBoolean)or(Readonly) then
      begin
         try
            Active := false;
            SelectPDF.Close;
            SelectPDF.Parameters[0].Value := Global_LetterID;
            SelectPDF.Open;
            SelectPDFImage.SaveToFile(_TempPath+'tmp.pdf');
            ShellExecute(Application.handle, 'open', pchar(_TempPath+'tmp.pdf'),nil,nil,SW_SHOWNORMAL) ;
            exit;
         except
         end;
      end;
   end;  }   //Mohamed 92/05/13
...
Var
   ...
   s : string;
begin
   if Not  GetUserSetting('InsertSignInWord') then // ﬁ—«— œ«œ‰ «„÷« œ— Ê—œ
      Exit;
   ...
   Try
      ...
      if MBaseForm.MessageShowString('¬Ì« „«Ì·Ìœ «„÷«Ì ŒÊœ —« œ— ›«Ì· Ê—œ ﬁ—«— œÂÌœ' ,True) then
      begin
 ...
 {  Try
      ...
      begin
         ...
         if FInputPass.ModalResult = mrOK then
         begin
            TemPicName  := _TempPath+'TemPic.Jpg';
            QrUserSignUserSign.SaveToFile(TemPicName);
            img := TImage.Create(nil);
            img.Picture.LoadFromFile(TemPicName);
            Clipboard.Assign(Img.Picture);    // òÅÌ ò—œ‰  ’ÊÌ— «„÷« œ— ò·ÌÅ»—œ
            WordApplication.ActiveDocument.ActiveWindow.Selection.SetRange(2147483646, 2147483647);   //
           // WordApplication.ActiveDocument.Range( DwdStory, DwdMove);//Commented by Hadi Mohamed//WordApplication.Selection.SetRange(2147483646, 2147483647);//End of Doc
            //WordApplication.ActiveDocument.Shapes.AddPicture(TemPicName,emptyparam,EmptyParam,EmptyParam,EmptyParam,EmptyParam,EmptyParam,EmptyParam);
            WordApplication.ActiveDocument.ActiveWindow.Selection.InsertAfter(#13);
            WordApplication.ActiveDocument.ActiveWindow.Selection.SetRange(2147483646, 2147483647);
            WordApplication.ActiveDocument.ActiveWindow.Selection.ParagraphFormat.Alignment := wdAlignParagraphLeft;
            WordApplication.ActiveDocument.ActiveWindow.Selection.Paste;
         end;
      end;
   Finally
      QrUserSign.Close;
...
procedure TDm.Timer_SaveWordTimer(Sender: TObject);// „‰ Ÿ—  « »” Â ‘œ‰ ›«Ì· Ê—œ Ê –ŒÌ—Â ¬‰ œ— œÌ «»Ì”
var
   i: integer;
begin
...
begin
   ...
   begin
      ...
      begin
         ...
         OpenWordFile(_TempPath+_WordFileName,ReadOnly);//»«“ ò—œ‰ ›«Ì·
...
begin
   ...
   if sp_inboxIsAnswer.AsBoolean then
      sp_inboxIsAnswerTitle.AsString := 'œ«—«Ì ÃÊ«»';

   {if sp_inboxISFollowup.AsBoolean then
      sp_inboxISFollowupTitle.AsString := 'œ«—«Ì ÅÌêÌ—Ì'; }
end;
...
begin
   ...
   begin
      ...
      begin
         Active := false;
         SQL.Text := 'INSERT INTO Extention(ExtentionTitle,Extention,IsSystem)VALUES(	''›«Ì· ÅÌ œÌ «›'',''pdf'',0)';
         ExecSQL;
      end;
      SQL.Text := 'SELECT ExtentionID FROM Extention WHERE Extention=''pdf''';
      Active := true;
...
//Provider=SQLOLEDB.1;Integrated Security=SSPI;Persist Security Info=False;Initial Catalog=ydabir;Data Source=.
