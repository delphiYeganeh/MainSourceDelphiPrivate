...
uses
  ...
  UemsVCL, ComCtrls, Grids, DBGrids, ActnList, ExtActns, Menus;
  ...
  if _selectedLettersID.Count>0 then
     for i:=0 to _selectedLettersID.Count-1 do
      begin
           ...
           (strtoint(_selectedLettersID[i]),3,DBLookupComboBox2.KeyValue,DBparaph.Text,DBShamsiDateEdit2.Text,_userid,DBCIsCopy.Checked,PCheck.Checked,DBEdit1.Text);
      end;
      end;
...
begin
  inherited;
 DBEdit9.SetFocus;
DBCIsCopy.Checked:=dm.RecommiteCopy;
end;
