...
uses
  ...
  ActnList, DB,hyiedefs, ieview, imageenview, DBCtrls;
...
begin
  ...
  if not(DMScanImage.QrSelImage.State in [dsEdit,dsInsert]) then
    Exit;

  With FmScanImage,ImageEnVectMain do
  begin
    //ÃÂ  »«“Ì«»Ì œÊ»«—Â  ’ÊÌ—
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        QrSelImage.Edit;
        FmScanImage.ImageEnVectMain.Proc.Resample(2480,3508,rfNone); //A4
        FmScanImage.ImageEnVectMain.FitToWidth;
        QrSelImage.Post;
        QrLetterData.Prior;
      end;
    end;
