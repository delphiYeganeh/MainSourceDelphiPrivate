...
begin
  ...
  try
    ...
    begin
      ...
      //ﬁ—«—œ œ«œ‰ ‘„«—Â  ’ÊÌ— Ê  ⁄œ«œ ﬂ·  ’«ÊÌ—

      if ChBoProperty.Checked then
      begin
        SecTitle := ' œ› — : ' + Qry_GetResult('Select SecTitle from Secretariats Where SecID = '
        ...
        FmScanImage.ImageEnVectMain.Bitmap.Canvas.Font.size := SEPropertyFont.Value; //ÅÌ‘ ›—÷ 8 „Ì»«‘œ
        ...
        FmScanImage.ImageEnVectMain.Bitmap.Canvas.TextOut(40,FmScanImage.ImageEnVectMain.Bitmap.Height-50,
                   ...
                   + DMScanImage.QrLetterDataLetterID.AsString,Dm.YeganehConnection)} +
                   ' _ ‘„«—Â ”‰œ : ' +  Qry_GetResult('Select IncommingNO from letter Where LetterID = '
                   + DMScanImage.QrLetterDataLetterID.AsString,Dm.YeganehConnection) +
                   ' _  «—ÌŒ ”‰œ: ' +  Qry_GetResult('Select Incommingdate from letter Where LetterID = '
                   + DMScanImage.QrLetterDataLetterID.AsString,Dm.YeganehConnection)
                   + ' _ ’›ÕÂ  ' + DMScanImage.QrLetterDataPageNumber.AsString + '  «“  '
                   + DMScanImage.QrLetterDataRecCount_Calc.AsString );

                   //+ ' _ ’›ÕÂ  ' + DMScanImage.QrLetterDataRecCount_Calc.AsString + '  «“  '
                  { + ' _ ’›ÕÂ  ' + IntToStr(DMScanImage.QrLetterData.RecordCount) + '  «“  '
