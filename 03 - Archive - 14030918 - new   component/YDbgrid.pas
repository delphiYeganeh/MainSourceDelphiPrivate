...
begin
  ...
  begin
    ...
    c:=t*c1+(1-t)*c2;
    if Assigned(FOnNeedColorCondition) then
      FOnNeedColorCondition(Column,State,C);
    Canvas.Brush.Color:=c;
    f:=Canvas.Font;
    if Assigned(FOnNeedFontCondition) then
      FOnNeedFontCondition(Column,State,F);
  end;
...
begin
  ...
  if key=Vk_Up then
    if (DataSource.DataSet.RecNo<=1)and(DataSource.DataSet.RecNo>0) then
...
begin
  ...
  OpenD.Filter:='›«Ì·Â«Ì „ ‰Ì|*.txt|Â„Â ›«Ì·Â«|*.*';
  OpenD.Title:='Œ—ÊÃÌ „ ‰Ì »—«Ì ÃœÊ·';
    ...
    if OpenD.Execute then
      begin
        ...
        adoConn.ConnectionString := 'Provider=Microsoft.Jet.OLEDB.4.0;Data Source='+
                                    OpenD.FileName+';Persist Security Info=False';
  ...
  if tmp.DataSet.Active then
    ...
    begin
      ...
      for i:=0 to Sums.Count-1 do
        ...
        begin
          if upperCase(Items[i])<>'COUNT' then
            Text:= '„Ã„Ê⁄ '+GetTitle(Items[i])+' = '+CommaSeperated(Sums[i])
          else
            Text:= ' ⁄œ«œ '+CommaSeperated(Sums[i]);
...
begin
  ...
  if ColumnWidthHelper.Index < 0 then
    Exit;
