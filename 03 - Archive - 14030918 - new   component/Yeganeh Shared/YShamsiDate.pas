...
function SaveDoc(letterID : integer): string;
procedure AddUserSign(WordFile: string; UserID: integer);
...
function setPass(S_In:string):string;
var
  A:array [0..50]of Char;  i,len:byte;
begin
  len:=length(S_in);
  StrPCopy(A, S_in);
  if len > 0 then
...
Procedure Form_SetShortCut(AForm:TForm;ADBNavigator:TDBNavigator;SBSearch,SBReport:TSpeedButton ;//TAdvGlowButton;
                          Sender: TObject; var Key: Word ; Shift: TShiftState);
begin
   if Shift=[] then
   begin
...
begin
   ...
   begin
      ...
      if Recordcount = 0 then
         Exit;
      dm.Get_LetterWordFileImage.SaveToFile(path);
   end;
   finally
   Result := path;
   end;
