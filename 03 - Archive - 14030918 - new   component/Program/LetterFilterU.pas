...
uses
  ...
  YDBComboBox, DBCtrls, Yeganeh_Word, AdvGlowButton;
...
type
  TLetterFilter = class(TMBaseForm)
    ...
    SBLetterArchiveFolder: TAdvGlowButton;
...
uses
  Dmu , YShamsiDate, BusinessLayer, SearchFM;
    ...
    begin
      ...
      begin
        Case CBEx_Color.ItemIndex of
          1: LetterColor := clRed; //ﬁ—„“
          2: LetterColor := clBlue; //¬»Ì
          3: LetterColor := clGreen; //”»“
          4: LetterColor := clGray; //Œ«ﬂ” —Ì
          5: LetterColor := clPurple; //»‰›‘
          6: LetterColor := RGB(255,95,0); //‰«—‰ÃÌ  LowOrange
          7: LetterColor := RGB(0,175,240); //›Ì—Ê“Â «Ì  LowTurquoise;
          8: LetterColor := RGB(240,90,160); //’Ê— Ì  LowPink;
        end;
...
begin
  inherited;
  ChangeKaf := False;
  ChangeKaf := Dm.GetValue(200);//  €ÌÌ— Õ—› "ﬂ" »Â Õ—› "ò"œ— Â‰ê«„  «ÌÅ
  dm.qDC.Open;
