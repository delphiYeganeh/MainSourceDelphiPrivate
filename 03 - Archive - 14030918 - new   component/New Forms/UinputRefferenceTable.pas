...
uses
  ...
  ActnList, Buttons, BusinessLayer, AdvGlowButton;
...
begin
  inherited;
  if dm.Y_InputQuery('⁄‰Ê«‰ ÃœÊ·',get_ReferenceTableDescription.AsString,s) then
    with get_ReferenceTable do
    begin
      edit;
...
begin
  ...
  if not dm.Y_InputQuery('⁄‰Ê«‰ ÃœÊ·','ÃœÊ· „—Ã⁄ ÃœÌœ',s) then
    Exit;
...
begin
  ...
  intHas := HasReferenceTableData(get_ReferenceTableTableId.AsInteger);

  if intHas = 0 then
  begin
    if MessageShowString('¬Ì« «“ Õ–› „ÿ„∆‰ Â” Ìœø', True) then
      Get_ReferenceTable.Delete;
  end
  else
  begin
    ShowMessage('«Ì‰ ÃœÊ· œ«—«Ì ⁄‰Ê«‰ Â«ÌÌ „Ì »«‘œ Ê ﬁ«»· Õ–› ‰„Ì »«‘œ');
  end;
end;
