...
uses
  ...
  Grids, DBGrids, ADODB, MssDBGrid, AdvGlowButton;
...
Ì«
//if FmSearch.GetSearchValue(QrOrg,QrOrgID.FieldName,QrOrgTitle.FieldName,' ',550,500)<>'0' then}
  ...
  try
    try
      ...
      begin
        ...
        if Result <> '' then
        begin
          if (UpperCase(QrSearch.Name)=UpperCase('QrUserMemo')) or
             (UpperCase(QrSearch.Name)=UpperCase('QrUserMemo1')) or
             (UpperCase(QrSearch.Name)=UpperCase('QrUserMemo2')) then
          begin
            QrSearch.Close;
            QrSearch.SQL.Text := SqlText +' AND '+KeyField+'='+Result;
            QrSearch.Open;
          end
          else
          begin
...
Var
   ...
   TempStr : String;
begin
  //F:=DBGridSearch.SelectedField.FieldName;
