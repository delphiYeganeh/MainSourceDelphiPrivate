object frm_ListTafkikiKol: Tfrm_ListTafkikiKol
  Left = 312
  Top = 220
  Width = 695
  Height = 616
  BiDiMode = bdRightToLeft
  Caption = #1604#1610#1587#1578' '#1578#1601#1705#1610#1705#1610' '#1705#1604
  Color = clBtnFace
  Font.Charset = ARABIC_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  ParentBiDiMode = False
  Position = poScreenCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Number: TLabel
    Left = 511
    Top = 491
    Width = 3
    Height = 13
  end
  object YDBGrid1: TYDBGrid
    Left = 0
    Top = 0
    Width = 679
    Height = 528
    Align = alClient
    Color = 16770790
    DataSource = dusp_ListTafkikiKol
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
    ReadOnly = True
    TabOrder = 0
    TitleFont.Charset = ARABIC_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    YeganehColor = True
    YeganehFinish = False
    PageNumber = False
    RecordNumber = False
    HasTime = False
    Oriention = poPortrait
    HasDate = False
    HasIndex = False
    PrintTitle = #1604#1610#1587#1578' '#1578#1601#1705#1610#1705#1610' '#1705#1604
    InvertFarsiDate = True
    TitleSort = True
    AutoInsert = False
    FooterFields = 'Count'
  end
  object Panel1: TPanel
    Left = 0
    Top = 528
    Width = 679
    Height = 50
    Align = alBottom
    TabOrder = 1
    object Button5: TxpBitBtn
      Left = 7
      Top = 9
      Width = 126
      Height = 35
      startColor = 16645629
      EndColor = 14666957
      Caption = #1582#1585#1608#1580
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        1DFEA31DFEA31DFEA31DFEA31DFEA21AFE9B0BFED99E}
      Gradient = True
      TabOrder = 0
      TabStop = True
      OnClick = Button5Click
    end
    object Button3: TxpBitBtn
      Left = 543
      Top = 9
      Width = 126
      Height = 35
      startColor = 16645629
      EndColor = 14666957
      Caption = 'Word'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ImageList = Dm.LetterImages
      ImageIndex = 24
      Gradient = True
      TabOrder = 1
      TabStop = True
      OnClick = Button3Click
    end
    object Button4: TxpBitBtn
      Left = 409
      Top = 9
      Width = 126
      Height = 35
      startColor = 16645629
      EndColor = 14666957
      Caption = 'Excel'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ImageList = Dm.LetterImages
      ImageIndex = 23
      Gradient = True
      TabOrder = 2
      TabStop = True
      OnClick = Button4Click
    end
    object xpBitBtn1: TxpBitBtn
      Left = 273
      Top = 9
      Width = 126
      Height = 35
      startColor = 16645629
      EndColor = 14666957
      Caption = 'Print'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ImageList = Dm.LetterImages
      ImageIndex = 55
      Gradient = True
      TabOrder = 3
      TabStop = True
      OnClick = xpBitBtn1Click
    end
  end
  object usp_ListTafkikiKol: TADOStoredProc
    Connection = Dm.YeganehConnection
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    ProcedureName = 'usp_ListTafkikiKol;1'
    Parameters = <
      item
        Name = '@ExamId'
        Value = Null
      end>
    Left = 376
    Top = 272
  end
  object dusp_ListTafkikiKol: TDataSource
    AutoEdit = False
    DataSet = usp_ListTafkikiKol
    Left = 264
    Top = 264
  end
  object usp_Course_Count_ForExam: TADOStoredProc
    Connection = Dm.YeganehConnection
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    ProcedureName = 'usp_Course_Count_ForExam;1'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
        Value = 0
      end
      item
        Name = '@ExamID'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = 285
      end>
    Left = 264
    Top = 376
    object usp_Course_Count_ForExamCourseCount: TIntegerField
      FieldName = 'CourseCount'
      ReadOnly = True
    end
  end
end
