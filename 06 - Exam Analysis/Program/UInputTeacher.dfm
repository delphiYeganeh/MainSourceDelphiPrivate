inherited FrInputTeacher: TFrInputTeacher
  Left = 407
  Top = 255
  Width = 429
  Height = 298
  Caption = #1608#1585#1608#1583' '#1575#1591#1604#1575#1593#1575#1578' '#1575#1587#1578#1575#1583
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 13
  object Label2: TLabel [0]
    Left = 306
    Top = 52
    Width = 94
    Height = 13
    Anchors = [akTop, akRight]
    Caption = #1705#1583' '#1575#1587#1578#1575#1583
    FocusControl = DBEdit2
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label3: TLabel [1]
    Left = 306
    Top = 89
    Width = 94
    Height = 13
    Anchors = [akTop, akRight]
    Caption = #1606#1575#1605' '
    FocusControl = DBEdit3
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label4: TLabel [2]
    Left = 306
    Top = 127
    Width = 94
    Height = 13
    Anchors = [akTop, akRight]
    Caption = #1606#1575#1605' '#1582#1575#1606#1608#1575#1583#1711#1610
    FocusControl = DBEdit4
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label5: TLabel [3]
    Left = 306
    Top = 161
    Width = 94
    Height = 13
    Anchors = [akTop, akRight]
    Caption = #1606#1575#1605' '#1705#1575#1605#1604
    FocusControl = DBEdit5
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label7: TLabel [4]
    Left = 306
    Top = 194
    Width = 94
    Height = 13
    Anchors = [akTop, akRight]
    Caption = #1588#1605#1575#1585#1607' '#1607#1575#1610' '#1578#1605#1575#1587
    FocusControl = DBEdit7
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object ActionMainMenuBar1: TActionMainMenuBar [5]
    Left = 0
    Top = 0
    Width = 413
    Height = 30
    UseSystemFont = False
    ActionManager = ActionManager1
    Caption = 'ActionMainMenuBar1'
    ColorMap.HighlightColor = clWhite
    ColorMap.BtnSelectedColor = clBtnFace
    ColorMap.UnusedColor = clWhite
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    Spacing = 0
  end
  object xpBitBtn1: TxpBitBtn [6]
    Left = 16
    Top = 224
    Width = 101
    Height = 32
    startColor = 16645629
    EndColor = 14666957
    Caption = #1582#1585#1608#1580
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clNavy
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Glyph.Data = {
      
      1DFEA31DFEA31DFEA31DFEA31DFEA21AFE9B0BFED99E}
    Gradient = True
    Anchors = [akLeft, akBottom]
    TabOrder = 1
    TabStop = True
    OnClick = xpBitBtn1Click
  end
  object DBEdit2: TDBEdit [7]
    Left = 160
    Top = 49
    Width = 144
    Height = 21
    Anchors = [akLeft, akTop, akRight]
    DataField = 'TeacherCode'
    DataSource = DSelect_Teacher_by_TeacherID
    TabOrder = 2
  end
  object DBEdit3: TDBEdit [8]
    Left = 40
    Top = 85
    Width = 264
    Height = 21
    Anchors = [akLeft, akTop, akRight]
    DataField = 'FirstName'
    DataSource = DSelect_Teacher_by_TeacherID
    TabOrder = 3
  end
  object DBEdit4: TDBEdit [9]
    Left = 40
    Top = 122
    Width = 264
    Height = 21
    Anchors = [akLeft, akTop, akRight]
    DataField = 'LastName'
    DataSource = DSelect_Teacher_by_TeacherID
    TabOrder = 4
  end
  object DBEdit5: TDBEdit [10]
    Left = 40
    Top = 158
    Width = 264
    Height = 21
    Anchors = [akLeft, akTop, akRight]
    DataField = 'FullName'
    DataSource = DSelect_Teacher_by_TeacherID
    TabOrder = 5
  end
  object DBEdit7: TDBEdit [11]
    Left = 8
    Top = 192
    Width = 296
    Height = 21
    Anchors = [akLeft, akTop, akRight]
    DataField = 'PhoneNumbers'
    DataSource = DSelect_Teacher_by_TeacherID
    TabOrder = 6
  end
  inherited ActionList: TActionList
    Left = 87
    Top = 264
  end
  inherited xpWindow1: TxpWindow
    Left = 128
    Top = 264
  end
  object Select_Teacher_by_TeacherID: TADOStoredProc
    Connection = Dm.YeganehConnection
    CursorType = ctStatic
    ProcedureName = 'Select_Teacher_by_TeacherID;1'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
        Value = 0
      end
      item
        Name = '@TeacherID'
        Attributes = [paNullable]
        DataType = ftSmallint
        Precision = 5
        Value = 1
      end>
    Left = 16
    Top = 32
    object Select_Teacher_by_TeacherIDTeacherID: TSmallintField
      FieldName = 'TeacherID'
      ReadOnly = True
    end
    object Select_Teacher_by_TeacherIDTeacherCode: TStringField
      FieldName = 'TeacherCode'
    end
    object Select_Teacher_by_TeacherIDFirstName: TWideStringField
      FieldName = 'FirstName'
      Size = 50
    end
    object Select_Teacher_by_TeacherIDLastName: TWideStringField
      FieldName = 'LastName'
      Size = 50
    end
    object Select_Teacher_by_TeacherIDFullName: TWideStringField
      FieldName = 'FullName'
      Size = 80
    end
    object Select_Teacher_by_TeacherIDTitle: TWideStringField
      FieldName = 'Title'
    end
    object Select_Teacher_by_TeacherIDPhoneNumbers: TWideStringField
      FieldName = 'PhoneNumbers'
      Size = 100
    end
  end
  object DSelect_Teacher_by_TeacherID: TDataSource
    AutoEdit = False
    DataSet = Select_Teacher_by_TeacherID
    Left = 136
    Top = 8
  end
  object ActionManager1: TActionManager
    ActionBars = <
      item
        Items = <
          item
            Action = DataSetPost1
            ImageIndex = 38
          end
          item
            Action = DataSetEdit1
            ImageIndex = 71
          end
          item
            Action = DataSetCancel1
            ImageIndex = 72
          end>
        ActionBar = ActionMainMenuBar1
      end>
    Images = Dm.LetterImages
    Left = 80
    Top = 8
    StyleName = 'XP Style'
    object DataSetInsert1: TDataSetInsert
      Category = 'Dataset'
      Caption = 'AInsert'
      DataSource = DSelect_Teacher_by_TeacherID
    end
    object DataSetEdit1: TDataSetEdit
      Category = 'Dataset'
      Caption = #1608#1610#1585#1575#1610#1588
      ImageIndex = 71
      DataSource = DSelect_Teacher_by_TeacherID
    end
    object DataSetPost1: TDataSetPost
      Category = 'Dataset'
      Caption = #1584#1582#1610#1585#1607
      ImageIndex = 38
      DataSource = DSelect_Teacher_by_TeacherID
    end
    object DataSetCancel1: TDataSetCancel
      Category = 'Dataset'
      Caption = #1575#1606#1589#1585#1575#1601
      ImageIndex = 72
      DataSource = DSelect_Teacher_by_TeacherID
    end
    object Action1: TAction
      Category = 'Dataset'
      Caption = #1582#1585#1608#1580
      ImageIndex = 0
    end
  end
end
