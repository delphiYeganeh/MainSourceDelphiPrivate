inherited FrInstructorExamInfo_byDate: TFrInstructorExamInfo_byDate
  Left = 231
  Top = 129
  Width = 800
  Height = 600
  Caption = #1575#1591#1604#1575#1593#1575#1578' '#1578#1581#1604#1610#1604' '#1570#1586#1605#1608#1606#1607#1575#1610#1610' '#1705#1607' '#1575#1587#1578#1575#1583' '#1583#1585' '#1570#1606' '#1605#1583#1585#1587' '#1576#1608#1583#1607' '#1575#1587#1578
  OldCreateOrder = True
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object number: TLabel [0]
    Left = 744
    Top = 528
    Width = 23
    Height = 13
    Caption = #1578#1593#1583#1575#1583
  end
  object Panel1: TPanel [1]
    Left = 0
    Top = 0
    Width = 792
    Height = 59
    Align = alTop
    TabOrder = 0
    object Label1: TLabel
      Left = 724
      Top = 8
      Width = 51
      Height = 14
      Caption = #1705#1583' '#1575#1587#1578#1575#1583
      FocusControl = DBEdit1
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label4: TLabel
      Left = 724
      Top = 33
      Width = 16
      Height = 14
      Caption = #1606#1575#1605
      FocusControl = DBEdit4
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object DBEdit1: TDBEdit
      Left = 536
      Top = 4
      Width = 128
      Height = 21
      DataField = 'TeacherCode'
      DataSource = Dm.Dselect_teacher_by_where
      TabOrder = 0
    end
    object DBEdit4: TDBEdit
      Left = 415
      Top = 29
      Width = 249
      Height = 21
      DataField = 'FullName'
      DataSource = Dm.Dselect_teacher_by_where
      TabOrder = 1
    end
  end
  object Panel2: TPanel [2]
    Left = 0
    Top = 59
    Width = 792
    Height = 41
    Align = alTop
    TabOrder = 1
    object Label3: TLabel
      Left = 523
      Top = 12
      Width = 12
      Height = 14
      Caption = #1578#1575' '
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label6: TLabel
      Left = 661
      Top = 12
      Width = 82
      Height = 14
      Caption = #1578#1575#1585#1610#1582' '#1570#1586#1605#1608#1606' '#1575#1586' '
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object EndExamDate: TEdit
      Left = 400
      Top = 8
      Width = 121
      Height = 21
      TabOrder = 0
      Text = '1390/12/29'
    end
    object BeginExamDate: TEdit
      Left = 544
      Top = 8
      Width = 121
      Height = 21
      TabOrder = 1
      Text = '1370/01/01'
    end
    object xpBitBtn1: TxpBitBtn
      Left = 280
      Top = 5
      Width = 110
      Height = 30
      startColor = 16645629
      EndColor = 14666957
      Caption = #1576#1585#1608#1586' '#1585#1587#1575#1606#1610
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ImageList = Dm.LetterImages
      ImageIndex = 28
      Gradient = True
      TabOrder = 2
      TabStop = True
      OnClick = xpBitBtn1Click
    end
  end
  object Button3: TxpBitBtn [3]
    Left = 227
    Top = 519
    Width = 100
    Height = 35
    startColor = 16645629
    EndColor = 14666957
    Caption = 'Word'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clNavy
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Glyph.Data = {
      
      38AE6138AE6138AE6138}
    Gradient = True
    TabOrder = 2
    TabStop = True
    OnClick = Button3Click
  end
  object Button4: TxpBitBtn [4]
    Left = 125
    Top = 519
    Width = 100
    Height = 35
    startColor = 16645629
    EndColor = 14666957
    Caption = 'Excel'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clNavy
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Glyph.Data = {
      
      5D417B57417B57417B57}
    Gradient = True
    TabOrder = 3
    TabStop = True
    OnClick = Button4Click
  end
  object Button5: TxpBitBtn [5]
    Left = 23
    Top = 519
    Width = 100
    Height = 35
    startColor = 16645629
    EndColor = 14666957
    Caption = #1582#1585#1608#1580
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clNavy
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Glyph.Data = {
      
      1DFEA31DFEA31DFEA31DFEA31DFEA21AFE9B0BFED99E}
    Gradient = True
    TabOrder = 4
    TabStop = True
    OnClick = Button5Click
  end
  object Button6: TxpBitBtn [6]
    Left = 334
    Top = 519
    Width = 123
    Height = 35
    startColor = 16645629
    EndColor = 14666957
    Caption = #1575#1606#1578#1582#1575#1576' '#1587#1578#1608#1606' '#1607#1575
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clNavy
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    Glyph.Data = {
      
      A116FFA116FFA116FFA116FFA116FFA116FFA116FFA218FF9C0A}
    ImageIndex = 2
    Gradient = True
    TabOrder = 5
    TabStop = True
    OnClick = Button6Click
  end
  object Panel3: TPanel [7]
    Left = 0
    Top = 100
    Width = 792
    Height = 405
    Align = alTop
    Caption = 'Panel3'
    TabOrder = 6
    object YDBGrid1: TYDBGrid
      Left = 1
      Top = 1
      Width = 790
      Height = 168
      Cursor = crHandPoint
      Align = alTop
      Color = 16770790
      DataSource = DGet_InstructorExamInfo_ByDate
      ReadOnly = True
      TabOrder = 0
      TitleFont.Charset = ARABIC_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Tahoma'
      TitleFont.Style = []
      YeganehColor = True
      YeganehFinish = False
      PageNumber = False
      RecordNumber = False
      HasTime = False
      Oriention = poLandscape
      HasDate = False
      HasIndex = False
      PrintTitle = #1604#1610#1587#1578' '#1578#1607#1610#1607' '#1588#1583#1607' '#1578#1608#1587#1591' '#1606#1585#1605' '#1575#1601#1586#1575#1585' '#1610#1711#1575#1606#1607
      InvertFarsiDate = True
      TitleSort = True
      AutoInsert = False
      FooterFields = 'Count'
      Columns = <
        item
          Expanded = False
          FieldName = 'ExamTitle'
          Title.Caption = #1593#1606#1608#1575#1606' '#1570#1586#1605#1608#1606
          Width = 133
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ExamDate'
          Title.Caption = #1578#1575#1585#1610#1582' '#1570#1586#1605#1608#1606
          Width = 65
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Difficulty'
          Title.Caption = #1583#1588#1608#1575#1585#1610' '#1705#1604
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'Discrimination'
          Title.Caption = #1575#1601#1578#1585#1575#1602' '#1705#1604
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'CorrectItemPoint'
          Title.Caption = #1575#1605#1578#1610#1575#1586' '#1662#1575#1587#1582' '#1583#1585#1587#1578
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'ExamVariance'
          Title.Caption = #1608#1575#1585#1610#1575#1606#1587
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'Number_of_Question'
          Title.Caption = #1578#1593#1583#1575#1583' '#1587#1574#1608#1575#1604
          Width = 60
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Number_of_Items'
          Title.Caption = #1578#1593#1583#1575#1583' '#1711#1586#1610#1606#1607
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'number_of_applicant'
          Title.Caption = #1578#1593#1583#1575#1583' '#1583#1575#1608#1591#1604#1576
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'WrongItemPoint'
          Title.Caption = #1575#1605#1578#1610#1575#1586' '#1662#1575#1587#1582' '#1606#1575#1583#1585#1587#1578
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'BlankItemPoint'
          Title.Caption = #1575#1605#1578#1610#1575#1586' '#1576#1583#1608#1606' '#1662#1575#1587#1582
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'StandardError'
          Title.Caption = #1582#1591#1575#1610' '#1575#1587#1578#1575#1606#1583#1575#1585#1583
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'RPBIS'
          Title.Caption = 'RPBIS '#1705#1604
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'NumberOf_1item_Question'
          Title.Caption = ' 1 '#1711#1586#1610#1606#1607' '#1575#1610' - '#1705#1604
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'NumberOf_2item_Question'
          Title.Caption = ' 2 '#1711#1586#1610#1606#1607' '#1575#1610' - '#1705#1604
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'NumberOf_3item_Question'
          Title.Caption = ' 3 '#1711#1586#1610#1606#1607' '#1575#1610'_'#1705#1604
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'NumberOf_4item_Question'
          Title.Caption = ' '#1670#1607#1575#1585' '#1711#1586#1610#1606#1607' '#1575#1610'-'#1705#1604
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'MinPassedMark'
          Title.Caption = #1705#1605#1578#1585#1610#1606' '#1606#1605#1585#1607' '#1602#1576#1608#1604#1610
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'Alpha'
          Title.Caption = 'alpha'
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'TermCode'
          Title.Caption = #1578#1585#1605' '#1570#1586#1605#1608#1606
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'avgmark'
          Title.Caption = #1605#1610#1575#1606#1711#1610#1606' '#1606#1605#1585#1575#1578
          Width = 75
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'varmark'
          Title.Caption = #1608#1575#1585#1610#1575#1606#1587' '#1606#1605#1585#1575#1578
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'minmark'
          Title.Caption = #1705#1605#1578#1585#1610#1606' '#1606#1605#1585#1607
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'maxmark'
          Title.Caption = #1576#1610#1588#1578#1585#1610#1606' '#1606#1605#1585#1607
          Width = 71
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Total_avgmark'
          Title.Caption = #1605#1610#1575#1606#1711#1610#1606' '#1606#1605#1585#1575#1578' '#1570#1586#1605#1608#1606
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Total_maxmark'
          Title.Caption = #1576#1610#1588#1578#1585#1610#1606' '#1606#1605#1585#1607' '#1570#1586#1605#1608#1606
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'Total_minmark'
          Title.Caption = #1705#1605#1578#1585#1610#1606' '#1606#1605#1585#1607' '#1570#1586#1605#1608#1606
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'Total_varmark'
          Title.Caption = #1608#1575#1585#1610#1575#1606#1587' '#1570#1586#1605#1608#1606
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'Total_AcceptedCount'
          Title.Caption = #1578#1593#1583#1575#1583' '#1602#1576#1608#1604#1610' '#1570#1586#1605#1608#1606
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Total_AllCount'
          Title.Caption = #1578#1593#1583#1575#1583' '#1705#1604' - '#1570#1586#1605#1608#1606
          Visible = True
        end>
    end
    object YDBGrid2: TYDBGrid
      Left = 1
      Top = 169
      Width = 790
      Height = 235
      Cursor = crHandPoint
      Align = alClient
      Color = 16770790
      DataSource = DSelect_Applicant_by_Examid_TrainingCourseID
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
      ReadOnly = True
      TabOrder = 1
      TitleFont.Charset = ARABIC_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Tahoma'
      TitleFont.Style = []
      YeganehColor = True
      YeganehFinish = False
      PageNumber = False
      RecordNumber = False
      HasTime = False
      Oriention = poLandscape
      HasDate = False
      HasIndex = False
      PrintTitle = #1604#1610#1587#1578' '#1578#1607#1610#1607' '#1588#1583#1607' '#1578#1608#1587#1591' '#1606#1585#1605' '#1575#1601#1586#1575#1585' '#1610#1711#1575#1606#1607
      InvertFarsiDate = True
      TitleSort = True
      AutoInsert = False
      FooterFields = 'Count'
      Columns = <
        item
          Expanded = False
          FieldName = 'ApplicantCode'
          Title.Caption = #1588#1605#1575#1585#1607' '#1583#1575#1606#1588#1580#1608#1610#1610
          Width = 104
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'FullName'
          Title.Caption = #1606#1575#1605' '#1705#1575#1605#1604
          Width = 102
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'FacultyTitle'
          Title.Caption = #1583#1575#1606#1588#1705#1583#1607
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'MajorTitle'
          Title.Caption = #1585#1588#1578#1607
          Width = 101
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'FinalMark'
          Title.Caption = #1606#1605#1585#1607' '#1606#1607#1575#1610#1610
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ord'
          Title.Caption = #1585#1578#1576#1607
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ACCEPT'
          Title.Caption = #1602#1576#1608#1604' / '#1585#1583
          Width = 61
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'GroupingTitle'
          Title.Caption = #1711#1585#1608#1607' '
          Width = 69
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'FirstName'
          Title.Caption = #1606#1575#1605
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'LastName'
          Title.Caption = #1606#1575#1605' '#1582#1575#1606#1608#1575#1583#1711#1610
          Visible = False
        end
        item
          Expanded = False
          FieldName = 'Termcode'
          Title.Caption = #1578#1585#1605' '#1608#1585#1608#1583
          Visible = True
        end>
    end
  end
  inherited ActionList: TActionList
    Left = 415
    Top = 272
  end
  inherited xpWindow1: TxpWindow
    Left = 200
    Top = 376
  end
  object Get_InstructorExamInfo_ByDate: TADOStoredProc
    Connection = Dm.YeganehConnection
    CursorType = ctStatic
    AfterScroll = Get_InstructorExamInfo_ByDateAfterScroll
    ProcedureName = 'Get_InstructorExamInfo_ByDate'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
        Value = 0
      end
      item
        Name = '@TeacherID'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@BeginExamDate'
        Attributes = [paNullable]
        DataType = ftString
        Size = 10
        Value = Null
      end
      item
        Name = '@EndExamDate'
        Attributes = [paNullable]
        DataType = ftString
        Size = 10
        Value = Null
      end
      item
        Name = '@examid'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end>
    Left = 288
    Top = 136
    object Get_InstructorExamInfo_ByDateExamID: TAutoIncField
      FieldName = 'ExamID'
      ReadOnly = True
    end
    object Get_InstructorExamInfo_ByDateExamTitle: TWideStringField
      FieldName = 'ExamTitle'
      Size = 50
    end
    object Get_InstructorExamInfo_ByDateMainCourseID: TSmallintField
      FieldName = 'MainCourseID'
    end
    object Get_InstructorExamInfo_ByDateMainDesignerTeacherID: TSmallintField
      FieldName = 'MainDesignerTeacherID'
    end
    object Get_InstructorExamInfo_ByDateExamDate: TStringField
      FieldName = 'ExamDate'
      FixedChar = True
      Size = 10
    end
    object Get_InstructorExamInfo_ByDateCorrectItemPoint: TFloatField
      FieldName = 'CorrectItemPoint'
    end
    object Get_InstructorExamInfo_ByDateWrongItemPoint: TFloatField
      FieldName = 'WrongItemPoint'
    end
    object Get_InstructorExamInfo_ByDateBlankItemPoint: TFloatField
      FieldName = 'BlankItemPoint'
    end
    object Get_InstructorExamInfo_ByDateMinPassedMark: TFloatField
      FieldName = 'MinPassedMark'
    end
    object Get_InstructorExamInfo_ByDateExamTypeID: TWordField
      FieldName = 'ExamTypeID'
    end
    object Get_InstructorExamInfo_ByDateNumber_of_Items: TWordField
      FieldName = 'Number_of_Items'
    end
    object Get_InstructorExamInfo_ByDateNumber_of_Question: TIntegerField
      FieldName = 'Number_of_Question'
    end
    object Get_InstructorExamInfo_ByDatenumber_of_applicant: TIntegerField
      FieldName = 'number_of_applicant'
    end
    object Get_InstructorExamInfo_ByDateTexFormatID: TIntegerField
      FieldName = 'TexFormatID'
    end
    object Get_InstructorExamInfo_ByDateAnswerKey: TStringField
      FieldName = 'AnswerKey'
      Size = 1000
    end
    object Get_InstructorExamInfo_ByDateDifficulty: TFloatField
      FieldName = 'Difficulty'
    end
    object Get_InstructorExamInfo_ByDateDiscrimination: TFloatField
      FieldName = 'Discrimination'
    end
    object Get_InstructorExamInfo_ByDateStandardError: TFloatField
      FieldName = 'StandardError'
    end
    object Get_InstructorExamInfo_ByDateAlpha: TFloatField
      FieldName = 'Alpha'
    end
    object Get_InstructorExamInfo_ByDateExamVariance: TFloatField
      FieldName = 'ExamVariance'
    end
    object Get_InstructorExamInfo_ByDateNumberOf_1item_Question: TSmallintField
      FieldName = 'NumberOf_1item_Question'
    end
    object Get_InstructorExamInfo_ByDateNumberOf_2item_Question: TSmallintField
      FieldName = 'NumberOf_2item_Question'
    end
    object Get_InstructorExamInfo_ByDateNumberOf_3item_Question: TSmallintField
      FieldName = 'NumberOf_3item_Question'
    end
    object Get_InstructorExamInfo_ByDateNumberOf_4item_Question: TSmallintField
      FieldName = 'NumberOf_4item_Question'
    end
    object Get_InstructorExamInfo_ByDateNumberOf_5item_Question: TSmallintField
      FieldName = 'NumberOf_5item_Question'
    end
    object Get_InstructorExamInfo_ByDateRPBIS: TFloatField
      FieldName = 'RPBIS'
    end
    object Get_InstructorExamInfo_ByDateTermCode: TStringField
      FieldName = 'TermCode'
      FixedChar = True
      Size = 3
    end
    object Get_InstructorExamInfo_ByDateCourseCode: TStringField
      FieldName = 'CourseCode'
      Size = 15
    end
    object Get_InstructorExamInfo_ByDateCourseTitle: TWideStringField
      FieldName = 'CourseTitle'
      Size = 60
    end
    object Get_InstructorExamInfo_ByDateCourseTermCode: TStringField
      FieldName = 'CourseTermCode'
      FixedChar = True
      Size = 3
    end
    object Get_InstructorExamInfo_ByDateGrourpCode: TStringField
      FieldName = 'GrourpCode'
      Size = 5
    end
    object Get_InstructorExamInfo_ByDateStartDate: TStringField
      FieldName = 'StartDate'
      FixedChar = True
      Size = 10
    end
    object Get_InstructorExamInfo_ByDateTrainingCourseID: TAutoIncField
      FieldName = 'TrainingCourseID'
      ReadOnly = True
    end
    object Get_InstructorExamInfo_ByDateTotal_avgmark: TFloatField
      FieldName = 'Total_avgmark'
      ReadOnly = True
    end
    object Get_InstructorExamInfo_ByDateTotal_minmark: TFloatField
      FieldName = 'Total_minmark'
      ReadOnly = True
    end
    object Get_InstructorExamInfo_ByDateTotal_maxmark: TFloatField
      FieldName = 'Total_maxmark'
      ReadOnly = True
    end
    object Get_InstructorExamInfo_ByDateTotal_varmark: TFloatField
      FieldName = 'Total_varmark'
      ReadOnly = True
    end
    object Get_InstructorExamInfo_ByDateTotal_AcceptedCount: TIntegerField
      FieldName = 'Total_AcceptedCount'
      ReadOnly = True
    end
    object Get_InstructorExamInfo_ByDateTotal_AllCount: TIntegerField
      FieldName = 'Total_AllCount'
      ReadOnly = True
    end
    object Get_InstructorExamInfo_ByDateavgmark: TFloatField
      FieldName = 'avgmark'
      ReadOnly = True
    end
    object Get_InstructorExamInfo_ByDateminmark: TFloatField
      FieldName = 'minmark'
      ReadOnly = True
    end
    object Get_InstructorExamInfo_ByDatemaxmark: TFloatField
      FieldName = 'maxmark'
      ReadOnly = True
    end
    object Get_InstructorExamInfo_ByDatevarmark: TFloatField
      FieldName = 'varmark'
      ReadOnly = True
    end
    object Get_InstructorExamInfo_ByDateAcceptedCount: TIntegerField
      FieldName = 'AcceptedCount'
      ReadOnly = True
    end
    object Get_InstructorExamInfo_ByDateAllCount: TIntegerField
      FieldName = 'AllCount'
      ReadOnly = True
    end
    object Get_InstructorExamInfo_ByDateKR20: TFloatField
      FieldName = 'KR20'
    end
  end
  object DGet_InstructorExamInfo_ByDate: TDataSource
    DataSet = Get_InstructorExamInfo_ByDate
    Left = 296
    Top = 192
  end
  object Select_Applicant_by_Examid_TrainingCourseID: TADOStoredProc
    Connection = Dm.YeganehConnection
    CursorType = ctStatic
    ProcedureName = 'Select_Applicant_by_Examid_TrainingCourseID;1'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
        Value = Null
      end
      item
        Name = '@ExamID'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end
      item
        Name = '@TrainingCourseID'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end>
    Left = 88
    Top = 360
  end
  object DSelect_Applicant_by_Examid_TrainingCourseID: TDataSource
    DataSet = Select_Applicant_by_Examid_TrainingCourseID
    Left = 88
    Top = 320
  end
end
