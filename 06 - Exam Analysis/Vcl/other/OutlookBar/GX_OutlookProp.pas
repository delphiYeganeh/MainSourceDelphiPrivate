...
type
  ...
  TfmGXOutlookEditor = class(TForm)
    ...
    procedure cmbActionKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
  private
    Outlook: TGXCustomOutlookBar;
    FUpdateFlag: Boolean;
  public
    { Public declarations }
    CompOutLook: TGXCustomOutLookBar;
    Modified: Boolean;
  end;

  type TGXOutlookEditor = class(TComponentEditor)
  public
...
procedure CopyOutlook(Source,Dest: TGXCustomOutLookBar);
...
type
  TFEGXOutlookHack = class(TGXCustomOutLookBar);
...
function TGXOutlookEditor.GetVerb(Index: Integer): string;
Begin
  Result := '&Edit Items...';
End;

function TGXOutlookEditor.GetVerbCount: Integer;
Begin
  Result := 1;
End;

procedure TGXOutLookEditor.ExecuteVerb(Index: Integer);
var
  Dlg: TfmGXOutLookEditor;
Begin
  Dlg:=TfmGXOutlookeditor.Create(Application);
  Try
    Dlg.CompOutlook := TGXCustomOutlookbar(Component);
    CopyOutlook(TGXCustomOutlookbar(Component),Dlg.Outlook);
    ...
    begin
      ...
      begin
        CopyOutlook(Dlg.Outlook, TGXCustomOutlookbar(Component));
        Designer.Modified;
      end;
    end;
  finally
...
begin
  Modified := False;
  FUpdateFlag := false;
  Outlook := TGXCustomOutlookBar.Create(self);
