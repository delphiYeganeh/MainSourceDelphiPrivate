...
function capCreateCaptureWindowA(lpszWindowName: pchar; dwStyle: dword; x, y, nWidth, nHeight: integer; hwndParent: HWND; nID: integer): HWND; stdcall; external DLL2;

function capGetDriverDescriptionA(wDriverIndex: integer; lpszName: pchar; cnName: integer; lpszVer: pchar; cbVer: integer): longbool; stdcall; external DLL2;
...
begin
  ...
  fWndC := capCreateCaptureWindowA(pchar(name), WS_CHILD, 0, 0, ClientWidth, ClientHeight, IEFindHandle(self), 0);
  if fShowVideo then
...
<FM>Description<FN>
Sets Frozen property to True. When you set the Frozen property to True, the video input is locked and the current image is copied to Bitmap field (in 24 bit x pixel). If you want to process/zoom/navigate the captured image, you must set the ShowVideo property to False.
