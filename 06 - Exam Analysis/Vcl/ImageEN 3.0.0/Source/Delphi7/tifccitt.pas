...
begin
  ...
  begin
    ...
    begin
      fillchar(pbyte(int64(dstbuf) + ibi)^, (rl - q) shr 3, 0);
      inc(posw, abi);
      inc(q, abi);
    end
    else
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if rl <> G3CODE_INVALID then
        begin
          if WB <> 0 then
...
begin
  ...
  begin
    ...
    if ibb = 0 then
    begin
      dw := pointer(int64(buf) + ibi);
      if CL = 0 then
        while (ipos < Width) and (dw^ = $FF) do
        begin
          inc(ipos, 8);
          inc(dw);
        end
      else
        while (ipos < Width) and (dw^ = 0) do
        begin
          inc(ipos, 8);
          inc(dw);
        end;
      ...
      12: // EOF
        begin
          ...
          begin
            ...
            if v = 1 then
            begin // decodifica ad una dimensione
              if a0 < 0 then
                a0 := 0;
...
begin
  ...
  begin
    ...
    begin
      ...
      // set dest
      bp := pbyte(int64(wbuf) + (int64(wposb) shr 3));
      if r <> 0 then
...
begin
  ...
  begin
    ...
    bwr := pbyte(int64(wbuf) + rt)^; // bits da scrivere alla prossima
  end
  else
    bwrl := 0;
...
begin
  ...
  begin
    ...
    _PutRLCode(wbuf, wpos, rt, bw, FillOrder); // modifica wposb
    bw := not bw;
  end;
end;
...
begin
  ...
  // invia EOL+1 (prima riga 1D)
    _PutBits(wbuf, wpos, 12, $1, FillOrder); // scrive EOF
    _PutBits(wbuf, wpos, 1, $1, FillOrder); // mark codifica 1D
    _CCITTHuffmanPutLine(rdata, wb, wbuf, wpos, FillOrder); // codifica 1D
    getmem(predline, rowlen);
    copymemory(predline, rdata, rowlen);
  end
  else if rdata <> nil then
  begin
    // ALTRE RIGHE
  // invia EOL+0
    _PutBits(wbuf, wpos, 12, $1, FillOrder); // scrive EOF
    _PutBits(wbuf, wpos, 1, $0, FillOrder); // mark codifica 2D
...
begin
  ...
  begin
    w:=T4Codes[0,q].code;
    ReverseBits(w); w:=w shr (32-T4Codes[0,q].dim);
    T4Codes[0,q].code:=w;
    w:=T4Codes[1,q].code;
    ReverseBits(w); w:=w shr (32-T4Codes[1,q].dim);
    T4Codes[1,q].code:=w;
  end;
  *)
end;
