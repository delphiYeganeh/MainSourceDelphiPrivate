...
uses
  ...
  Controls, StdCtrls,math, Forms, hyieutils, hyiedefs, ietextc, imageenio, iegdiplus;
...
// x,y è il centro del grip, mentre la dimensione è data dalla costante fObjGripSize
// ty=0 grip quadrato
// ty=1 grip rombo
...
var
  ...
  charwidth,CharPerLine,endp,yht:INTEGER;
  st,s:string;
    ...
    if aobj.Text <> nil then
      ...
      begin
        ...
        begin
          ...
          wcanvas.Brush.Style := bsClear;
           {yeganeh}
           if trim(aobj.Text)<>'' then
           begin
             charwidth:= wcanvas.TextWidth(aobj.Text) div length(aobj.Text) ;
             CharPerLine:=((x2-x1) div charwidth)-1;
             IF CharPerLine<1 THEN CharPerLine:=1;
              yht:=y1;
              st:=aobj.Text;
              while length(st)>0 do
               begin
               if pos(#13,st)<>0 then
                 endp:=pos(#13,st) else endp:=CharPerLine;
               if endp>CharPerLine then endp:=CharPerLine;
                 s:=copy(st,1,endp);

               WCanvas.TextRect(rect(x2-wcanvas.TextWidth(s), yht, x2,y2), x2-wcanvas.TextWidth(s), yht ,s);
//               WCanvas.TextRect(rect(x1, yht, x2,y2), x1 + ox, yht ,s);
                yht:=yht+wcanvas.TextHeight(st);
               delete(st,1,endp );
              end;
            end;
        ...
        begin
          ...
          begin
            ...
            while (curlen >= llength) and (pc^ <> #0) do
            begin
              x00 := dx0;
              y00 := dy0;
              if curlen <> 0 then
              begin
                dx0 := dx0 + (dx1 - dx0) * llength / curlen;
                dy0 := dy0 + (dy1 - dy0) * llength / curlen;
                drawc(WCanvas, rect(x1, y1, x2, y2), x00, y00, dx0, dy0, pc^, aobj.CurvedCharRot, plim);
              end;
              curlen := curlen - llength;
              inc(pc);
            end;
            dx0 := dx1;
            dy0 := dy1;
...
begin
  ...
  case Key of
    ...
    VK_RETURN:
    if ssctrl in shift then  {yeganeh}
      begin
        Key := 0;
        CancelInteracts;
        DoVectorialChanged;
