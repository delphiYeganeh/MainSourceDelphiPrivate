...
function OutputMessage(s: string): string;
 begin
    if s='' then
      Result:='Ìﬂ  «—ÌŒ „⁄ »— ÂÃ—Ì ŒÊ—‘ÌœÌ ‰Ì”  '''''
    else
     Result:=s+' Ìﬂ  «—ÌŒ „⁄ »— ÂÃ—Ì ŒÊ—‘ÌœÌ ‰Ì” ';
 end;
...
procedure TShamsiDateEdit.WantsToExit(Sender: TObject);
 ...
 begin
  if FCheckValidity then
   if not IsValidDate then
    begin
     ShowMessage(OutputMessage(MyGetText));
     try Text:='';
