...
Type
  ...
  private
    ...
    FUNCTION GET_BTN_VALUE(BTN1,BTN2 : TSpeedButton):String;
  public
     ...
     //---
     DoRefresh:boolean;
     DelDBGridSettings : Boolean;//«“ ÿ—Ìﬁ ›—„  ‰ŸÌ„«  »«Ìê«‰Ì MainForm_DBGrid.Cfg ÃÂ  Õ–› ›«Ì·Â«Ì
...
Function TMainForm.GET_BTN_VALUE(BTN1,BTN2 : TSpeedButton):String;
begin
  if BTN1.Down then
    Result := 'DATE-AVAL'
  else
...
begin
  ...
  List.Align:=alClient;
  Panel4.Caption := '‰”ŒÂ ' + _SoftVersion;
  Panel5.Caption := ' «—ÌŒ ‰”ŒÂ '  +_SoftLastUpdate;
   // ÊÃÂ : «Ì „ﬁ«œÌ— œ— ›—„ "ê“«—‘ «“ ”‰œ" ‰Ì“ «” ›«œÂ ‘œÂ «‰œ
  LowOrange := RGB(255,95,0) ;
  LowTurquoise := RGB(0,175,240) ; //›Ì—Ê“Â «Ì
  Lowpink := RGB(240,90,160) ; //’Ê— Ì
  Caption := '‰—„ «›“«— »«Ìê«‰Ì '+_yeganeh;
...
begin
  ...
  if DisplayMode<>archive then //»—êÂ Ã” ÃÊ
    if Only_Current.Checked then //›ﬁÿ ”‰œÂ«Ì »«Ìê«‰Ì ‰‘œÂ
      FarchiveFolderID := 0
    else
      FarchiveFolderID := -1;  //»œÊ‰  Ìﬂ
...
//        archiveFolderID := 0;
  ...
  begin
    ...
    dm.ExecGet_All_Letter(archiveFolderID,Letter_Type,LetterFormat,dm.CurrentMYear,dm.SecID,SpinEdit.AsInteger,w,trim(edtWord.Text),trim(edtAttach.Text),GET_BTN_VALUE(HEAD,TAIL),SpinEdit.AsInteger);
  ...
  status3.Caption := ' ⁄œ«œ «”‰«œ :'+IntToStr(dm.Get_All_Letter.RecordCount);
  //status3.Caption := ' ⁄œ« «”‰«œ :'+IntToStr(dm.SHOW_ALL_LETTER.RecordCount);
  ...
  //  Status2.Caption := '«”‰«œ ò«—»— Ã«—Ì :'+IntToStr(_MyDocs div 3);//IntToStr(receivedNum);
  Status2.Caption := '«”‰«œ ò«—»— Ã«—Ì :'+IntToStr(receivedNum);
...
begin
  ...
  if not messageShow(25, True) then
    ...
    begin
      ...
      if DelDBGridSettings then  //«“ ÿ—Ìﬁ ›—„  ‰ŸÌ„«  »«Ìê«‰Ì MainForm_DBGrid.Cfg ÃÂ  Õ–› ›«Ì·Â«Ì
      begin
      ...
      if (DblclickAction = ViewWord) And  dm.Get_All_Letterhas_WordPage.AsBoolean then
         ...
         begin
            ...
            if LetterDataCount = 1 then
               ...
               begin
                  //«ê—  ⁄œ«œ ›«Ì·Â«Ì ÅÌÊ”  »Ì‘ «“ ÌﬂÌ »Êœ ›—„ ÅÌÊ” Â« »«“ ‘Êœ
...
begin
  ...
  begin
    for i :=0 to lettersDbGrid.Columns.Count - 1 do
      ...
      begin
        lettersDbGrid.Columns.Items[I].Title.Caption := IfThen(Trim(Dm.GetSystemValue(15)) = '', '›Ì·œ ò«—»—Ì 1', Trim(Dm.GetSystemValue(15)));
        lettersDbGrid.Columns.Items[I].Width := 120;
      end
      else if lettersDbGrid.Columns.Items[I].FieldName = 'UserMemo1' then
      begin
        lettersDbGrid.Columns.Items[I].Title.Caption := IfThen(Trim(Dm.GetSystemValue(45)) = '', '›Ì·œ ò«—»—Ì 2', Trim(Dm.GetSystemValue(45)));
        lettersDbGrid.Columns.Items[I].Width := 120;
      end
      else if lettersDbGrid.Columns.Items[I].FieldName = 'UserMemo2' then
      begin
        lettersDbGrid.Columns.Items[I].Title.Caption := IfThen(Trim(Dm.GetSystemValue(46)) = '', '›Ì·œ ò«—»—Ì 3', Trim(Dm.GetSystemValue(46)));
        lettersDbGrid.Columns.Items[I].Width := 120;
      end
      else if lettersDbGrid.Columns.Items[I].FieldName = 'IncommingNO' then
      begin
        lettersDbGrid.Columns.Items[I].Title.Caption := IfThen(Trim(Dm.GetSystemValue(52)) = '', '‘„«—Â ”‰œ', '‘„«—Â ' + Trim(Dm.GetSystemValue(52)));
        lettersDbGrid.Columns.Items[I].Width := 80;
      end
      else if lettersDbGrid.Columns.Items[I].FieldName = 'IncommingDate' then
      begin
        lettersDbGrid.Columns.Items[I].Title.Caption := IfThen(Trim(Dm.GetSystemValue(52)) = '', ' «—ÌŒ ”‰œ', ' «—ÌŒ ' + Trim(Dm.GetSystemValue(52)));
        lettersDbGrid.Columns.Items[I].Width := 80;
      end
      else if lettersDbGrid.Columns.Items[I].FieldName = 'IncommingMiladiDate_Calc' then
      begin
        lettersDbGrid.Columns.Items[I].Title.Caption := IfThen(Trim(Dm.GetSystemValue(52)) = '', ' «—ÌŒ ”‰œ »Â „Ì·«œÌ', ' «—ÌŒ ' + Trim(Dm.GetSystemValue(52)) + ' »Â „Ì·«œÌ');
        lettersDbGrid.Columns.Items[I].Width := 90;
      end
      else if lettersDbGrid.Columns.Items[I].FieldName = 'VersionNumber' then
      begin
        lettersDbGrid.Columns.Items[I].Title.Caption := IfThen(Trim(Dm.GetSystemValue(52)) = '', '‘„«—Â ÊÌ—«Ì‘ ”‰œ', '‘„«—Â ÊÌ—«Ì‘ ' + Trim(Dm.GetSystemValue(52)));
        lettersDbGrid.Columns.Items[I].Width := 100;
      end
      else if lettersDbGrid.Columns.Items[I].FieldName = 'VersionDate' then
      begin
        lettersDbGrid.Columns.Items[I].Title.Caption := IfThen(Trim(Dm.GetSystemValue(52)) = '', '¬Œ—Ì‰ ÊÌ—«Ì‘ ”‰œ', '¬Œ—Ì‰ ÊÌ—«Ì‘ ' + Trim(Dm.GetSystemValue(52)));
        lettersDbGrid.Columns.Items[I].Width := 100;
      end
      else if lettersDbGrid.Columns.Items[I].FieldName = 'Subject' then
      begin
        lettersDbGrid.Columns.Items[I].Title.Caption := IfThen(Trim(Dm.GetSystemValue(53)) = '', '„Ê÷Ê⁄', Trim(Dm.GetSystemValue(53)));
        lettersDbGrid.Columns.Items[I].Width := 100;
      end
      else if lettersDbGrid.Columns.Items[I].FieldName = 'Memo' then
      begin
        lettersDbGrid.Columns.Items[I].Title.Caption := IfThen(Trim(Dm.GetSystemValue(54)) = '', 'Œ·«’Â', Trim(Dm.GetSystemValue(54)));
        lettersDbGrid.Columns.Items[I].Width := 100;
      end;
  end;
end;
...
//            MainForm.SkinData1.LoadFromFile(SkinPathName)
            ...
            ShowMessage('  Ì«›  ‰‘œ  ' + SkinPathName + '  ﬂ«—»— ê—«„Ì ° ›«Ì· ê—«›ÌﬂÌ  ');
      end;
   //--
   if dm.UsersChangeByAdmin.Value then
   begin
     showmessage('ò«—»— ê—«„Ì ·ÿ›« Å”Ê—œ ŒÊœ —«  €ÌÌ— œÂÌœ');
...
begin
   ...
   if dm.Get_All_LetterRentStatus.AsInteger=1 then
      //ARent.Caption := '«„«‰  œÂÌ ”‰œ'
      ARent.Caption := '«„«‰  œÂÌ ' + Document
   else
      Arent.Caption := '»«“ê‘   ' + Document + ' «„«‰Ì';
   check;
end;
...
begin
  ...
  if Uppercase(Column.FieldName)=Uppercase('HasLetterAlerm') then
     Case dm.Get_All_LetterHasLetterAlerm.AsInteger of
        0: ImageIndex:=0;  //”‰œ »œÊ‰ ¬·«—„ «” 
        1: ImageIndex:=15; //”‰œ œ«—«Ì ¬·«—„ Ê·Ì »œÊ‰  «ŒÌ—
        2: ImageIndex:=16; //”‰œ œ«—«Ì ¬·«—„ Ê·Ì »«  «ŒÌ—
     end;   
...
begin
 ...
 if not Dm.UsersShowAllDoc.AsBoolean then
   if Dm.UsersId.AsInteger <> UserIdCreator then
    begin
     ShowMyMessage('ÅÌ€«„','ò«—»— ê—«„Ì ° ‘„« «Ã«“Â  €ÌÌ— «Ì‰ ”‰œ —« ‰œ«—Ìœ',[mbOK],mtInformation);
     Abort;
    end;
 *)
 if dm.Get_All_LetterLetter_Type.AsInteger=1 then
  ...
  begin
    ...
    begin
      ...
      begin // «Œ’«’ —‰ê
        ...
        Dm.Insert_LetterEdit(Dm.Get_All_LetterLetterID.AsInteger,'«Œ ’«’ —‰ê »Â ”‰œ');
      end
      else
      begin // ÊÌ—«Ì‘ —‰ê
        Exec_insert_UserLog('ChangeLetterColor',Dm.Get_All_LetterLetterID.AsInteger);
        Dm.Insert_LetterEdit(Dm.Get_All_LetterLetterID.AsInteger,'ÊÌ—«Ì‘ —‰ê ”‰œ');
      end;
    end
    else
    begin
       ...
       Dm.Insert_LetterEdit(Dm.Get_All_LetterLetterID.AsInteger,'Õ–› —‰ê ”‰œ');
     end;
  end;
  RefreshQuery;
end;
...
begin
   ...
   begin
      MBaseForm.MessageShowString(' «—ÌŒ À»  «‘ »«Â „Ì »«‘œ. ·ÿ›«  ’ÕÌÕ ‰„«ÌÌœ', False);
      MeSDate.SetFocus;
      Abort;
   end;
...
begin
  inherited;
   {Ranjbar 88.03.27}//À»  ”‰œ œ— “Ê‰ﬂ‰ „Ê—œ ‰Ÿ—
  ArchiveTree.SetFocus;
  with FrAddDocument do
  begin
    FrAddDocument := TFrAddDocument.Create(Application);
...
begin
  inherited;
  DBGrid_LoadColumns(Self.Name+'_Temp',lettersDbGrid);// ŸÌ„«  ÅÌ‘ ›—÷ ” Ê‰Â«
  ShowMessage(' ‰ŸÌ„«  «⁄„«· ‘œ' );
end;
...
begin
  ...
  if MessageDlg('¬Ì« „«Ì· »Â Õ–› «Ì‰ —òÊ—œ(Â«) Â” Ìœ ø', mtWarning, mbOKCancel, 0) = mrOK then
  begin
    MakeSelectedRecommites;
    if _selectedRecommitesID.Count > 0  then
...
//Commented By Hamed_Ansari_981205_End
        ShowMsgString('⁄„·Ì«  «‰Ã«„ ‘œ');
      end;// end of if SelectDirectory(' ⁄ÌÌ‰ „”Ì— ÃÂ  «—”«· ›«Ì· Â«','',Path) then
    end
    else     // »—«Ì ‰«„Â Â«Ì «‰ Œ«»Ì
    begin
      LetterID:= Dm.Get_All_LetterLetterID.AsInteger;
