  ...
  tvColorView.Refresh;
{
  _Query:=  'Select MAX(dc.Parent) as Parent, MAX(dc.Descriptions) as Descriptions, MAX(dc.Color) as Color, LD.ColorID ' +
            ...
            'Group by LD.ColorID ' +
            'Union ALL ' +
            'Select 0 as Parent, N'' „«„ „Ê«—œ'' as Descriptions,N''clBtnFace'' as Color ,-1 as ColorID ' +
            'Union ALL ' +
            'Select 0 as Parent, N''›«ﬁœ —‰ê'' as Descriptions,N''clBtnFace'' as Color ,-2 as ColorID ';
}
  _Query:=  'With Tree AS ( ' +
...
begin
  ...
  try
    if (Button = nbInsert) and (PageControl2.ActivePageIndex = 0) and (_MyPuletColors = '›«ﬁœ —‰ê') then
    ...
    if (Button <> nbInsert) then
    begin
      if (_MyPuletColors = '') or (_MyPuletColors = '›«ﬁœ —‰ê') or
...
//                if _BaseSQLConnection.GetKeyvalue('SaveFileType')= '1' then
  ...
  try
    if (_MyPuletColors <> '') and (_MyPuletColors <> '›«ﬁœ —‰ê') and (_MyPuletColors <> ' „«„ „Ê«—œ') then
      Edit1.Color:= StringToColor(_MyPuletColors);
    if (_MyPuletColors = '') or (_MyPuletColors = '›«ﬁœ —‰ê') or (DMScanImage.QrLetterDataColorID.AsString = '0') or (DMScanImage.QrLetterDataColorID.AsString = '-2') then
      Edit1.Color := clBtnFace;
  except
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ImageEnVectMain.DeSelect;
        ImageEnVectMain.UnSelAllObjects;
        ImageEnVectMain.LayersMergeAll;  //«‘Ì«Ì «÷«›Â ‘œÂ —« Â„  —ﬂÌ» „Ì ﬂ‰œ
        ImageEnVectMain.RemoveAllObjects;
...
begin
  inherited;
  LabelZoom .Caption :='»“—ê‰„«ÌÌ  :    '+ FloatToStr( Round(ImageEnVectMain.Zoom) )+'%';
end;
  ...
  begin
    try
      ...
      begin
        ...
        begin
          ChangeMultyTifToTifsAndSave(OpenPictureDialog.Files.Strings[I]);  //›«Ì· TIFF œ— Â— ’Ê—  »’Ê—  TIFF –ŒÌ—Â ‘Êœ
        end
        else if IsJpegFormat then  //›«Ì· JPG œ— ’Ê— Ì òÂ ›—„  Â„ JPG »«‘œ »« »’Ê—  JPG –ŒÌ—Â ‘Êœ
        begin
...
begin
  ...
  if ImageEnVectMain.Selected then
    ...
    ShowMessage ('«» œ« ‰«ÕÌÂ «Ì —« »—«Ì »—‘ œ«œ‰ »« „«Ê” «‰ Œ«» ò‰Ìœ');
