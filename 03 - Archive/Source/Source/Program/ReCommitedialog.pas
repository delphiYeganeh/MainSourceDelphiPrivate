...
uses
  ...
  CheckLst, YchecklistBox, ADODB, ImgList, DBTreeView, Menus;
     ...
     if OneOrg.Checked then
       begin
        ...
        lastUpdate := false;
         ...
         if (not CheckWorkGroup(Dm.Select_LetterReCommiteOrgID.AsInteger))
            or
            ( not GetHasSecureLetterAccess(Dm.Select_LetterReCommiteOrgID.AsInteger) and (dm.Get_All_LetterClassificationID.Value<>1))
          then
         begin
         ...
         //Add Sanaye 950721
         //Ã·ÊêÌ—Ì «“ «—Ã«⁄ ‰«ÂÂ Â«ÌÌ òÂ ⁄ÿ› Ì« ÅÌ—Ê „Õ—„«‰Â œ«—‰œ
         if (not GetHasSecureLetterAccess(Dm.Select_LetterReCommiteOrgID.AsInteger)) and (CheckClassificationFollowRetroactionLetter(mLetterID)) then
         begin
            ...
            tmp:=OrgTree.Items[0];
             ...
             begin
               if (tmp.StateIndex=2) then
                   if not CheckWorkGroup(integer(tmp.Data))
                   or
                   (not  GetHasSecureLetterAccess(integer(tmp.Data)) and (dm.Get_All_LetterClassificationID.Value<>1))
                   then
                   begin
               ...
               //Add Sanaye 950721
               //Ã·ÊêÌ—Ì «“ «—Ã«⁄ ‰«ÂÂ Â«ÌÌ òÂ ⁄ÿ› Ì« ÅÌ—Ê „Õ—„«‰Â œ«—‰œ
               if (tmp.StateIndex=2) and (not GetHasSecureLetterAccess(integer(tmp.Data))) and (CheckClassificationFollowRetroactionLetter(mLetterID)) then
               begin
                MessageShowString('œ— ⁄ÿ› Ê ÅÌ—Ê ‰«„Â œ—ŒÊ«” Ì ° ‰«„Â Â«Ì œ«—«Ì ÿ»ﬁÂ »‰œÌ ÊÃÊœ œ«—œ ' + #13#10 + '«„ò«‰ «—Ã«⁄ »Â ‘Œ’ ›«ﬁœ œ” —”Ì „Õ—„«‰Â ÊÃÊœ ‰œ«—œ', False);
                Exit;
               end;
...
begin
  if MoreOrg.Checked then GroupFlag1:=True;
  if OneOrg.Checked then GroupFlag1:=False;
