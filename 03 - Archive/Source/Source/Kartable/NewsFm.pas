...
uses
  ...
  Menus, xpBitBtn, xpWindow, ExtActns;
...
begin
   ...
   begin
      SpNews.Close;
      SpNews.Parameters.ParamByName('@Like').Value := aLike;//äãÇíÔ 500 Ñ˜æÑÏ ÂÎÑ
      SpNews.Open;
      SpNews.Sort :='NewsID';
      SpNews.Last;
   end;
...
begin
  inherited;
   ...
   begin
      ShowMessage('áØÝÇ ÞÈá ÇÒ ÒÇÑÔ íÑí ÇØáÇÚÇÊ ÎæÏ ÑÇ ÐÎíÑå äãÇííÏ');
      Exit;
   end;
   ...
   begin
      Try
         ...
         if Sender = NShow  then
            Rep_Show(frxRepNews,rtShow);//íÔ äãÇíÔ
         if Sender = NPrint  then
            Rep_Show(frxRepNews,rtPrint);//Ç ÓÑíÚ
         if Sender = NDesign  then
            Rep_Show(frxRepNews,rtDesign);//ØÑÇÍí
      Finally
         QrRepNews.Close;
      End;
   end;
