unit BusinessLayer;
interface
uses DB,ADODB,dialogs,Graphics,Jpeg,Types ,Forms,Classes ,Controls , BaseUnit, SysUtils;
...
Procedure Exec_SendLetter_To_Other_Sec(NewSecID:integer;LetterID:Integer);
...
Function Exec_get_LetterID_ByRetroActionNo(RetroActionNo:widestring;LetterFormat:byte;Myear:byte;SecretariatID:integer):integer;
...
Function Exec_Get_LetterID_ByInCommingNo(InCommingNo:widestring;LetterFormat:byte;Myear:byte;SecretariatID:integer):integer;
Function Exec_Get_LetterID_ByIndicatorid(indicatrid:integer;letter_type:byte;LetterFormat:byte;Myear:byte;SecretariatID:integer):integer;
...
function  Get_SysAppMessage(MsgID:integer):wideString;
 ...
 begin
  ...
  with ADOSP.Parameters.AddParameter do
    ...
    ADOSP.Open;
    try Result:=ADOSP.FieldByname('FarsiMessage').Value; except result:='„⁄«œ· ›«—”Ì Œÿ«Ì òœ '+IntToStr(MsgID)+ ' œ— »«‰ò «ÿ·«⁄« Ì ‘„« »—Ê“ ‰Ì”  »« ‘—ò    „«” »êÌ—Ìœ' end;
end;
...
Function Exec_get_LetterID_ByIndicatorid(indicatrid:integer;letter_type:byte;LetterFormat:byte;
                                             Myear:byte;SecretariatID:integer):integer;
 var ADOSP:TADOStoredProc;
 begin
  ADOSP:=TADOStoredProc.create(nil);
...
begin
 ...
 //Result:=dm.SELECT_ParaphTemplateTitle.AsString;
  try
      ...
      if OrderID = 1 then
         Result := dm.SELECT_ParaphTemplateTitle.AsString
      else if OrderID <= dm.SELECT_ParaphTemplate.RecordCount then
...
FileName:=path + '\*.*';
  ...
  if FindFirst(fileName, FileAttrs, sr) = 0 then
      repeat
       try
       if not DeleteFile(pchar(path + '\'+ sr.Name)) then result:=false except end;
      until FindNext(sr) <> 0;
