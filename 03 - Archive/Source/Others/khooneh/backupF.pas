...
begin
  case BackupRestore.mode of
   backup : result:=_EYeganeh+'_'+copy(dm.today,3 ,2)+'-'+copy(dm.today,6 ,2)+'-'+copy(dm.today,9 ,2)+'-h'+inttostr(HourOf(now))+'.ybk';
...
begin
 ...
 ProgressBar1.Position:=40;
  ...
  if mode=shrink then
    begin
     progressbar1.Position:=40;
     Exec_ysp_shrink(DatabaseName,path);
     progressbar1.Position:=80;
     Exec_ysp_shrink(DatabaseName,path);
    end
   else
     Exec_ysp_Backup(DatabaseName,path);
