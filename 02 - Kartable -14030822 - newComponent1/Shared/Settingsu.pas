...
type
  TSettings = class(TMBaseForm)
    ...
    procedure TreeView1KeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure rgModeColorClick(Sender: TObject);
...
begin
   ...
   if Trim(GetUserSetting('EmailSendPort')) = '' then //ÅÊ—  «—”«·
      SetUserSetting('EmailSendPort',25); //ÅÌ‘ ›—÷

   if Trim(GetUserSetting('EmailRecivePort')) = '' then //ÅÊ—  œ—Ì«› 
      SetUserSetting('EmailRecivePort',995); //GMail=995 ÅÌ‘ ›—÷

   if Trim(GetUserSetting('EmailEmailConTime')) = '' then
      SetUserSetting('EmailEmailConTime',10000);//ÅÌ‘ ›—÷}
   ...
   LblGraphic.Caption := '  ‰«„ ê—«›Ìﬂ –ŒÌ—Â ‘œÂ :  '+ SkinFileName;
   Try
   ...
   rgModeColor.ItemIndex := GetUserSetting('KAppModeColor');
   if  rgModeColor.ItemIndex = 2 then
   begin
      ColorBox1.Selected := GetUserSetting('KAppMColor1');
      ColorBox2.Selected := GetUserSetting('KAppMColor2');
      ColorBox3.Selected := GetUserSetting('KAppMColor3');
      ColorBox4.Selected := GetUserSetting('KAppMColor4');
   end;
...
//    SetUserSetting('MemoFont','_178'+'_'+ MFont.Font. +'_'+IntToStr(MFont.Font) +'_'+MFont.Font.Name);
   {Ranjbar 89.09.20 ID=247}
   SetUserSetting('FaxExtention',FaxExtention.Text); //Å”Ê‰œ ›ﬂ”
   //---

   SetUserSetting('ImageSize', rgSizeImage.ItemIndex);

   SetUserSetting('KAppModeColor', rgModeColor.ItemIndex);
   _Color5 := clWindow  ;
   ...
   begin      {
      ...
      SetUserSetting('KAppMColor1', $00FFF4F4 );
      SetUserSetting('KAppMColor2', $00FFD9D9);
      SetUserSetting('KAppMColor3', clWhite );
      SetUserSetting('KAppodeColor4', $00FFCACA);
   ...
   begin
    ...
    //  _Color3  :=  $0044ACB5 ;
     ...
     // _Color3  := $0025635C ;
      ...
      SetUserSetting('KAppMColor1', $00B1D0D1);
      SetUserSetting('KAppMColor2', $005FB6BA);
      SetUserSetting('KAppMColor3', $002F7B72);
      SetUserSetting('KAppodeColor4', $00002F2F);
   end
   else
   if rgModeColor.ItemIndex = 2 then
   begin
      SetUserSetting('KAppMColor1', ColorBox1.Selected);
      SetUserSetting('KAppMColor2', ColorBox2.Selected);
      SetUserSetting('KAppMColor3', ColorBox3.Selected);
      SetUserSetting('KAppodeColor4', ColorBox4.Selected);
      {
      _Color1 := ColorBox1.Selected ;
      _Color2 := ColorBox1.Selected ;
      _Color3 := ColorBox1.Selected ;
      _Color4 := ColorBox1.Selected ;
      }
   end;
...
begin
  ...
  if DirectoryExists(ExtractFilePath(Application.ExeName)+ 'Skins\') then
    ...
    // ShowMyMessage('ÅÌ€«„',' ÅÊ‘Â ' + ' Skins ' + ' Ì«›  ‰‘œ ',[mbOK],mtInformation);
  Dm.Signers.Open;
  GetFromDm;
  PageControl_SetTabSheet(PageControl1,'TShBaseform');// ›—„ «’·Ì »—‰«„Â
  {$IFDEF LIGHT}
...
begin
  ...
  begin
     ...
     ShowMessage('ﬂ«—»— ê—«„Ì ·ÿ›« œ—  ‰ŸÌ„«  ›ﬂ” ¬Ì „ "ﬂ«— «»· ›ﬂ” Â«Ì —”ÌœÂ" —« «‰ Œ«» ‰„«ÌÌœ');
     Abort;
  end;
  ...
  begin
     ...
     ShowMessage('ﬂ«—»— ê—«„Ì ° ·ÿ›« œ— »—êÂ "‰«„Â Ê«—œÂ" ¬Ì „ "ﬂœ êÌ—‰œÂ" —«  ⁄ÌÌ‰ ‰„«ÌÌœ');
     Abort;
  end;
  ...
  begin
     ...
     ShowMessage('ﬂ«—»— ê—«„Ì ° ·ÿ›« œ— »—êÂ "‰«„Â ’«œ—Â" ¬Ì „ "›—” ‰œÂ" —«  ⁄ÌÌ‰ ‰„«ÌÌœ');
     Abort;
  end;
...
begin
  ...
  begin
     MBaseForm.MessageShowString('ﬂ«—»— ê—«„Ì : „œ  “„«‰ « ’«· —« ‰„Ì  Ê«‰ ﬂ„ — «“ 5 À«‰ÌÂ ﬁ—«— œ«œ', False);
     EmailEmailConTime.Text := '10';
  end;
end;
...
//       MainForm.SkinData1.LoadFromFile(ItemNamePath);
       LblGraphic.Caption := '  ‰«„ ê—«›Ìﬂ –ŒÌ—Â ‘œÂ :  '+ FileListBox.Items[FileListBox.Itemindex];
      SetFromDm;
      dm.GetUserSettings;
   end;
end;
...
begin
   inherited;
      if SelectDirectory('‘—ﬂ  ‰—„ «›“«—Ì '+_yeganeh,'',S) then
...
procedure TSettings.rgModeColorClick(Sender: TObject);
begin
  inherited;
  pnlColor.Enabled := (rgModeColor.ItemIndex = 2) ;
end;
