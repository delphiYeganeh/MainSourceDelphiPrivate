...
uses Forms,DateUtils,SysUtils,classes,Graphics,windows,dialogs ,
     ...
     IdMessageClient, IdSMTP,winsock,Grids , YDbgrid,IniFiles , MapI
...
Function ShowMyMessage(Titile , Msg: String;
                       ...
                       Const aMsgDlgType: TMsgDlgType = mtConfirmation;FormWidth : Integer = 0): Integer;
...
function ExcelToPDF(Source: string; var Destination : string): boolean;

function SignedDoc(letterID : integer): string;
function CangeDocToZip(Path : string): string;
function SaveDoc(letterID : Integer; Path: String; ty : Integer = 3): string;
...
uses u_c_process_list, Dmu, businessLayer, ZLib, Variants;
   ...
   begin
      try
         ...
         if StrToInt(copy(s,1,4))<1300 then
            ...
            res := LeftStr(ShamsiString(Date), 4);
...
//   ShowMessage(IntToStr(Errorcode));
   ...
   begin
      ...
      if ExceptedProductCode in [254,0] then p1 := DongleProductCode;
      if not (DongleProductCode in [254,p1,p2,p3]) then Errorcode:=4;
   end;
   Result:=Errorcode;
end;
...
begin
  ExcelFileName := Source; //replace file name with the name of your file
  PDF_FileName := Destination;                     //change pdf file name as desired
  ...
  begin
    try
      ...
      begin
        ExcelWorkbook.ExportAsFixedFormat(0, PDF_FileName, 0, True, False, 1, 10, False, EmptyParam);
      end;
    finally
...
var
  ...
  ty : Integer;
begin
  ty := Exec_has_WordExcel(letterID);

  tempDOC := SaveDoc(letterID, ExtractFileDir(Application.ExeName), ty);
  if tempDOC = '' then
  begin
    Result := '';
  ...
  begin
    if ExcelToPDF(tempDOC,tempPDF) then
    begin
      DeleteFile(pchar(tempDOC));
      Result := tempPDF;
    end
...
procedure CloseWord();
var
   OReadOnly,olv,emp,f,f1,fals,tru,SaveChs:OleVariant;
begin
   emp := '';
   fals := false;
   tru := true;
