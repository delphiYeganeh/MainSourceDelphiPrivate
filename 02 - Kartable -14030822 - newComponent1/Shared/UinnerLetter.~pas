...
uses
  ...
  YCheckGroupBox, DBLookupEdit, YDbgrid, ExtActns, XPStyleActnCtrls,
   xpPages, xpBitBtn, Menus, AppEvnts, xpPanel, AdvGlowButton;
...
type
  TFinnerLetter = class(TMBaseForm)
    ...
    SBFollowLetterNo: TAdvGlowButton;
    SBRetroactionNo: TAdvGlowButton;
...
begin
  ...
  if  pnlDetail.Minimized then
    SetUserSetting('KartableInnerLetterpnlDetailMinimize', 1)
  else SetUserSetting('KartableInnerLetterpnlDetailMinimize', 0) ;
...
begin
 ...
 if all then
  ...
  ADOQuery1.Close;
  ADOQuery1.SQL.Clear;
  ADOQuery1.SQL.Add('SELECT code,ResponsibleStaffer FROM FromOrganizations WHERE id = ' + IntToStr(_UserFromOrgID));
  ADOQuery1.Open;
										 
  SenderCode.Text:= ADOQuery1code.AsString;
...
begin
   ...
   begin
      ...
      FieldByName('SignerID').AsInteger     := _UserFromOrgID; //«„÷«Ì ﬂ‰‰œÂ Ì« À»  ﬂ‰‰œÂ ‰«„Â
      If _Kartable then
...
begin
   inherited;
   // ÊÃÂ œ— ﬂ«— «»· ‰„Ì  Ê«‰ ›—” ‰œÂ Ì« «ÌÃ«œ ﬂ‰‰œÂ ‰«„Â —«  €ÌÌ— œ«œ

   OldOrgID := Select_LetterToOrgID.OldValue;
   if not Dm.GetActionAccess(TAction(Sender).Name,Tag) then
      Exit;

   {Ranjbar 89.11.02 ID=278} //œ— Õ«·  ÊÌ—«Ì‘ «„ﬂ«‰  €ÌÌ— ›—” ‰œÂ Ê êÌ—‰œÂ ÊÃÊœ ‰œ«—œ
   ...
   begin
      ...
      if (Select_LetterToOrgID.OldValue <> Select_LetterToOrgID.NewValue)
         And (Select_LetterToOrgID.OldValue <> 0) then //œ— Õ«·  ÃœÌœ „Ì  Ê«‰ êÌ—‰œÂ —«  ⁄ÌÌ‰ ‰ﬂ—œ
      begin
   ...
   begin
      ...
      begin
         ...
         Exec_Insert_ReCommites(1,False,Select_LetterLetterID.AsInteger,select_LetterFromOrgID.AsInteger,
               '«ÌÃ«œ‰«„Â œ«Œ·Ì',0,(select_LetterToOrgID.AsInteger<>0){Proceeded «ﬁœ«„ ‘œÂ},_Today,Time_GetTime(Dm.YeganehConnection,False),3,_userid);

      end;
      {Ranjbar 88.03.16}
      //«—Ã«⁄ ‰«„Â œ«Œ·Ì »Â êÌ—‰œÂ
      //if (Select_LetterToOrgID.AsInteger <> 0)then //_UserFromOrgID ”„  ﬂ«—»—
      if ((OldOrgID=Null)or(OldOrgID=0)) And (Select_LetterToOrgID.AsInteger <> 0) then //_UserFromOrgID ”„  ﬂ«—»—
      begin
         ...
         Exec_Insert_ReCommites(2,Dm.DefaultInnerCopy,select_LetterLetterID.AsInteger,
                Select_LetterToOrgID.AsInteger,'‰«„Â œ«Œ·Ì',1,False,_Today,Time_GetTime(Dm.YeganehConnection,False),3,_UserID,dld);

         if not IsInsert then
            //Ì« «ﬁœ«„ ﬂ—œ‰ «—Ã«⁄ «Ê·Ì Proceeded
...
//   exit;
  ...
  DontShow_xpFormsTab;
   ...
   begin
      {Ranjbar 89.11.18 ID=325} // »œÌ· œﬂ„Â »Â „‰Ê
      //ADoCommite.Hide;
      //AAnswer.Hide;
      //AnewRecommite.Hide;
...
begin
  ...
  SenderCode.Text:= ADOQuery1code.AsString;
  Select_LetterFromStaffer.AsString:=ADOQuery1ResponsibleStaffer.AsString;
...
begin
   ...
   FromOrgForm.InnerMode := True; //IsFrom ;
...
begin
   ...
   begin
      ...
      if LetterID > 0 then
         ...
         ShowMyMessage('ÅÌ€«„','·ÿ›« œ— ›Ì·œ ÅÌ—Ê Ê ⁄ÿ› ‰«„Â œ«Œ·Ì ‘„«—Â «‰œÌﬂ« Ê— Ê«—œ ‰„«ÌÌœ',[mbOK],mtInformation);
   //---
end;
...
begin
  ...
  if select_Letterletterformat.AsInteger=1 then
    if Select_LetterUserID.AsInteger = dm.UserID then
