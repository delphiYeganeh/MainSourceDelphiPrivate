...
begin
   ...
   begin
      if Components[i].ClassNameIs('Tedit') then
         try TEdit(Components[i]).Text:=GetSystemSetting(Components[i].Name) except end
      else
         if Components[i].ClassNameIs('TCheckBox') then
            try TCheckBox(Components[i]).Checked:= GetSystemSetting(Components[i].Name) except end
         else
            if Components[i].ClassNameIs('TDBLookupComboBox') then
               try TDBLookupComboBox(Components[i]).KeyValue:= GetSystemSetting(Components[i].Name)  except end
            else
               if Components[i].ClassNameIs('TComboBox') then
                  try TComboBox(Components[i]).ItemIndex:=   GetSystemSetting(Components[i].Name)   except end
               else
                  {Ranjbar 89.07.29 ID=}
                  if Components[i].ClassNameIs('TSpinEdit') then
                     try TSpinEdit(Components[i]).Value := GetSystemSetting(Components[i].Name) except end;
                  //---
   end;
