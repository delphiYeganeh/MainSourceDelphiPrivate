...
{$IFDEF FR_SERVER_TEMPLATES}, frxServerTemplates{$ENDIF};
...
{$IFDEF FR_SERVER_TEMPLATES}
    property AvExports: String read FAvExports write FAvExports;
...
{$ELSE}
        ...
        FTemplate.Variables.AddVariable('PGCNT', IntToStr(FCurrentPage));
        FTemplate.Variables.AddVariable('REPNAME', HTMLCodeStr(StringReplace(Report.FileName, FReportPath, '', [])));
        if FMultipage then
          s := '1'
        else
          s := '0';
        ...
        print := '';
        FTemplate.Variables.AddVariable('PRINT_LINK', FPrintLink);
        ...
        FTemplate.Variables.AddVariable('L_PRINT', UTF8Encode(frxResources.Get('HTMLNavPrint')));
        ...
        FTemplate.Prepare;
        FTemplate.TemplateStrings.SaveToStream(Exp);
{$ENDIF}
      finally
        Exp.Free;
      end;
...
{$ELSE}
        ...
        FTemplate.Variables.AddVariable('TITLE', UTF8Encode(s));
        if FMultipage then
          s := '1.html'
        else
          s := 'main.html';
        FTemplate.Variables.AddVariable('MAIN_PAGE', ReverseSlash(GetFrameFolder) + s);
        FTemplate.Variables.AddVariable('NAV_PAGE', ReverseSlash(GetFrameFolder) + 'nav.html');
        FTemplate.Prepare;
        FTemplate.TemplateStrings.SaveToStream(Exp);
{$ENDIF}
      finally
