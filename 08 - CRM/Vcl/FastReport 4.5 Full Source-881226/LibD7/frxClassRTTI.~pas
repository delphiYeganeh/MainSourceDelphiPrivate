     ...
     Negativ:Boolean;
        ...
        begin
                ...
                begin
                        ...
                        else if ragham[2]='0' then hor:=dahgan[strtoint(ragham[1])] else hor:=dahgan[strtoint(ragham[1])]+' Ê '+yekan[strtoint(ragham[2])];
                end;
                if tool=3 then
                begin
                    ...
                    begin
                        ...
                        if (ragham[2]<>'1') and (ragham[2]<>'0') then if ragham[3]='0' then hor:=dahgan[strtoint(ragham[2])] else hor:=dahgan[strtoint(ragham[2])]+' Ê '+yekan[strtoint(ragham[3])];
                    end;
                    hor:=sadgan[strtoint(ragham[1])]+' Ê '+hor;
                    if copy(hor,length(hor)-2,2)=' Ê' then delete(hor,length(hor)-2,3);
                    if copy(hor,1,2)=' Ê' then delete(hor,1,3);
                end;
                cth:=hor;
        end;
begin
    myNumber := IntToStr(ROUND(StrToFloat(myNumber)));
     ...
     begin
              ...
              TmpStr:=cth(copy(myNumber,1,length(myNumber)-3))+ ' Â“«— Ê ' +TmpStr;
     end;
     ...
     begin
              ...
              then TmpStr:=cth(copy(myNumber,length(myNumber)-5,3))+ ' Â“«— Ê ' +TmpStr;;
              TmpStr:=cth(copy(myNumber,1,length(myNumber)-6))+ ' „Ì·ÌÊ‰ Ê ' +TmpStr;
     end;
     ...
     begin
              ...
              IF copy(myNumber,length(myNumber)-8,3)<>'000'
                  then TmpStr:=cth(copy(myNumber,length(myNumber)-8,3))+ ' „Ì·ÌÊ‰ Ê ' +TmpStr;
                       TmpStr:=cth(copy(myNumber,1,length(myNumber)-9))+ ' „Ì·Ì«—œ Ê ' +TmpStr;
     end;
     if copy(TmpStr,length(TmpStr)-2,2)=' Ê' then delete(TmpStr,length(TmpStr)-2,3);
     if Negativ then TmpStr:='„‰›Ì '+TmpStr;
     Result:=TmpStr;
end;
