    ...
    begin
      ...
      if PrevTag <> nil then
        Tag.Default := (FStyle = PrevTag.Style) and
                       ...
                       (FSize = PrevTag.Size) and (FAddY = PrevTag.AddY)
      else
...
var
  ...
  begin
    ...
    begin
      ansis := _UnicodeToAnsi(s, Canvas.Font.Charset);
      ...
      if (C.Font.Charset = DEFAULT_CHARSET) and (Win32Platform = VER_PLATFORM_WIN32_NT) then
        ...
        if FWysiwyg then
          ExtTextOut(C.Handle, X + Round(add1 * SinA), Y + Round(add1 * CosA),
            FOptions, @FScaledRect, PChar(_UnicodeToAnsi(s, C.Font.Charset)) + j, i - j, @FTempArray[j])
        else
          ExtTextOut(C.Handle, X + Round(add1 * SinA), Y + Round(add1 * CosA),
            FOptions, @FScaledRect, PChar(_UnicodeToAnsi(s, C.Font.Charset)) + j, i - j, nil);
      if i < n then
      begin
        if IsPrinter(C) then
...
begin
  ...
  if FLineSpacing < 0 then
    Result := Result + Abs(FLineSpacing);
end;
