...
{$ENDIF}
    ...
    FOnAfterPrepareScript: TNotifyEvent;
  ...
  public
    ...
    property OnAfterPrepareScript: TNotifyEvent read FOnAfterPrepareScript write FOnAfterPrepareScript;
...
{$IFDEF FR_COM}
  public
    ...
    function ExportToPDF(const FileName: WideString; Compressed: WordBool; EmbeddedFonts: WordBool; 
                         PrintOptimized: WordBool; Background: WordBool): HResult; overload; stdcall;
    ...
    function ExportToXLS(const szFileName: WideString; Pictures, PageBreaks, WYSIWYG, AsText, Background: WordBool): HResult; overload; stdcall;
    function ExportToXML(const FileName: WideString; Styles, PageBreaks, WYSIWYG, Background: WordBool): HResult; overload; stdcall;
...
begin
  ...
  if VarIsNull(Value) then
    ...
    if Font.Charset <> DEFAULT_CHARSET then
      Result := AnsiToUnicode(VarToStr(Value), Font.Charset)
...
begin
  ...
  try
    ...
    begin
      ...
      if Assigned(c.OnAfterPrepareScript) then
        AfterPrepareList.Add(@TMethod(c.OnAfterPrepareScript));
  ...
  {$ELSE}
    Result := True;
  {$ENDIF}
    for i := 0 to AfterPrepareList.Count - 1 do
      TNotifyEvent(AfterPrepareList[i]^)(Self);
  finally
    FScript.AddedBy := nil;
    AfterPrepareList.Free;
  end;
end;
...
function TfrxReport.ExportToPDF(const FileName: WideString; Compressed: WordBool; EmbeddedFonts: WordBool; 
                         PrintOptimized: WordBool; Background: WordBool): HResult;
{$IFNDEF FR_LITE}
var
  Export2PDF:         TfrxPDFExport;
begin
...
begin
  ...
  try
    ...
    Export2Mail.Mail(Server, Port, User, Password, From, To_, Subject, Text, '', List);
  finally
    List.Free
  end;
  Result := S_OK;
...
//  if Assigned(frxDefaultConnection) then frxDefaultConnection.Free;
