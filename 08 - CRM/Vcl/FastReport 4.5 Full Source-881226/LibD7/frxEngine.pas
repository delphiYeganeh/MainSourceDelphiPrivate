...
begin
  ...
  begin
   { use own CurX - we may be inside subreports now }
    ...
    if Band <> nil then
      if not FKeeping or not FHeaderList[i].IsInKeepList or FKeepFooter then
        begin
          if ((Band is TfrxHeader) and FDontShowHeaders) or ((Band is TfrxHeader) and (FCurBand = Band))  then
            continue;
          Band.Overflow := True;
          SaveCurLine := CurLine;
  ...
  try
    ...
    begin
      ...
      if not IsMultiColumnBand and not (Band is TfrxOverlay) and not (Band is TfrxNullBand) and
        ...
        if FOutputTo = nil then
          if (Band.AllowSplit)and(not FKeeping) then
          begin
            DrawSplit(Band);
            IsSplit := True;
          end
