...
begin
  ...
  Result := string(PWideChar(AGUIDString)); MD5String(string(PWideChar(AGUIDString)));
end;
...
function StrToUTF16U(const Value: WideString): String;
var
  i: integer;
  pwc: ^Word;
begin
...
function HexEncode7F(Str: WideString): string;
var
  s: string;
  Index, Len: Integer;
begin
...
function Dec2Oct(const i: Longint): string;
var
  m, j: Longint;
Begin
  Result := '';
...
function CryptStr(Source: String; Key: String; Enc: Boolean; id: Integer): String;
var
{$IFDEF PDF_RC4}
...
function CryptStream(Source: TStream; Target: TStream; Key: String; id: Integer): String;
var
  s: String;
{$IFDEF PDF_RC4}
  k: array [ 1..21 ] of Byte;
...
function PrepareString(const Text: WideString; Key: String; Enc: Boolean; id: Integer): String;
begin
  if Enc then
  begin
    Result := '(' + CryptStr(Text, Key, Enc, id) + ')'
  end
  else
    Result := '<' + StrToUTF16(Text) + '>'
end;

function UnicodeToANSI(const Str: WideString; Codepage: Integer): String;
var
  i: Integer;
begin
  Result := '';
...
function PMD52Str(p: Pointer): String;
begin
  SetLength(Result, 16);
  Move(p^, Result[1], 16);
end;
...
// ENCRYPTION KEY
  ...
  try
    ...
    fid := '';
    for i := 1 to 16 do
      fid := fid + chr(StrToInt('$' + FFileID[i * 2 - 1] + FFileID[i * 2]));
    md5.Update(@fid[1], 16);
    md5.Finalize;
    s := PMD52Str(md5.Digest);
    for i := 1 to 50 do
...
function TfrxPDFFile.GetOwnerPassword: String;
begin
  Result := FOPass;
end;

function TfrxPDFFile.GetUserPassword: String;
begin
  Result := FUPass;
end;
...
var
  ...
  begin
    l := Length(Str);
    if (Str[l - 1] = #13) and (Str[l] = #10) then
  ...
  begin
    ...
    begin
      ...
      Write(OutStream, s + ' rg'#13#10 + s + ' RG'#13#10 +
        frFloat2Str(GetLeft(Obj.AbsLeft + ow)) + ' ' + frFloat2Str(GetTop(Obj.AbsTop + oh + Obj.Frame.ShadowWidth)) + ' ' +
