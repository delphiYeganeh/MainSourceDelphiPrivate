...
procedure TfrxChartEditorForm.WMSizing(var Message: TMessage);
begin
  PRect(Message.LParam)^.Bottom := PRect(Message.LParam)^.Top + FDefHeight;
  if PRect(Message.LParam)^.Right < PRect(Message.LParam)^.Left + FDefWidth then
    PRect(Message.LParam)^.Right := PRect(Message.LParam)^.Left + FDefWidth;
  inherited;
end;
