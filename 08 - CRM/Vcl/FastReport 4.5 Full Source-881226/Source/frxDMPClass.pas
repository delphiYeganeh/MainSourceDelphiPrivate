...
var
  ...
  Ansis: String;
begin
  if (Win32Platform <> VER_PLATFORM_WIN32_NT) or (FCanvas.Font.Charset <> DEFAULT_CHARSET) then
  begin
    Ansis := Text;
    GetTextExtentExPoint(FCanvas.Handle, PAnsiChar(Ansis), Length(Ansis) , Round(Width), @Cnt, nil, Sz);
  end
  else
    GetTextExtentExPointW(FCanvas.Handle, PwideChar(Text), Length(Text) , Round(Width), @Cnt, nil, Sz);
  Result := Copy(Text, 1, Cnt + 1);
end;
