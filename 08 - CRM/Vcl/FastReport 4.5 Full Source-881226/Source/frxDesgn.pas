...
{$IFDEF FR_COM}
    ...
    function Get_OpenDir(out Value: WideString): HResult; stdcall;
    function Set_OpenDir(const Value: WideString): HResult; stdcall;
    function Get_SaveDir(out Value: WideString): HResult; stdcall;
    function Set_SaveDir(const Value: WideString): HResult; stdcall;
...
{$IFDEF FR_COM}
    FStoredExpired: Boolean; // avoid easy cracks ;)
{$ENDIF}
...
function TfrxDesigner.Get_OpenDir(out Value: WideString): HResult; stdcall;
begin
  Value := OpenDir;
  Result := S_OK;
end;

function TfrxDesigner.Set_OpenDir(const Value: WideString): HResult; stdcall;
begin
  OpenDir := Value;
  Result := S_OK;
end;

function TfrxDesigner.Get_SaveDir(out Value: WideString): HResult; stdcall;
begin
  Value := SaveDir;
  Result := S_OK;
end;

function TfrxDesigner.Set_SaveDir(const Value: WideString): HResult; stdcall;
begin
  SaveDir := Value;
  Result := S_OK;
end;
...
{$ENDIF}
  ...
  NewReportCmd.Enabled := CheckOp(drDontCreateReport){$IFDEF FR_COM} and not IsExpired{$ENDIF};
  NewItemCmd.Enabled := CheckOp(drDontCreateReport){$IFDEF FR_COM} and not IsExpired{$ENDIF};
  NewPageCmd.Enabled := CheckOp(drDontCreatePage){$IFDEF FR_COM} and not IsExpired{$ENDIF};
  NewDialogCmd.Enabled := NewDialogCmd.Enabled and CheckOp(drDontCreatePage){$IFDEF FR_COM} and not IsExpired{$ENDIF};
...
begin
  if (not CheckOp(drDontInsertObject) or (FWorkspace.Insertion.Top < 0)){$IFDEF FR_COM} or IsExpired{$ENDIF} then
  begin
...
{$IFDEF FR_COM}
  if IsExpired <> FStoredExpired then
  begin    {simple crack checking}
    IsExpired := True;
    FStoredExpired := True;
  end;
{$ENDIF}
...
begin
  if not CheckOp(drDontCreatePage){$IFDEF FR_COM} or IsExpired{$ENDIF} then Exit;
...
var
  ...
  w: Word;
begin
  if not CheckOp(drDontCreateReport){$IFDEF FR_COM} or IsExpired{$ENDIF} then Exit;
...
begin
  if CheckOp(drDontCreateReport){$IFDEF FR_COM} and not IsExpired{$ENDIF} then
...
{$ENDIF}
  ...
  if Assigned(frxFR2Events.OnLoad) then
    OpenDialog.Filter := 'Report (*.fr3, ' + frxFR2Events.Filter + ')' + '|*.fr3;' + frxFR2Events.Filter;
end;
