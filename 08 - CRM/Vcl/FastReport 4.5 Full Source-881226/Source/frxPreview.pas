...
{$ENDIF}
  ...
  private
    FOnDesignButtonClick: TNotifyEvent;
  ...
  published
    property OnDesignButtonClick: TNotifyEvent read FOnDesignButtonClick write FOnDesignButtonClick;    
  end;
...
begin
  ...
  if Page <> nil then
    Result := Round((VertPosition - FPageList.GetPageBounds(Preview.PageNo - 1,ClientWidth, Zoom).Top + 10)/ Zoom - Page.TopMargin * fr01cm);
end;
...
procedure TfrxPreviewForm.Button1Click(Sender: TObject);
begin
  if Assigned(FOnDesignButtonClick) then
    FOnDesignButtonClick(Self);
end;
