...
//   procedure FYDBGridTitleClick(Column: TColumn);
  ...
  procedure ExportToWord(fileName:string='');
...
var
   ...
   Data ,Title: String;
   //---
   Function MakeCopy(const S: WideString; Index, Count: Integer):String;
   begin
   ...
   with DataSource,DataSet do
    ...
    with WordApplication.Selection do
     begin
        ...
        if pos('^',Title)<>0 then
        begin
            TypeText(MakeCopy(Title,1,pos('^',Title)-1) );
            TypeParagraph;
        end;

        TypeText(MakeCopy(Title,pos('^',Title)+1,length(Title)- pos('^',Title) ));
...
begin
  if Column.Field.FieldKind=fkLookup then
   ...
   except  ShowMessage('»— «”«” «Ì‰ ›Ì·œ ‰„Ì  Ê«‰ «ÿ·«⁄«  —« „— » ò—œ'); end;
end;
