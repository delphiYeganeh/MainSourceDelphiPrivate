...
uses
  ...
  Forms, Graphics, Buttons, Dialogs, Menus, xcalAdvEdit
  {$IFNDEF TMSDOTNET}
  , xcalAEBXPVS
  {$ENDIF}
  {$IFDEF TMSDOTNET}
...
{ TXCalAdvMaskEditBtn }
  ...
  TXCalAdvMaskEditBtn = class(TXCalAdvMaskEdit)
  private
    FUnitSize : integer;
    FButton: TEditButton;
  ...
  TUniTAdvMaskEditBtn = class(TXCalAdvMaskEditBtn)
  private
    FUnitID: string;
    FUnits: TStringList;
...
begin
  case TXCalAdvMaskEditBtn(Owner.Owner).ButtonStyle of
...
begin
  if Assigned(Owner) then
    ...
    begin
      if (Owner.Owner is TXCalAdvMaskEdit) then
        Canvas.Font.Assign(TXCalAdvMaskEdit(Owner.Owner).Font);
    end;
...
constructor TXCalAdvMaskEditBtn.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
destructor TXCalAdvMaskEditBtn.Destroy;
begin
  FButton := nil;
  inherited Destroy;
end;

procedure TXCalAdvMaskEditBtn.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(Params);
end;

procedure TXCalAdvMaskEditBtn.CreateWnd;
begin
  inherited CreateWnd;
  Width := Width - 1;
  Width := Width + 1;
...
procedure TXCalAdvMaskEditBtn.Loaded;
begin
  inherited Loaded;
  SetEditRect;
  ResizeControl;
...
procedure TXCalAdvMaskEditBtn.SetGlyph(value:TBitmap);
begin
  FButton.Glyph := Value;
end;

function TXCalAdvMaskEditBtn.GetGlyph:TBitmap;
begin
  Result := FButton.Glyph;
end;

procedure TXCalAdvMaskEditBtn.SetCaption(value:string);
begin
  FButton.ButtonCaption := value;
end;

function TXCalAdvMaskEditBtn.GetCaption:string;
begin
  Result := FButton.ButtonCaption;
end;

procedure TXCalAdvMaskEditBtn.SetEditRect;
var
  Loc: TRect;
begin
  {$IFNDEF TMSDOTNET}
  ...
  begin
    Loc.Right := ClientWidth - 4 - FUnitsize;
    if (BorderStyle = bsNone) then
    begin
      Loc.Top := 4;
      Loc.Left := 2 + FButton.Width;
    end
    else
    begin
      Loc.Top := 1;
...
procedure TXCalAdvMaskEditBtn.ResizeControl;
var
  MinHeight: Integer;
   ...
   begin
    ...
    begin
      if NewStyleControls and Ctl3D then
        FButton.SetBounds(0, 1 + FlatCorr, FButton.FBWidth, Height - Dist)
      else
        FButton.SetBounds (0, 1, FButton.FBWidth, Height - 2);
    end
    else
    begin
      if NewStyleControls and Ctl3D then
...
procedure TXCalAdvMaskEditBtn.WMSize(var Message: TWMSize);
begin
  inherited;
  ResizeControl;
end;

procedure TXCalAdvMaskEditBtn.WMKeyDown(var Msg:TWMKeydown);
begin
  inherited;
...
function TXCalAdvMaskEditBtn.GetMinHeight: Integer;
var
  DC: HDC;
  SaveFont: HFont;
  I: Integer;
...
procedure TXCalAdvMaskEditBtn.BtnClick (Sender: TObject);
begin
  if Assigned(FOnClickBtn) then
    FOnClickBtn(Self);
end;

procedure TXCalAdvMaskEditBtn.WMPaste(var Message: TWMPaste);
begin
  if not FEditorEnabled or ReadOnly then Exit;
  inherited;
end;

procedure TXCalAdvMaskEditBtn.WMCut(var Message: TWMPaste);
begin
  if not FEditorEnabled or ReadOnly then Exit;
  inherited;
end;

procedure TXCalAdvMaskEditBtn.CMExit(var Message: TCMExit);
begin
  inherited;
  DrawBorders;
end;

procedure TXCalAdvMaskEditBtn.CMEnter(var Message: TCMGotFocus);
begin
...
procedure TXCalAdvMaskEditBtn.CMMouseEnter(var Message: TMessage);
begin
  inherited;
...
procedure TXCalAdvMaskEditBtn.CMMouseLeave(var Message: TMessage);
begin
  inherited;
...
procedure TXCalAdvMaskEditBtn.SetFlat(const Value: boolean);
begin
  if (FFlat <> value) then
  begin
    FFlat := Value;
...
procedure TXCalAdvMaskEditBtn.SetEtched(const Value: boolean);
begin
...
function TXCalAdvMaskEditBtn.Is3DBorderControl: Boolean;
begin
...
function TXCalAdvMaskEditBtn.Is3DBorderButton: Boolean;
begin
  if csDesigning in ComponentState then
    Result := Enabled
  else
...
procedure TXCalAdvMaskEditBtn.DoEnter;
begin
  inherited;
  SetEditRect;
end;
...
procedure TXCalAdvMaskEditBtn.DrawControlBorder(DC: HDC);
var
  ARect:TRect;
...
procedure TXCalAdvMaskEditBtn.DrawButtonBorder;
begin
  FButton.FButton.Focused := Is3DBorderButton;
end;

procedure TXCalAdvMaskEditBtn.DrawBorders;
var
  DC: HDC;
begin
...
procedure TXCalAdvMaskEditBtn.WMPaint(var Msg: TWMPAINT);
begin
  inherited;
//  DrawBorders;
end;
...
{$IFNDEF TMSDOTNET}
procedure TXCalAdvMaskEditBtn.WMNCPaint(var Message: TMessage);
begin
  inherited;
//  DrawBorders;
end;
...
{$IFDEF TMSDOTNET}
procedure TXCalAdvMaskEditBtn.WndProc(var Message: TMessage);
begin
  inherited;
end;
{$ENDIF}

procedure TXCalAdvMaskEditBtn.CMFontChanged(var Message: TMessage);
begin
  inherited;
  SetEditRect;
end;

procedure TXCalAdvMaskEditBtn.KeyDown(var Key: Word; Shift: TShiftState);
begin
  inherited;
  if (Key = vk_F4) and
...
function TXCalAdvMaskEditBtn.GetButtonWidth: integer;
begin
...
procedure TXCalAdvMaskEditBtn.SetButtonWidth(const Value: integer);
begin
  if Assigned(FButton) then
  begin
...
procedure TXCalAdvMaskEditBtn.SetButtonHint(const Value: string);
begin
...
function TXCalAdvMaskEditBtn.GetVersionNr: Integer;
begin
...
procedure TXCalAdvMaskEditBtn.SetButtonStyle(const Value: TButtonStyle);
begin
  FButtonStyle := Value;
  Invalidate;
end;
...
//mmm
procedure TXCalAdvMaskEditBtn.CMBiDiModeChanged(var Message: TMessage);
begin
  inherited;
  ResizeControl;
end;
