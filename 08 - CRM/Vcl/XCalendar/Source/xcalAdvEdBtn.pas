...
{ TXCalAdvEditBtn }
  ...
  TXCalAdvEditBtn = class(TXCalAdvEdit)
  private
    FUnitSize : integer;
    FButton: TEditButton;
  ...
  TUnitAdvEditBtn = class(TXCalAdvEditBtn)
  private
    FUnitID: string;
    FUnits: TStringList;
...
begin
  case TXCalAdvEditBtn(Owner.Owner).ButtonStyle of
...
begin
  ...
  begin
    Enabled := TXCalAdvEditBtn(Owner.Owner).Enabled;
  ...
  begin
    ...
    if not TXCalAdvEditBtn(Owner.Owner).Enabled then
    begin
      {$IFNDEF TMSDOTNET}
...
var
  ...
  HTheme: THandle;
begin
  Canvas.Font.Assign(TXCalAdvEdit(Owner.Owner).Font);
...
constructor TXCalAdvEditBtn.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
destructor TXCalAdvEditBtn.Destroy;
begin
  FButton := nil;
  inherited Destroy;
end;

procedure TXCalAdvEditBtn.CreateParams(var Params: TCreateParams);
begin
...
procedure TXCalAdvEditBtn.CreateWnd;
begin
  inherited CreateWnd;
  Width := Width - 1;
  Width := Width + 1;
...
procedure TXCalAdvEditBtn.Loaded;
begin
  inherited Loaded;
  SetEditRect;
  ResizeControl;
...
procedure TXCalAdvEditBtn.SetGlyph(value:TBitmap);
begin
  FButton.Glyph := Value;
end;

function TXCalAdvEditBtn.GetGlyph:TBitmap;
begin
  Result := FButton.Glyph;
end;

procedure TXCalAdvEditBtn.SetCaption(value:string);
begin
  FButton.ButtonCaption := value;
end;

function TXCalAdvEditBtn.GetCaption:string;
begin
  Result := FButton.ButtonCaption;
end;

procedure TXCalAdvEditBtn.SetEditRect;
var
  Loc: TRect;
begin
  {$IFNDEF TMSDOTNET}
...
procedure TXCalAdvEditBtn.ResizeControl;
var
  MinHeight: Integer;
   ...
   begin
    ...
    begin
      if NewStyleControls and Ctl3D then
        ...
        FButton.SetBounds (-2, 1, FButton.FBWidth, Height - 2);
    end
    else
    begin
      if NewStyleControls and Ctl3D then
...
procedure TXCalAdvEditBtn.WMSize(var Message: TWMSize);
begin
  inherited;
  ResizeControl;
end;

procedure TXCalAdvEditBtn.WMKeyDown(var Msg:TWMKeydown);
begin
...
function TXCalAdvEditBtn.GetMinHeight: Integer;
var
  DC: HDC;
  SaveFont: HFont;
  I: Integer;
...
procedure TXCalAdvEditBtn.BtnClick (Sender: TObject);
begin
  if Assigned(FOnClickBtn) then
    FOnClickBtn(Self);
end;

procedure TXCalAdvEditBtn.WMPaste(var Message: TWMPaste);
begin
  if not FEditorEnabled or ReadOnly then Exit;
  inherited;
end;

procedure TXCalAdvEditBtn.WMCut(var Message: TWMPaste);
begin
  if not FEditorEnabled or ReadOnly then Exit;
  inherited;
end;

procedure TXCalAdvEditBtn.CMExit(var Message: TCMExit);
begin
  inherited;
  DrawBorders;
end;

procedure TXCalAdvEditBtn.CMEnabledChanged(var Msg: TMessage);
begin
  inherited;
  FButton.Enabled := self.Enabled;
end;

procedure TXCalAdvEditBtn.CMEnter(var Message: TCMGotFocus);
begin
...
procedure TXCalAdvEditBtn.CMMouseEnter(var Message: TMessage);
begin
  inherited;
...
procedure TXCalAdvEditBtn.CMMouseLeave(var Message: TMessage);
begin
  inherited;
...
procedure TXCalAdvEditBtn.SetFlat(const Value: boolean);
begin
  if (FFlat <> value) then
  begin
    FFlat := Value;
...
procedure TXCalAdvEditBtn.SetEtched(const Value: boolean);
begin
...
function TXCalAdvEditBtn.Is3DBorderControl: Boolean;
begin
...
function TXCalAdvEditBtn.Is3DBorderButton: Boolean;
begin
  if csDesigning in ComponentState then
    Result := Enabled
  else
...
procedure TXCalAdvEditBtn.DoEnter;
begin
  inherited;
  SetEditRect;
end;
...
procedure TXCalAdvEditBtn.DrawControlBorder(DC: HDC);
var
  ARect:TRect;
...
procedure TXCalAdvEditBtn.DrawButtonBorder;
begin
  FButton.FButton.Focused := Is3DBorderButton;
end;

procedure TXCalAdvEditBtn.DrawBorders;
var
  DC: HDC;
begin
...
procedure TXCalAdvEditBtn.WMPaint(var Msg: TWMPAINT);
begin
  inherited;
//  DrawBorders;
end;
...
{$IFNDEF TMSDOTNET}
procedure TXCalAdvEditBtn.WMNCPaint(var Message: TMessage);
begin
  inherited;
//  DrawBorders;
end;
...
{$IFDEF TMSDOTNET}
procedure TXCalAdvEditBtn.WndProc(var Message: TMessage);
begin
  inherited;
end;
{$ENDIF}

procedure TXCalAdvEditBtn.CMFontChanged(var Message: TMessage);
begin
  inherited;
  SetEditRect;
end;

procedure TXCalAdvEditBtn.KeyDown(var Key: Word; Shift: TShiftState);
begin
  inherited;
  if (Key = vk_F4) and
...
function TXCalAdvEditBtn.GetButtonWidth: integer;
begin
...
procedure TXCalAdvEditBtn.SetButtonWidth(const Value: integer);
begin
  if Assigned(FButton) then
  begin
...
procedure TXCalAdvEditBtn.SetButtonHint(const Value: string);
begin
...
function TXCalAdvEditBtn.GetVersionNr: Integer;
begin
...
procedure TXCalAdvEditBtn.SetButtonStyle(const Value: TButtonStyle);
begin
  FButtonStyle := Value;
  Invalidate;
end;

procedure TXCalAdvEditBtn.WMChar(var Msg: TWMKey);
begin
  if not EditorEnabled then
    Exit;
  inherited;
...
//mmm
procedure TXCalAdvEditBtn.CMBiDiModeChanged(var Message: TMessage);
begin
  inherited;
  ResizeControl;
end;
