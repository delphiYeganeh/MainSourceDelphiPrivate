inherited FActionTypeLevel: TFActionTypeLevel
  Left = 635
  Top = 219
  Width = 687
  Height = 659
  Caption = #1583#1587#1578#1607' '#1576#1606#1583#1610
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 13
  inherited ShapeBase: TShape
    Width = 671
    Height = 620
  end
  object pnlMain: TPanel [1]
    Left = 0
    Top = 0
    Width = 671
    Height = 620
    Align = alClient
    TabOrder = 0
    object Panel1: TPanel
      Left = 1
      Top = 570
      Width = 669
      Height = 49
      Align = alBottom
      TabOrder = 0
      DesignSize = (
        669
        49)
      object BBOK: TBitBtn
        Left = 96
        Top = 8
        Width = 75
        Height = 28
        Cursor = crHandPoint
        Caption = #1578#1575#1610#1610#1583
        Default = True
        Font.Charset = ARABIC_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ModalResult = 1
        ParentFont = False
        TabOrder = 0
        OnClick = BBOKClick
        Glyph.Data = {
          
          8000808000FF00008080}
      end
      object BBCancel: TBitBtn
        Left = 16
        Top = 8
        Width = 75
        Height = 28
        Cursor = crHandPoint
        Cancel = True
        Caption = #1575#1606#1589#1585#1575#1601
        Font.Charset = ARABIC_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        OnClick = BBCancelClick
        Glyph.Data = {
          
          8000808000808000808000808000}
      end
      object BBInsert: TBitBtn
        Left = 577
        Top = 8
        Width = 75
        Height = 28
        Cursor = crHandPoint
        Anchors = [akTop, akRight]
        Caption = #1580#1583#1610#1583
        Font.Charset = ARABIC_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        OnClick = BBInsertClick
        Glyph.Data = {
          
          9377C67FE1F1E3FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF00}
      end
      object deleteBtn: TBitBtn
        Left = 497
        Top = 8
        Width = 75
        Height = 28
        Cursor = crHandPoint
        Anchors = [akTop, akRight]
        Caption = #1581#1584#1601
        Font.Charset = ARABIC_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 3
        OnClick = deleteBtnClick
        Glyph.Data = {
          
          F55F5EC0B3B2DEFEFDFCFFFFFF00}
      end
    end
    object GroupBox1: TGroupBox
      Left = 1
      Top = 464
      Width = 669
      Height = 106
      Align = alBottom
      Caption = #1575#1591#1604#1575#1593#1575#1578' '#1587#1575#1586#1605#1575#1606
      Color = clBtnFace
      ParentColor = False
      TabOrder = 1
      DesignSize = (
        669
        106)
      object Label5: TLabel
        Left = 625
        Top = 23
        Width = 38
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1585#1610#1588#1607'   :'
      end
      object Label2: TLabel
        Left = 597
        Top = 76
        Width = 66
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1606#1608#1593' '#1575#1602#1583#1575#1605#1575#1578'   :'
      end
      object Label3: TLabel
        Left = 624
        Top = 50
        Width = 39
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1593#1606#1608#1575#1606'   :'
      end
      object Label6: TLabel
        Left = 203
        Top = 50
        Width = 65
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1578#1585#1578#1610#1576'            :'
      end
      object edtParent: TLabel
        Left = 561
        Top = 24
        Width = 48
        Height = 13
        Anchors = [akTop, akRight]
        Caption = 'edtParent'
      end
      object edtDescription: TEdit
        Left = 280
        Top = 47
        Width = 341
        Height = 21
        Anchors = [akTop, akRight]
        MaxLength = 100
        TabOrder = 0
      end
      object edtOrder: TEdit
        Left = 26
        Top = 47
        Width = 173
        Height = 21
        Anchors = [akTop, akRight]
        MaxLength = 2
        TabOrder = 1
      end
      object DBActionType: TDBLookupComboBox
        Left = 304
        Top = 72
        Width = 285
        Height = 21
        Anchors = [akTop, akRight]
        BiDiMode = bdLeftToRight
        DropDownRows = 5
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        KeyField = 'ActionTypeID'
        ListField = 'ActionTypeTitle'
        ListSource = Dm.DActionType
        ParentBiDiMode = False
        ParentFont = False
        TabOrder = 2
      end
      object Button1: TButton
        Left = 279
        Top = 72
        Width = 22
        Height = 20
        Anchors = [akTop, akRight]
        Caption = '*'
        TabOrder = 3
        OnClick = Button1Click
      end
    end
    object PageControl: TPageControl
      Left = 1
      Top = 1
      Width = 669
      Height = 463
      ActivePage = TabSheet1
      Align = alClient
      BiDiMode = bdRightToLeft
      MultiLine = True
      ParentBiDiMode = False
      TabOrder = 2
      TabPosition = tpLeft
      object TabSheet1: TTabSheet
        Caption = #1583#1585#1582#1578#1610
        object ActionTypeLevelTree: TTreeView
          Left = 0
          Top = 0
          Width = 641
          Height = 455
          Align = alClient
          Indent = 19
          ReadOnly = True
          TabOrder = 0
          OnChange = ActionTypeLevelTreeChange
          OnChanging = ActionTypeLevelTreeChanging
          OnDblClick = ActionTypeLevelTreeDblClick
          OnGetImageIndex = ActionTypeLevelTreeGetImageIndex
        end
      end
    end
  end
  inherited DSForm: TDataSource
    DataSet = PActionType
    Left = 94
    Top = 290
  end
  inherited ActionList: TActionList
    Left = 26
    Top = 181
  end
  object PActionType: TADODataSet
    Connection = Dm.YeganehConnection
    CursorType = ctStatic
    Parameters = <>
    Left = 92
    Top = 234
    object PActionTypeID: TAutoIncField
      FieldName = 'ID'
      ReadOnly = True
    end
    object PActionTypeTitle: TWideStringField
      FieldName = 'Description'
      Size = 100
    end
  end
  object ActionType: TADODataSet
    Connection = Dm.YeganehConnection
    CursorType = ctStatic
    AfterScroll = ActionTypeAfterScroll
    Parameters = <>
    Left = 22
    Top = 233
    object ActionTypeTitle: TWideStringField
      FieldName = 'Description'
      Size = 100
    end
    object ActionTypeparentid: TIntegerField
      FieldName = 'ParentID'
    end
    object ActionTypeID: TAutoIncField
      FieldName = 'ID'
      ReadOnly = True
    end
    object ActionTypeChildID: TIntegerField
      FieldName = 'ChildID'
    end
    object ActionTypeOrderId: TIntegerField
      FieldName = 'OrderId'
    end
    object ActionTypeLevel: TIntegerField
      FieldName = 'Level'
    end
    object ActionTypeType: TIntegerField
      FieldName = 'Type'
    end
  end
  object DActionType: TDataSource
    DataSet = ActionType
    Left = 27
    Top = 290
  end
  object TempAllData: TADOQuery
    Connection = Dm.YeganehConnection
    Parameters = <>
    SQL.Strings = (
      'SELECT c.[ID]'
      '      ,c.[ParentID]'
      '      ,c.[ChildID]'
      '      ,c.[OrderId]'
      
        '      ,CASE WHEN c.ChildID>0 THEN a.[ActionTypeTitle] ELSE  c.[D' +
        'escription] END [Description]'
      '      ,c.[Level]'
      '      ,c.[Type]'
      '      ,isnull(p.[Description],'#39#1588#1585#1705#1578#39') parentDescription'
      '      ,p.ParentID pparentid'
      '  FROM [dbo].[ActionType_Level] c'
      '  LEFT JOIN [dbo].[ActionType_Level] p ON c.ParentID = p.ID'
      '  LEFT JOIN [dbo].[ActionType] a ON a.ActionTypeID = c.ChildID')
    Left = 565
    Top = 13
    object TempAllDataId: TIntegerField
      FieldName = 'Id'
    end
    object TempAllDataParentID: TIntegerField
      FieldName = 'ParentID'
    end
    object TempAllDataChildID: TIntegerField
      FieldName = 'ChildID'
    end
    object TempAllDataOrderId: TIntegerField
      FieldName = 'OrderId'
    end
    object TempAllDataDescription: TStringField
      FieldName = 'Description'
      Size = 100
    end
    object TempAllDataLevel: TIntegerField
      FieldName = 'Level'
    end
    object TempAllDataType: TIntegerField
      FieldName = 'Type'
    end
    object TempAllDataparentDescription: TStringField
      FieldName = 'parentDescription'
      Size = 100
    end
    object TempAllDatapparentId: TIntegerField
      FieldName = 'pparentId'
    end
  end
  object Select_FollowUP_By_Organization: TADOStoredProc
    Connection = Dm.YeganehConnection
    CursorType = ctStatic
    ProcedureName = 'Select_FollowUP_By_Organization'
    Parameters = <
      item
        Name = '@OrgID'
        DataType = ftSmallint
        Value = Null
      end>
    Left = 95
    Top = 38
    object Select_FollowUP_By_OrganizationFollowUPID: TAutoIncField
      FieldName = 'FollowUPID'
      ReadOnly = True
    end
    object Select_FollowUP_By_OrganizationCustomerID: TIntegerField
      FieldName = 'CustomerID'
    end
    object Select_FollowUP_By_OrganizationActionTypeID: TIntegerField
      FieldName = 'ActionTypeID'
    end
    object Select_FollowUP_By_OrganizationDoneStatusID: TWordField
      FieldName = 'DoneStatusID'
    end
    object Select_FollowUP_By_OrganizationMarketerID: TIntegerField
      FieldName = 'MarketerID'
    end
    object Select_FollowUP_By_OrganizationComment: TWideStringField
      Alignment = taRightJustify
      DisplayWidth = 1000
      FieldName = 'Comment'
      Size = 1000
    end
    object Select_FollowUP_By_OrganizationToDoDate: TStringField
      Alignment = taRightJustify
      FieldName = 'ToDoDate'
      FixedChar = True
      Size = 10
    end
    object Select_FollowUP_By_OrganizationDoneDate: TStringField
      Alignment = taRightJustify
      FieldName = 'DoneDate'
      FixedChar = True
      Size = 10
    end
    object Select_FollowUP_By_OrganizationAttachment1: TWideStringField
      Alignment = taRightJustify
      FieldName = 'Attachment1'
      Size = 255
    end
    object Select_FollowUP_By_OrganizationAttachment2: TWideStringField
      Alignment = taRightJustify
      FieldName = 'Attachment2'
      Size = 255
    end
    object Select_FollowUP_By_OrganizationToDoTime: TStringField
      Alignment = taRightJustify
      FieldName = 'ToDoTime'
      FixedChar = True
      Size = 5
    end
    object Select_FollowUP_By_OrganizationActionTypeTitle: TStringField
      Alignment = taRightJustify
      FieldKind = fkLookup
      FieldName = 'ActionTypeTitle'
      LookupKeyFields = 'ActionTypeID'
      LookupResultField = 'ActionTypeTitle'
      KeyFields = 'ActionTypeID'
      Lookup = True
    end
    object Select_FollowUP_By_OrganizationMarketerTilte: TStringField
      Alignment = taRightJustify
      FieldKind = fkLookup
      FieldName = 'MarketerTilte'
      LookupKeyFields = 'MarketerID'
      LookupResultField = 'MarketerTitle'
      KeyFields = 'MarketerID'
      Lookup = True
    end
    object Select_FollowUP_By_OrganizationDoneStatusTitle: TStringField
      Alignment = taRightJustify
      FieldKind = fkLookup
      FieldName = 'DoneStatusTitle'
      LookupKeyFields = 'DoneStatusID'
      LookupResultField = 'DoneStatustitle'
      KeyFields = 'DoneStatusID'
      Lookup = True
    end
    object Select_FollowUP_By_OrganizationSuccessChance: TWordField
      FieldName = 'SuccessChance'
    end
    object Select_FollowUP_By_OrganizationDoneComment: TWideStringField
      Alignment = taRightJustify
      DisplayWidth = 1000
      FieldName = 'DoneComment'
      Size = 1000
    end
    object Select_FollowUP_By_OrganizationWordAttachment: TBlobField
      FieldName = 'WordAttachment'
    end
    object Select_FollowUP_By_OrganizationPicAttachment: TBlobField
      FieldName = 'PicAttachment'
    end
    object Select_FollowUP_By_OrganizationActionTypeTitleAll: TWideStringField
      FieldKind = fkLookup
      FieldName = 'ActionTypeTitleAll'
      LookupDataSet = ActionType
      LookupKeyFields = 'ActionTypeID'
      LookupResultField = 'ActionTypeTitle'
      KeyFields = 'ActionTypeID'
      Size = 100
      Lookup = True
    end
    object Select_FollowUP_By_OrganizationParentFollowUpID: TIntegerField
      FieldName = 'ParentFollowUpID'
    end
    object Select_FollowUP_By_OrganizationStatusSaveDate: TStringField
      FieldName = 'StatusSaveDate'
      Size = 10
    end
    object Select_FollowUP_By_OrganizationFollowUpInsertDate: TStringField
      FieldName = 'FollowUpInsertDate'
      Size = 10
    end
    object Select_FollowUP_By_OrganizationFullActionTitle: TStringField
      FieldName = 'FullActionTitle'
      Size = 200
    end
    object Select_FollowUP_By_OrganizationExcellAttachment: TBlobField
      FieldName = 'ExcellAttachment'
    end
    object Select_FollowUP_By_OrganizationPdfAttachment: TBlobField
      FieldName = 'PdfAttachment'
    end
    object Select_FollowUP_By_OrganizationIsFactor: TBooleanField
      Alignment = taCenter
      FieldName = 'IsFactor'
    end
    object Select_FollowUP_By_OrganizationIsPreFactor: TBooleanField
      Alignment = taCenter
      FieldName = 'IsPreFactor'
    end
    object Select_FollowUP_By_OrganizationFactorId: TIntegerField
      FieldName = 'FactorId'
    end
    object Select_FollowUP_By_OrganizationPicAttachmentType: TStringField
      FieldName = 'PicAttachmentType'
      Size = 5
    end
    object Select_FollowUP_By_OrganizationIsAuto: TBooleanField
      FieldName = 'IsAuto'
    end
  end
  object DSelect_FollowUP_By_Organization: TDataSource
    AutoEdit = False
    DataSet = Select_FollowUP_By_Organization
    Left = 88
    Top = 98
  end
end
