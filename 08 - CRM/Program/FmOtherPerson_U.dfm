inherited FmOtherPerson: TFmOtherPerson
  Left = 660
  Top = 255
  Width = 617
  Height = 627
  BorderIcons = [biSystemMenu]
  Caption = #1601#1585#1605' '#1579#1576#1578' '#1605#1588#1582#1589#1575#1578' '#1587#1575#1610#1585' '#1575#1588#1582#1575#1589' '#1605#1585#1578#1576#1591
  KeyPreview = True
  OldCreateOrder = True
  Position = poDesktopCenter
  OnCanResize = FormCanResize
  PixelsPerInch = 96
  TextHeight = 13
  inherited ShapeBase: TShape
    Width = 601
    Height = 588
  end
  object pnlMain: TPanel [1]
    Left = 0
    Top = 0
    Width = 601
    Height = 588
    Align = alClient
    TabOrder = 0
    object Panel3: TPanel
      Left = 1
      Top = 535
      Width = 599
      Height = 52
      Align = alBottom
      TabOrder = 0
      object SBClose: TSpeedButton
        Left = 12
        Top = 1
        Width = 55
        Height = 50
        Caption = #1582#1585#1608#1580
        Flat = True
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
        Layout = blGlyphTop
        NumGlyphs = 2
        Spacing = 0
        OnClick = SBCloseClick
      end
      object DBNavigator1: TDBNavigator
        Left = 102
        Top = 1
        Width = 496
        Height = 50
        DataSource = DSelect_Person_By_CustomerID
        VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast, nbInsert, nbDelete, nbEdit, nbPost, nbCancel]
        Align = alRight
        Constraints.MaxHeight = 50
        Constraints.MinHeight = 50
        Constraints.MinWidth = 50
        Flat = True
        Ctl3D = True
        ParentCtl3D = False
        ParentShowHint = False
        ConfirmDelete = False
        ShowHint = True
        TabOrder = 0
        TabStop = True
        OnClick = DBNavigator1Click
      end
    end
    object Panel2: TPanel
      Left = 1
      Top = 374
      Width = 599
      Height = 161
      Align = alBottom
      TabOrder = 1
      DesignSize = (
        599
        161)
      object Label1: TLabel
        Left = 527
        Top = 16
        Width = 13
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1606#1575#1605
        FocusControl = DBEFirstName
      end
      object Label2: TLabel
        Left = 527
        Top = 42
        Width = 58
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1606#1575#1605' '#1582#1575#1606#1608#1575#1583#1711#1610
        FocusControl = DBELastName
      end
      object Label3: TLabel
        Left = 527
        Top = 94
        Width = 25
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1575#1610#1605#1610#1604
        FocusControl = DBEEmailAddress
      end
      object Label4: TLabel
        Left = 259
        Top = 96
        Width = 46
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1578#1604#1601#1606' '#1605#1606#1586#1604
        FocusControl = DBEHomePhone
      end
      object Label5: TLabel
        Left = 259
        Top = 17
        Width = 21
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1578#1604#1601#1606
        FocusControl = DBEWorkPhone
      end
      object Label6: TLabel
        Left = 259
        Top = 69
        Width = 28
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1605#1608#1576#1575#1610#1604
        FocusControl = DBEMobilePhone
      end
      object Label7: TLabel
        Left = 259
        Top = 43
        Width = 25
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1601#1705#1587
        FocusControl = DBEFaxNumber
      end
      object Label8: TLabel
        Left = 527
        Top = 68
        Width = 28
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1587#1605#1578
        FocusControl = DBEJobTitle
      end
      object Label9: TLabel
        Left = 527
        Top = 120
        Width = 40
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1578#1608#1590#1610#1581#1575#1578
        FocusControl = DBENotes
      end
      object DBEFirstName: TDBEdit
        Left = 320
        Top = 16
        Width = 200
        Height = 21
        Anchors = [akTop, akRight]
        DataField = 'FirstName'
        DataSource = DSelect_Person_By_CustomerID
        TabOrder = 0
      end
      object DBELastName: TDBEdit
        Left = 320
        Top = 42
        Width = 200
        Height = 21
        Anchors = [akTop, akRight]
        DataField = 'LastName'
        DataSource = DSelect_Person_By_CustomerID
        TabOrder = 1
      end
      object DBEEmailAddress: TDBEdit
        Left = 320
        Top = 94
        Width = 200
        Height = 21
        Anchors = [akTop, akRight]
        BiDiMode = bdLeftToRight
        DataField = 'EmailAddress'
        DataSource = DSelect_Person_By_CustomerID
        ParentBiDiMode = False
        TabOrder = 3
        OnEnter = DBEEmailAddressEnter
        OnExit = DBEEmailAddressExit
      end
      object DBEHomePhone: TDBEdit
        Left = 53
        Top = 94
        Width = 200
        Height = 21
        Anchors = [akLeft, akTop, akRight]
        DataField = 'HomePhone'
        DataSource = DSelect_Person_By_CustomerID
        TabOrder = 7
      end
      object DBEWorkPhone: TDBEdit
        Left = 53
        Top = 16
        Width = 200
        Height = 21
        Anchors = [akLeft, akTop, akRight]
        DataField = 'WorkPhone'
        DataSource = DSelect_Person_By_CustomerID
        TabOrder = 4
      end
      object DBEMobilePhone: TDBEdit
        Left = 53
        Top = 68
        Width = 200
        Height = 21
        Anchors = [akLeft, akTop, akRight]
        DataField = 'MobilePhone'
        DataSource = DSelect_Person_By_CustomerID
        TabOrder = 6
      end
      object DBEFaxNumber: TDBEdit
        Left = 53
        Top = 42
        Width = 200
        Height = 21
        Anchors = [akLeft, akTop, akRight]
        DataField = 'FaxNumber'
        DataSource = DSelect_Person_By_CustomerID
        TabOrder = 5
      end
      object DBEJobTitle: TDBEdit
        Left = 320
        Top = 68
        Width = 200
        Height = 21
        Anchors = [akTop, akRight]
        DataField = 'JobTitle'
        DataSource = DSelect_Person_By_CustomerID
        TabOrder = 2
      end
      object DBENotes: TDBEdit
        Left = 53
        Top = 120
        Width = 468
        Height = 21
        Anchors = [akLeft, akTop, akRight]
        DataField = 'Notes'
        DataSource = DSelect_Person_By_CustomerID
        TabOrder = 8
      end
    end
    object Panel1: TPanel
      Left = 1
      Top = 73
      Width = 599
      Height = 301
      Align = alClient
      TabOrder = 2
      object YDBGrid1: TYDBGrid
        Left = 1
        Top = 1
        Width = 597
        Height = 299
        Cursor = crHandPoint
        TabStop = False
        Align = alClient
        DataSource = DSelect_Person_By_CustomerID
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = ARABIC_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        YeganehColor = False
        YeganehFinish = False
        PageNumber = False
        RecordNumber = False
        HasTime = False
        Oriention = poPortrait
        HasDate = False
        HasIndex = False
        PrintTitle = #1604#1610#1587#1578' '#1578#1607#1610#1607' '#1588#1583#1607' '#1578#1608#1587#1591' '#1606#1585#1605' '#1575#1601#1586#1575#1585' '#1610#1711#1575#1606#1607
        InvertFarsiDate = True
        TitleSort = True
        AutoInsert = False
        FooterFields = 'Count'
        Columns = <
          item
            Expanded = False
            FieldName = 'FirstName'
            Title.Alignment = taCenter
            Title.Caption = #1606#1575#1605
            Width = 114
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'LastName'
            Title.Alignment = taCenter
            Title.Caption = #1606#1575#1605' '#1582#1575#1606#1608#1575#1583#1711#1610
            Width = 182
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'JobTitle'
            Title.Alignment = taCenter
            Title.Caption = #1587#1605#1578
            Width = 100
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'WorkPhone'
            Title.Alignment = taCenter
            Title.Caption = #1578#1604#1601#1606
            Width = 100
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'MobilePhone'
            Title.Alignment = taCenter
            Title.Caption = #1605#1608#1576#1575#1610#1604
            Width = 100
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'FaxNumber'
            Title.Alignment = taCenter
            Title.Caption = #1601#1705#1587
            Width = 122
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'HomePhone'
            Title.Alignment = taCenter
            Title.Caption = #1578#1604#1601#1606' '#1605#1606#1586#1604
            Width = 116
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'EmailAddress'
            Title.Alignment = taCenter
            Title.Caption = #1575#1610#1605#1610#1604
            Width = 212
            Visible = True
          end>
      end
    end
    object pnlSearch: TPanel
      Left = 1
      Top = 1
      Width = 599
      Height = 72
      Align = alTop
      TabOrder = 3
      DesignSize = (
        599
        72)
      object Label10: TLabel
        Left = 495
        Top = 16
        Width = 58
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1606#1575#1605
      end
      object Label11: TLabel
        Left = 495
        Top = 42
        Width = 58
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1606#1575#1605' '#1582#1575#1606#1608#1575#1583#1711#1610
      end
      object Label14: TLabel
        Left = 227
        Top = 17
        Width = 21
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1578#1604#1601#1606
      end
      object Label15: TLabel
        Left = 227
        Top = 45
        Width = 28
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1605#1608#1576#1575#1610#1604
      end
      object pnlFilter: TPanel
        Left = 568
        Top = 1
        Width = 30
        Height = 70
        Align = alRight
        TabOrder = 0
        DesignSize = (
          30
          70)
        object SBRefresh: TSpeedButton
          Left = 2
          Top = 25
          Width = 26
          Height = 24
          Hint = #1576#1575#1586#1610#1575#1576#1610' '#1583#1608#1576#1575#1585#1607' '#1575#1591#1604#1575#1593#1575#1578
          Anchors = [akTop, akRight]
          Flat = True
          Glyph.Data = {
            
            48E169472F931F150D2B00000000000000000000000000000000}
          OnClick = SBRefreshClick
        end
        object btnHome: TSpeedButton
          Left = 2
          Top = 1
          Width = 26
          Height = 24
          Hint = #1576#1575#1586#1711#1588#1578' '#1576#1607' '#1581#1575#1604#1578' '#1575#1608#1604#1610#1607
          Anchors = [akTop, akRight]
          Flat = True
          Glyph.Data = {
            
            8BFFA1758BFFA1758BFFA1758BFFA1758BFFA1758BFFA1758BFF}
          ParentShowHint = False
          ShowHint = True
          OnClick = btnHomeClick
        end
      end
      object edtName: TEdit
        Left = 286
        Top = 16
        Width = 200
        Height = 21
        Anchors = [akTop, akRight]
        TabOrder = 1
      end
      object edtTel: TEdit
        Left = 15
        Top = 15
        Width = 200
        Height = 21
        Anchors = [akTop, akRight]
        TabOrder = 2
      end
      object edtMobile: TEdit
        Left = 15
        Top = 40
        Width = 200
        Height = 21
        Anchors = [akTop, akRight]
        TabOrder = 3
      end
    end
  end
  object edtLastName: TEdit [2]
    Left = 287
    Top = 41
    Width = 200
    Height = 21
    Anchors = [akTop, akRight]
    TabOrder = 1
  end
  object Select_Person_By_CustomerID: TADOStoredProc
    Connection = Dm.YeganehConnection
    CursorType = ctStatic
    AfterInsert = Select_Person_By_CustomerIDAfterInsert
    ProcedureName = 'Select_Person_By_CustomerID;1'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
        Value = 0
      end
      item
        Name = '@CustomerID'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end>
    Left = 225
    Top = 103
    object Select_Person_By_CustomerIDPersonID: TAutoIncField
      FieldName = 'PersonID'
      ReadOnly = True
    end
    object Select_Person_By_CustomerIDCustomerID: TIntegerField
      FieldName = 'CustomerID'
    end
    object Select_Person_By_CustomerIDFirstName: TWideStringField
      Alignment = taRightJustify
      FieldName = 'FirstName'
      Size = 50
    end
    object Select_Person_By_CustomerIDLastName: TWideStringField
      Alignment = taRightJustify
      FieldName = 'LastName'
      Size = 100
    end
    object Select_Person_By_CustomerIDEmailAddress: TWideStringField
      FieldName = 'EmailAddress'
      Size = 50
    end
    object Select_Person_By_CustomerIDHomePhone: TWideStringField
      Alignment = taRightJustify
      FieldName = 'HomePhone'
      Size = 30
    end
    object Select_Person_By_CustomerIDWorkPhone: TWideStringField
      Alignment = taRightJustify
      DisplayWidth = 50
      FieldName = 'WorkPhone'
      Size = 50
    end
    object Select_Person_By_CustomerIDMobilePhone: TWideStringField
      Alignment = taRightJustify
      FieldName = 'MobilePhone'
      Size = 30
    end
    object Select_Person_By_CustomerIDFaxNumber: TWideStringField
      Alignment = taRightJustify
      FieldName = 'FaxNumber'
      Size = 30
    end
    object Select_Person_By_CustomerIDNotes: TWideStringField
      Alignment = taRightJustify
      FieldName = 'Notes'
      Size = 255
    end
    object Select_Person_By_CustomerIDJobTitle: TWideStringField
      Alignment = taRightJustify
      FieldName = 'JobTitle'
      Size = 50
    end
  end
  object DSelect_Person_By_CustomerID: TDataSource
    AutoEdit = False
    DataSet = Select_Person_By_CustomerID
    Left = 225
    Top = 151
  end
end
