...
type
  TFrContract = class(TMBaseForm)
    ...
    edtGuaranteeAmount: TKimiyaEdit;
...
//    dbeGuaranteeAmount.DataSource := dsrGuarantee;
   ...
   if not QryGuarantee.FieldByName('Amount').IsNull then
    begin
          ...
          edtGuaranteeAmount.Text := AddSeprator(QryGuarantee.FieldByName('Amount').AsString,length(QryGuarantee.FieldByName('Amount').AsString) div 3)
          else   edtGuaranteeAmount.Text := QryGuarantee.FieldByName('Amount').AsString;
    end else edtGuaranteeAmount.Clear;
...
begin
  ...
  if (pnlGuarantee.Minimized)and (QryGuarantee.State in [dsinsert,dsEdit])
              then if edtGuaranteeAmount.Text = '' then   QryGuarantee.Cancel else QryGuarantee.Post;
...
begin
  ...
  if (StrToIntDef(edtGuaranteeAmount.Text,0) <> 0) then
  begin
  if not (QryGuarantee.State in [dsedit,dsinsert]) then QryGuarantee.Edit;
     QryGuarantee.FieldByName('Amount').Value := edtGuaranteeAmount.Text
  end
