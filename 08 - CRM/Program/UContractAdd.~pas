...
begin
  ...
  begin
  ShowMessage('‘„« œ” —”Ì ÃÂ  ÊÌ—«Ì‘ ‰œ«—Ìœ');
  abort;
  end;
  if (dbeEndDateCNT.Text = '')or(dbeBeginDateCNT.Text = '') then
  Begin
     ShowMessage(' «—ÌŒ ‘—Ê⁄ Ê Å«Ì«‰ ﬁ—«—œ«œ Õ „« »«Ìœ À»  ‘Êœ');
     exit;
  end;
...
begin
  inherited;
  if MessageDlg('ò«—»— ê—«„Ì ° ¬Ì« «“ Õ–› ›«Ì· «ò”· «ÿ„Ì‰«‰ œ«—Ìœø',mtConfirmation, [mbYes, mbNo], 0) = mrYes then
   begin
...
begin
  inherited;
  if MessageDlg('ò«—»— ê—«„Ì ° ¬Ì« «“ Õ–› „ ‰ ﬁ—«—œ«œ «ÿ„Ì‰«‰ œ«—Ìœø',mtConfirmation, [mbYes, mbNo], 0) = mrYes then
   begin
    AQry := TADOQuery.Create(Self);
     try
      AQry.Connection := Dm.YeganehConnection;
      ...
      AQry.ExecSQL;
      ShowMessage('„ ‰ ﬁ—«—œ«œ »« „Ê›ﬁÌ  Õ–› ê—œÌœ');
     finally
      if Assigned(AQry) then
        FreeAndNil(AQry);
     end;
