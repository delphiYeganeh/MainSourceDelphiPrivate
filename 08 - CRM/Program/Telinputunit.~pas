...
type
  TPropertiesForm = class(TMBaseForm)
    ...
    procedure DBECustomerIDChange(Sender: TObject);
  private
    CustomerID: integer;
    IsNewRecord : Boolean;
...
begin
  ...
  if gbInformationSale.Visible then
  begin
    //Panel2.Height := Panel2.Height - gbInformationSale.Height ;

    Height := Height + gbInformationSale.Height ;
  end;
...
//   DBNav_Setup(DBNavigator1);
   ...
   else Self.Caption := '›—„ À»  Ê «’·«Õ „‘Œ’«  „‘ —Ì«‰' + '"' + Dm.Select_Customer_By_CustomerIDCustomerID.AsString  + '"' ;
...
begin
  ...
  if  button = nbEdit then
      Self.Caption := '›—„ À»  Ê «’·«Õ „‘Œ’«  „‘ —Ì«‰' + '"ÊÌ—«Ì‘"' + Dm.Select_Customer_By_CustomerIDCustomerID.AsString
  else
  if  button = nbInsert then
      Self.Caption := '›—„ À»  Ê «’·«Õ „‘Œ’«  „‘ —Ì«‰' + '"„‘ —Ì ÃœÌœ"'
  else
  Self.Caption := '›—„ À»  Ê «’·«Õ „‘Œ’«  „‘ —Ì«‰' + '"' + Dm.Select_Customer_By_CustomerIDCustomerID.AsString  + '"' ;

  if  button in [nbInsert,nbEdit] then
    edtAmount.Enabled := True
  else  edtAmount.Enabled := False;
...
//     ButtonName  := 'Insert'
   ...
   if (ButtonName <> '') and (Dm.Select_Customer_By_CustomerIDCustomerID.AsString <> '') then
     Qry_SetResult('Insert into dbo.ActivityUser_Log(CustomerID,UserId,Date,FormName,Activity,ComputerName,IPAddress) select '+Dm.Select_Customer_By_CustomerIDCustomerID.AsString+','+ Dm.UserId.AsString + ',getdate(),''PropertiesForm'','''+ButtonName+''', '''+UpperCase(_ComputerName) +''','''+_IpAddress +'''' ,dm.YeganehConnection);

   if Button = nbPost then
   begin
     edtAmount2.text := RemoveSeparetor (edtAmount.Text);
...
procedure TPropertiesForm.edtAmountKeyPress(Sender: TObject;
  var Key: Char);
...
procedure TPropertiesForm.edtAmountChange(Sender: TObject);
var
   AmountSTR : string;
begin
  inherited;
...
function TPropertiesForm.RemoveSeparetor(text: String): String;
  var tx : String;
begin
   while pos('°',text) <> 0 do
   begin
...
procedure TPropertiesForm.DBECustomerIDChange(Sender: TObject);
begin
  inherited;
  edtAmount.text := edtAmount2.Text;
end;
