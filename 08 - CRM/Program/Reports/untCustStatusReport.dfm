inherited frmCustStatusReport: TfrmCustStatusReport
  Left = 334
  Top = 225
  Width = 1173
  Height = 713
  Caption = #1711#1586#1575#1585#1588' '#1608#1590#1593#1610#1578' '#1605#1588#1578#1585#1610#1575#1606
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel [0]
    Left = 0
    Top = 0
    Width = 1157
    Height = 41
    Align = alTop
    TabOrder = 0
    DesignSize = (
      1157
      41)
    object Label1: TLabel
      Left = 1115
      Top = 10
      Width = 31
      Height = 13
      Caption = #1587#1575#1604' : '
    end
    object Label2: TLabel
      Left = 902
      Top = 11
      Width = 73
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1591#1585#1610#1602#1607' '#1605#1593#1585#1601#1610' : '
      FocusControl = DBLookupComboBox1
    end
    object Label3: TLabel
      Left = 590
      Top = 11
      Width = 91
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1605#1581#1589#1608#1604#1575#1578' / '#1582#1583#1605#1575#1578' :'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label4: TLabel
      Left = 330
      Top = 12
      Width = 49
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1606#1608#1593' '#1601#1593#1575#1604#1610#1578
      FocusControl = DBLkCBGroupTitle
    end
    object ComboBox1: TComboBox
      Left = 1032
      Top = 8
      Width = 81
      Height = 21
      ItemHeight = 13
      ItemIndex = 13
      TabOrder = 0
      Text = '1399'
      Items.Strings = (
        '1386'
        '1387'
        '1388'
        '1389'
        '1390'
        '1391'
        '1392'
        '1393'
        '1394'
        '1395'
        '1396'
        '1397'
        '1398'
        '1399'
        '1400'
        '1401'
        '1402'
        '1403'
        '1404'
        '1405')
    end
    object DBLookupComboBox1: TDBLookupComboBox
      Left = 727
      Top = 8
      Width = 170
      Height = 21
      Anchors = [akTop, akRight]
      BiDiMode = bdLeftToRight
      DropDownRows = 20
      KeyField = 'CompaginID'
      ListField = 'CompaginTitle'
      ListSource = Dm.DCompagin
      ParentBiDiMode = False
      TabOrder = 1
    end
    object DBLookupComboBox2: TDBLookupComboBox
      Left = 415
      Top = 8
      Width = 170
      Height = 21
      Anchors = [akTop, akRight]
      BiDiMode = bdLeftToRight
      DropDownRows = 20
      KeyField = 'ProductID'
      ListField = 'ProductTitle'
      ListSource = DProduct
      ParentBiDiMode = False
      TabOrder = 2
    end
    object Button1: TButton
      Left = 396
      Top = 8
      Width = 17
      Height = 21
      Caption = '*'
      TabOrder = 3
      OnClick = Button1Click
    end
    object Button2: TButton
      Left = 708
      Top = 8
      Width = 17
      Height = 21
      Caption = '*'
      TabOrder = 4
      OnClick = Button2Click
    end
    object DBLkCBGroupTitle: TDBLookupComboBox
      Left = 156
      Top = 9
      Width = 170
      Height = 21
      Anchors = [akTop, akRight]
      BiDiMode = bdLeftToRight
      DropDownRows = 20
      KeyField = 'GroupID'
      ListField = 'GroupTitle'
      ListSource = DGroup
      ParentBiDiMode = False
      TabOrder = 5
    end
    object Button3: TButton
      Left = 136
      Top = 8
      Width = 17
      Height = 21
      Caption = '*'
      TabOrder = 6
      OnClick = Button3Click
    end
  end
  object CustGrid: TYDBGrid [1]
    Left = 0
    Top = 41
    Width = 1157
    Height = 592
    Cursor = crHandPoint
    Align = alClient
    DataSource = DSForm
    Font.Charset = ARABIC_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
    ParentFont = False
    ReadOnly = True
    TabOrder = 1
    TitleFont.Charset = ARABIC_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    YeganehColor = True
    YeganehFinish = False
    PageNumber = False
    RecordNumber = False
    HasTime = False
    Oriention = poPortrait
    HasDate = False
    HasIndex = False
    PrintTitle = #1711#1586#1575#1585#1588' '#1608#1590#1593#1610#1578' '#1605#1588#1578#1585#1610#1575#1606
    InvertFarsiDate = True
    TitleSort = True
    AutoInsert = False
    FooterFields = 'Count'
    OnNeedFontCondition = CustGridNeedFontCondition
    Columns = <
      item
        Expanded = False
        FieldName = 'Id'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'CustomerStatusID'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'CustomerStatusTitle'
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        Title.Caption = #1608#1590#1593#1610#1578
        Width = 208
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'One'
        Title.Alignment = taCenter
        Title.Caption = #1601#1585#1608#1585#1583#1610#1606
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Two'
        Title.Alignment = taCenter
        Title.Caption = #1575#1585#1583#1610#1576#1607#1588#1578
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Three'
        Title.Alignment = taCenter
        Title.Caption = #1582#1585#1583#1575#1583
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Four'
        Title.Alignment = taCenter
        Title.Caption = #1578#1610#1585
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Five'
        Title.Alignment = taCenter
        Title.Caption = #1605#1585#1583#1575#1583
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Six'
        Title.Alignment = taCenter
        Title.Caption = #1588#1607#1585#1610#1608#1585
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Seven'
        Title.Alignment = taCenter
        Title.Caption = #1605#1607#1585
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Eight'
        Title.Alignment = taCenter
        Title.Caption = #1570#1576#1575#1606
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Nine'
        Title.Alignment = taCenter
        Title.Caption = #1570#1584#1585
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Ten'
        Title.Alignment = taCenter
        Title.Caption = #1583#1610
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Eleven'
        Title.Alignment = taCenter
        Title.Caption = #1576#1607#1605#1606
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Twelve'
        Title.Alignment = taCenter
        Title.Caption = #1575#1587#1601#1606#1583
        Visible = True
      end>
  end
  object Panel2: TPanel [2]
    Left = 0
    Top = 633
    Width = 1157
    Height = 41
    Align = alBottom
    TabOrder = 2
    OnClick = Panel2Click
    DesignSize = (
      1157
      41)
    object SpeedButton1: TSpeedButton
      Left = 970
      Top = 8
      Width = 26
      Height = 25
      Hint = #1575#1585#1587#1575#1604' '#1576#1607' '#1608#1585#1583
      Anchors = [akTop, akRight]
      Flat = True
      Glyph.Data = {
        
        54C9744EC36F47C46F46AA5B3000}
      OnClick = SpeedButton1Click
    end
    object SpeedButton2: TSpeedButton
      Left = 996
      Top = 8
      Width = 26
      Height = 25
      Hint = #1670#1575#1662
      Anchors = [akTop, akRight]
      Flat = True
      Glyph.Data = {
        
        ADE9F2E7FFFFFFFFFFFF}
      OnClick = SpeedButton2Click
    end
    object SpeedButton3: TSpeedButton
      Left = 1022
      Top = 8
      Width = 24
      Height = 25
      Hint = #1575#1585#1587#1575#1604' '#1576#1607' '#1575#1603#1587#1604
      Anchors = [akTop, akRight]
      Flat = True
      Glyph.Data = {
        
        A47A63A0745B986D5693684F8F634B8B6044865B4A8D5D3A734F}
      OnClick = SpeedButton3Click
    end
    object BitBtn1: TBitBtn
      Left = 1076
      Top = 8
      Width = 75
      Height = 25
      Caption = #1578#1607#1610#1607' '#1711#1586#1575#1585#1588
      TabOrder = 0
      OnClick = BitBtn1Click
    end
  end
  inherited DSForm: TDataSource
    DataSet = ADOQuery
  end
  object ADOQuery: TADOQuery
    Connection = Dm.YeganehConnection
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      
        'select Id, CustomerStatusID, CustomerStatusTitle,One, Two, Three' +
        ', Four, Five, Six, Seven, Eight, Nine, Ten, Eleven, Twelve from ' +
        'CustomerStatusReport')
    Left = 136
    Top = 224
    object ADOQueryId: TAutoIncField
      FieldName = 'Id'
      ReadOnly = True
    end
    object ADOQueryCustomerStatusID: TIntegerField
      FieldName = 'CustomerStatusID'
    end
    object ADOQueryCustomerStatusTitle: TStringField
      FieldName = 'CustomerStatusTitle'
      Size = 1000
    end
    object ADOQueryOne: TIntegerField
      Alignment = taCenter
      FieldName = 'One'
    end
    object ADOQueryTwo: TIntegerField
      Alignment = taCenter
      FieldName = 'Two'
    end
    object ADOQueryThree: TIntegerField
      Alignment = taCenter
      FieldName = 'Three'
    end
    object ADOQueryFour: TIntegerField
      Alignment = taCenter
      FieldName = 'Four'
    end
    object ADOQueryFive: TIntegerField
      Alignment = taCenter
      FieldName = 'Five'
    end
    object ADOQuerySix: TIntegerField
      Alignment = taCenter
      FieldName = 'Six'
    end
    object ADOQuerySeven: TIntegerField
      Alignment = taCenter
      FieldName = 'Seven'
    end
    object ADOQueryEight: TIntegerField
      Alignment = taCenter
      FieldName = 'Eight'
    end
    object ADOQueryNine: TIntegerField
      Alignment = taCenter
      FieldName = 'Nine'
    end
    object ADOQueryTen: TIntegerField
      Alignment = taCenter
      FieldName = 'Ten'
    end
    object ADOQueryEleven: TIntegerField
      Alignment = taCenter
      FieldName = 'Eleven'
    end
    object ADOQueryTwelve: TIntegerField
      Alignment = taCenter
      FieldName = 'Twelve'
    end
  end
  object DProduct: TDataSource
    DataSet = Product
    Left = 470
    Top = 223
  end
  object Product: TADOQuery
    Connection = Dm.YeganehConnection
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'Select *'
      'From Product'
      'Where IsSell = 1')
    Left = 472
    Top = 176
    object ProductProductID: TWordField
      FieldName = 'ProductID'
      ReadOnly = True
    end
    object ProductProductTitle: TWideStringField
      Alignment = taRightJustify
      FieldName = 'ProductTitle'
      Size = 100
    end
    object ProductComment: TWideStringField
      Alignment = taRightJustify
      FieldName = 'Comment'
      Size = 100
    end
    object ProductProductCategoryID: TWordField
      FieldName = 'ProductCategoryID'
    end
    object ProductCommisionPercent: TWordField
      FieldName = 'CommisionPercent'
    end
    object ProductCode: TStringField
      Alignment = taRightJustify
      FieldName = 'Code'
      Size = 5
    end
    object ProductLastVertion: TStringField
      Alignment = taRightJustify
      FieldName = 'LastVertion'
      Size = 50
    end
    object ProductIsNetwork: TBooleanField
      Alignment = taRightJustify
      FieldName = 'IsNetwork'
    end
    object ProductavgFollouwup: TIntegerField
      FieldName = 'avgFollouwup'
    end
  end
  object DGroup: TDataSource
    DataSet = Group
    Left = 246
    Top = 255
  end
  object Group: TADOQuery
    Connection = Dm.YeganehConnection
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'select * from Groups')
    Left = 248
    Top = 208
    object GroupGroupID: TAutoIncField
      FieldName = 'GroupID'
      ReadOnly = True
    end
    object GroupGroupTitle: TWideStringField
      FieldName = 'GroupTitle'
      Size = 50
    end
    object GroupGroupNo: TStringField
      FieldName = 'GroupNo'
      Size = 50
    end
    object Groupeffect: TWordField
      FieldName = 'effect'
    end
  end
end
