...
//-------------------
  ...
  StatusBar1.Panels[2].Text:=' ⁄œ«œ : '+IntToStr(PhoneGrid.DataSource.DataSet.RecordCount);
...
begin
   if LockReport.Visible then
   begin
     if MessageDlg('ê“«—‘ ﬁ›· „Ì »«‘œ ·ÿ›« «» œ« ê“«—‘ «“ Õ«·  ﬁ›· Œ«—Ã ‰„«ÌÌœ',mtWarning,[mbyes,mbno],0)=mryes then
...
begin
  ...
  else StatusBar1.Panels[2].Text := '  ⁄œ«œ 0';
               }
   CountPanelSet;
...
begin
  ...
  if (FollowGrid.DataSource <> nil) and  (FollowGrid.DataSource.DataSet.Active) then
    StatusBar1.Panels[2].Text:=' ⁄œ«œ : '+IntToStr(FollowGrid.DataSource.DataSet.RecordCount)
  else StatusBar1.Panels[2].Text:=' ⁄œ«œ : ' + '0' ;
  }
   // Amin 1391/10/04 End
...
begin
   ...
   else ShowMyMessage('ÅÌ€«„','ÂÌç «ÿ·«⁄« Ì Ì«›  ‰‘œ',[mbOK],mtInformation);
end;
...
begin
    ...
    SetQueryDataSet(dsdblsrchContractType,qrydblsrchContractType
     ...
     begin
         ...
         StatusBar1.Panels[2].Text:=' ⁄œ«œ : '+IntToStr(qrydblsrchContractType.RecordCount);
     end
     else StatusBar1.Panels[2].Text:=' ⁄œ«œ : '+ ' 0';
...
begin
  ...
  if (ContractGrid.DataSource <> nil) and (ContractGrid.DataSource.DataSet.Active) then
   StatusBar1.Panels[2].Text:=' ⁄œ«œ : '+IntToStr(ContractGrid.DataSource.DataSet.RecordCount)
  else StatusBar1.Panels[2].Text:=' ⁄œ«œ : ' + '0';
end;
...
begin
  ...
  if  qryMyTasks.Active then
    StatusBar1.Panels[2].Text:=' ⁄œ«œ : '+IntToStr(qryMyTasks.RecordCount)
  else StatusBar1.Panels[2].Text:=' ⁄œ«œ : ' + '0' ;
end;
...
begin
  ...
  begin
    ...
    qryUpdate.SQL.Text := 'Insert dbo.Messages (InserteadUserId, CurrentDate,MessageType,Subject,Description,UserID)'+
                          ' select '+IntToStr(_IsSystemUserID)+',getdate(),1,'+''''+'„Ê÷Ê⁄'+''''+','+''''+' ” '+''''+','+ inttostr(_userid) +
...
begin
   ...
   if (FollowGrid.DataSource <> nil) and  (FollowGrid.DataSource.DataSet.Active) then
      StatusBar1.Panels[2].Text := '  ⁄œ«œ „‘ —Ì : ' +IntToStr(Dm.Select_FollowUP_By_Date.Parameters.ParamByName('@CountUniqCustomer').Value)
   else
      StatusBar1.Panels[2].Text := ' ⁄œ«œ : ' + '0' ;
