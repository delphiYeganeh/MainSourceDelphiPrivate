inherited frCustomerReport: TfrCustomerReport
  Left = 340
  Top = 184
  Width = 696
  Height = 385
  Caption = ''
  Color = 16244694
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel [0]
    Left = 0
    Top = 0
    Width = 688
    Height = 49
    Align = alTop
    Color = 16244694
    TabOrder = 0
    DesignSize = (
      688
      49)
    object Label1: TLabel
      Left = 470
      Top = 20
      Width = 109
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1585#1608#1586' '#1605#1575#1606#1583#1607' '#1576#1607' '#1662#1575#1610#1575#1606' '#1602#1585#1575#1585#1583#1575#1583' '
    end
    object Label2: TLabel
      Left = 645
      Top = 20
      Width = 31
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1603#1605#1578#1585' '#1575#1586
    end
    object edtNumber: TEdit
      Left = 584
      Top = 16
      Width = 49
      Height = 21
      Anchors = [akTop, akRight]
      TabOrder = 0
      OnKeyPress = edtNumberKeyPress
    end
    object BitBtn1: TBitBtn
      Left = 382
      Top = 12
      Width = 75
      Height = 25
      Anchors = [akTop, akRight]
      Caption = #1580#1587#1578#1580#1608
      TabOrder = 1
      OnClick = BitBtn1Click
    end
  end
  object Panel2: TPanel [1]
    Left = 0
    Top = 304
    Width = 688
    Height = 47
    Align = alBottom
    Color = 16244694
    TabOrder = 1
    DesignSize = (
      688
      47)
    object BtnCancel: TBitBtn
      Left = 11
      Top = 5
      Width = 95
      Height = 34
      Anchors = [akTop, akRight]
      Cancel = True
      Caption = #1582#1585#1608#1580
      ModalResult = 2
      TabOrder = 0
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object btnShowContractWord: TBitBtn
      Left = 108
      Top = 5
      Width = 95
      Height = 34
      Caption = #1575#1585#1587#1575#1604' '#1576#1607' Word'
      TabOrder = 1
      OnClick = btnShowContractWordClick
      Glyph.Data = {
        
        54C9744EC36F47C46F46AA5B3000}
    end
    object BitBtn2: TBitBtn
      Left = 205
      Top = 5
      Width = 95
      Height = 34
      Caption = #1670#1575#1662' '#1604#1610#1587#1578
      TabOrder = 2
      OnClick = BitBtn2Click
    end
    object BitBtn3: TBitBtn
      Left = 302
      Top = 5
      Width = 95
      Height = 34
      Caption = #1575#1585#1587#1575#1604' '#1576#1607' Excel'
      TabOrder = 3
      OnClick = BitBtn3Click
    end
  end
  object dbgCustomerReport: TYDBGrid [2]
    Left = 0
    Top = 121
    Width = 688
    Height = 183
    Cursor = crHandPoint
    Align = alClient
    Color = 16053492
    DataSource = DSForm
    ReadOnly = True
    TabOrder = 2
    TitleFont.Charset = ARABIC_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    OnDblClick = dbgCustomerReportDblClick
    YeganehColor = True
    YeganehFinish = False
    PageNumber = False
    RecordNumber = False
    HasTime = False
    Oriention = poPortrait
    HasDate = False
    HasIndex = False
    InvertFarsiDate = True
    TitleSort = True
    AutoInsert = False
    FooterFields = 'Count'
    Columns = <
      item
        Expanded = False
        FieldName = 'CustomerId'
        Title.Caption = #1603#1583' '#1605#1588#1578#1585#1610
        Width = 62
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'NickName'
        Title.Caption = #1606#1575#1605' '#1605#1588#1578#1585#1610
        Width = 265
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'productid'
        Title.Caption = #1603#1583' '#1605#1581#1589#1608#1604
        Width = 65
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ProductTitle'
        Title.Caption = #1606#1575#1605' '#1605#1581#1589#1608#1604
        Width = 131
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ContractFinish'
        Title.Caption = #1585#1608#1586' '#1578#1575' '#1662#1575#1610#1575#1606' '#1602#1585#1575#1585#1583#1575#1583
        Visible = True
      end>
  end
  object xpPanel1: TxpPanel [3]
    Left = 0
    Top = 49
    Width = 688
    Height = 72
    StartColor = 16511469
    EndColor = 16244694
    FillDirection = fdLeftToRight
    Title = #1601#1610#1604#1578#1585
    TitleFont.Charset = ARABIC_CHARSET
    TitleFont.Color = clNavy
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    TitleAlignment = taRightJustify
    TitleStartColor = clWhite
    TitleEndColor = 15777194
    TitleColor = clWhite
    TitleFillDirect = fdLeftToRight
    TitleImageAlign = tiaLeft
    TitleButtons = [tbMinimize]
    DefaultHeight = 72
    BorderColor = clWhite
    BGImageAlign = iaStretch
    Align = alTop
    TabOrder = 3
    DesignSize = (
      686
      40)
    object Label3: TLabel
      Left = 617
      Top = 12
      Width = 51
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1603#1583' '#1605#1588#1578#1585#1610
      Transparent = True
    end
    object Label4: TLabel
      Left = 494
      Top = 12
      Width = 34
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1605#1581#1589#1608#1604
      Transparent = True
    end
    object dblProduct: TDBLookupComboBox
      Left = 342
      Top = 8
      Width = 144
      Height = 21
      Anchors = [akTop, akRight]
      BiDiMode = bdLeftToRight
      KeyField = 'ProductID'
      ListField = 'ProductTitle'
      ListSource = Dm.DProduct
      ParentBiDiMode = False
      TabOrder = 0
      OnCloseUp = edtCustomerIdChange
      OnKeyUp = dblProductKeyUp
    end
    object edtCustomerId: TEdit
      Left = 536
      Top = 8
      Width = 73
      Height = 21
      Anchors = [akTop, akRight]
      TabOrder = 1
      OnChange = edtCustomerIdChange
    end
  end
  inherited DSForm: TDataSource
    DataSet = SpCustomerReport
  end
  inherited ActionList: TActionList
    Left = 48
    Top = 128
  end
  object SpCustomerReport: TADOStoredProc
    Connection = Dm.YeganehConnection
    CursorType = ctStatic
    ProcedureName = 'Customer_NDayReport'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
        Value = Null
      end
      item
        Name = '@Number'
        Attributes = [paNullable]
        DataType = ftInteger
        Precision = 10
        Value = Null
      end>
    Left = 184
    Top = 72
    object SpCustomerReportCustomerId: TIntegerField
      FieldName = 'CustomerId'
    end
    object SpCustomerReportNickName: TWideStringField
      FieldName = 'NickName'
      ReadOnly = True
      Size = 255
    end
    object SpCustomerReportProductTitle: TWideStringField
      FieldName = 'ProductTitle'
      Size = 100
    end
    object SpCustomerReportproductid: TWordField
      FieldName = 'productid'
    end
    object SpCustomerReportContractFinish: TIntegerField
      FieldName = 'ContractFinish'
      ReadOnly = True
    end
    object SpCustomerReportCustomerIdStr: TWideStringField
      FieldName = 'CustomerIdStr'
      ReadOnly = True
      Size = 30
    end
  end
end
