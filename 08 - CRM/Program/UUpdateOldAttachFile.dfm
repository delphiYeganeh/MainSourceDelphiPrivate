inherited frUpdateOldAttachFile: TfrUpdateOldAttachFile
  Left = 340
  Top = 165
  Width = 670
  Height = 399
  Caption = #1604#1610#1587#1578' '#1601#1575#1610#1604' '#1607#1575#1610' '#1602#1585#1575#1585' '#1583#1575#1583#1575' '#1607#1575' '#1608' '#1575#1602#1583#1575#1605#1575#1578
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel [0]
    Left = 0
    Top = 320
    Width = 662
    Height = 45
    Align = alBottom
    TabOrder = 0
    object BitBtn1: TBitBtn
      Left = 111
      Top = 6
      Width = 122
      Height = 32
      Caption = #1576#1585#1608#1586#1585#1587#1575#1606#1610' '#1601#1575#1610#1604' '#1607#1575
      TabOrder = 0
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object btnCancel: TBitBtn
      Left = 7
      Top = 6
      Width = 98
      Height = 32
      Cancel = True
      Caption = #1575#1606#1589#1585#1575#1601
      ModalResult = 2
      TabOrder = 1
      OnClick = btnCancelClick
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
    end
    object rdgUpdateType: TRadioGroup
      Left = 466
      Top = 2
      Width = 185
      Height = 39
      Caption = #1606#1608#1593' '#1576#1585#1608#1586#1585#1587#1575#1606#1610
      Columns = 2
      ItemIndex = 1
      Items.Strings = (
        #1607#1605#1607
        #1585#1603#1608#1585#1583' '#1575#1606#1578#1582#1575#1576#1610)
      TabOrder = 2
    end
    object btnShowContractWord: TBitBtn
      Left = 237
      Top = 5
      Width = 99
      Height = 33
      Caption = #1582#1585#1608#1580#1610' '#1604#1610#1587#1578
      TabOrder = 3
      OnClick = btnShowContractWordClick
      Glyph.Data = {
        
        54C9744EC36F47C46F46AA5B3000}
    end
  end
  object Panel2: TPanel [1]
    Left = 0
    Top = 0
    Width = 662
    Height = 41
    Align = alTop
    TabOrder = 1
    object Label1: TLabel
      Left = 330
      Top = 4
      Width = 317
      Height = 13
      Caption = #1575#1576#1578#1583#1575' '#1605#1587#1610#1585#1607#1575' '#1585#1575' '#1576#1585#1585#1587#1610' '#1603#1606#1610#1583' '#1583#1585#1589#1608#1585#1578' '#1589#1581#1578' '#1576#1585#1608#1586#1585#1587#1575#1606#1610' '#1585#1575' '#1575#1606#1580#1575#1605' '#1583#1607#1610#1583'  '
    end
    object Label9: TLabel
      Left = 289
      Top = 22
      Width = 362
      Height = 13
      Caption = 
        #1578#1608#1580#1607' :  '#1583#1585' '#1589#1608#1585#1578#1610' '#1603#1607' '#1578#1593#1583#1575#1583' '#1602#1585#1575#1585' '#1583#1575#1583' '#1576#1575#1604#1575' '#1576#1575#1588#1583' '#1575#1606#1580#1575#1605' '#1593#1605#1604#1610#1575#1578' '#1576#1607' '#1586#1605#1575 +
        #1606' '#1586#1610#1575#1583#1610' '#1606#1610#1575#1586' '#1583#1575#1585#1583' '
      Color = clGray
      Font.Charset = ARABIC_CHARSET
      Font.Color = clRed
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentColor = False
      ParentFont = False
      Transparent = True
    end
  end
  object pgcUpdate: TxpPageControl [2]
    Left = 0
    Top = 41
    Width = 662
    Height = 279
    ActivePage = xpTabSheet2
    Align = alClient
    ParentShowHint = False
    ShowHint = True
    Style = pcsXP
    TabHeight = 20
    TabOrder = 2
    TabPosition = tpTop
    BorderColor = clSilver
    TabTextAlignment = taCenter
    object xpTabSheet1: TxpTabSheet
      Caption = #1601#1575#1610#1604' '#1607#1575#1610' '#1575#1602#1583#1575#1605#1575#1578
      Color = clWhite
      BGStyle = bgsNone
      GradientStartColor = clWhite
      GradientEndColor = clSilver
      GradientFillDir = fdTopToBottom
      object dbgFollowUp: TYDBGrid
        Left = 0
        Top = 41
        Width = 654
        Height = 208
        Cursor = crHandPoint
        Align = alClient
        BiDiMode = bdLeftToRight
        ParentBiDiMode = False
        TabOrder = 0
        TitleFont.Charset = ARABIC_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        YeganehColor = False
        YeganehFinish = False
        PageNumber = False
        RecordNumber = False
        HasTime = False
        Oriention = poPortrait
        HasDate = False
        HasIndex = False
        InvertFarsiDate = True
        TitleSort = True
        AutoInsert = False
        FooterFields = 'Count'
        Columns = <
          item
            Expanded = False
            FieldName = 'CustomerId'
            ReadOnly = True
            Title.Alignment = taCenter
            Title.Caption = #1603#1583' '#1605#1588#1578#1585#1610
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ToDoDate'
            ReadOnly = True
            Title.Alignment = taCenter
            Title.Caption = #1578#1575#1585#1610#1582' '#1575#1602#1583#1575#1605
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'Attachment1'
            Title.Alignment = taCenter
            Title.Caption = #1605#1587#1610#1585' '#1608#1608#1585#1583
            Width = 240
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'Attachment2'
            Title.Alignment = taCenter
            Title.Caption = #1605#1587#1610#1585' '#1578#1589#1608#1610#1585
            Width = 243
            Visible = True
          end>
      end
      object Panel4: TPanel
        Left = 0
        Top = 0
        Width = 654
        Height = 41
        Align = alTop
        TabOrder = 1
        object Label3: TLabel
          Left = 345
          Top = 16
          Width = 70
          Height = 13
          Caption = #1588#1605#1575#1585#1607' '#1605#1588#1578#1585#1610
        end
        object Label4: TLabel
          Left = 581
          Top = 16
          Width = 58
          Height = 13
          Caption = #1578#1575#1585#1610#1582' '#1575#1602#1583#1575#1605' '#1575#1586
        end
        object Label5: TLabel
          Left = 498
          Top = 16
          Width = 6
          Height = 13
          Caption = #1578#1575
        end
        object EdtCustomerIdFollow: TEdit
          Left = 215
          Top = 12
          Width = 121
          Height = 21
          TabOrder = 0
          OnChange = EdtCustomerIdFollowChange
        end
        object edtFollowStart: TEdit
          Left = 512
          Top = 12
          Width = 64
          Height = 21
          TabOrder = 1
          OnExit = edtFollowStartExit
        end
        object edtFollowEnd: TEdit
          Left = 424
          Top = 12
          Width = 64
          Height = 21
          TabOrder = 2
          OnExit = edtFollowEndExit
        end
      end
    end
    object xpTabSheet2: TxpTabSheet
      Caption = #1601#1575#1610#1604' '#1607#1575#1610' '#1602#1585#1575#1585#1583#1575#1583#1607#1575
      Color = clWhite
      BGStyle = bgsNone
      GradientStartColor = clWhite
      GradientEndColor = clSilver
      GradientFillDir = fdTopToBottom
      object dbgContract: TYDBGrid
        Left = 0
        Top = 41
        Width = 654
        Height = 208
        Cursor = crHandPoint
        Align = alClient
        BiDiMode = bdLeftToRight
        ParentBiDiMode = False
        TabOrder = 0
        TitleFont.Charset = ARABIC_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        YeganehColor = False
        YeganehFinish = False
        PageNumber = False
        RecordNumber = False
        HasTime = False
        Oriention = poPortrait
        HasDate = False
        HasIndex = False
        InvertFarsiDate = True
        TitleSort = True
        AutoInsert = False
        FooterFields = 'Count'
        Columns = <
          item
            Expanded = False
            FieldName = 'CustomerID'
            ReadOnly = True
            Title.Alignment = taCenter
            Title.Caption = #1588#1605#1575#1585#1607' '#1605#1588#1578#1585#1610
            Width = 80
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'BeginDate'
            ReadOnly = True
            Title.Alignment = taCenter
            Title.Caption = #1578#1575#1585#1610#1582' '#1602#1585#1575#1585#1583#1575#1583
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'Attachment'
            Title.Alignment = taCenter
            Title.Caption = #1605#1587#1610#1585
            Width = 454
            Visible = True
          end>
      end
      object Panel3: TPanel
        Left = 0
        Top = 0
        Width = 654
        Height = 41
        Align = alTop
        TabOrder = 1
        object Label2: TLabel
          Left = 345
          Top = 16
          Width = 70
          Height = 13
          Caption = #1588#1605#1575#1585#1607' '#1605#1588#1578#1585#1610
        end
        object Label6: TLabel
          Left = 578
          Top = 16
          Width = 61
          Height = 13
          Caption = #1578#1575#1585#1610#1582' '#1602#1585#1575#1585#1583#1575#1583#1575#1586
        end
        object Label7: TLabel
          Left = 499
          Top = 16
          Width = 6
          Height = 13
          Caption = #1578#1575
        end
        object edtCustomerIdContract: TEdit
          Left = 223
          Top = 12
          Width = 121
          Height = 21
          TabOrder = 0
          OnChange = edtContractStartExit
        end
        object edtContractStart: TEdit
          Left = 512
          Top = 12
          Width = 64
          Height = 21
          TabOrder = 1
          OnExit = edtContractStartExit
        end
        object edtContractEnd: TEdit
          Left = 424
          Top = 12
          Width = 64
          Height = 21
          TabOrder = 2
          OnExit = edtContractStartExit
        end
      end
      object pnlProcess: TPanel
        Left = 240
        Top = 112
        Width = 185
        Height = 41
        Caption = #1583#1585#1581#1575#1604' '#1582#1608#1575#1606#1583#1606' '#1575#1586' '#1601#1575#1610#1604
        TabOrder = 2
        Visible = False
        object Label8: TLabel
          Left = 99
          Top = -1
          Width = 79
          Height = 13
          Caption = #1604#1591#1601#1575' '#1605#1606#1578#1592#1585' '#1576#1605#1575#1606#1610#1583
          Color = clBtnFace
          Font.Charset = ARABIC_CHARSET
          Font.Color = clRed
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentColor = False
          ParentFont = False
        end
        object Bevel1: TBevel
          Left = 1
          Top = 1
          Width = 183
          Height = 39
          Align = alClient
        end
      end
    end
  end
  inherited DSForm: TDataSource
    Left = 504
    Top = 240
  end
  inherited ActionList: TActionList
    Left = 504
    Top = 192
    inherited Aexit: TAction
      ShortCut = 0
    end
  end
end
