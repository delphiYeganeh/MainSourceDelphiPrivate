inherited UserDefineF: TUserDefineF
  Tag = 2
  Left = 751
  Top = 165
  BorderIcons = [biSystemMenu]
  BorderStyle = bsDialog
  Caption = #1705#1575#1585#1576#1585#1575#1606
  ClientHeight = 573
  ClientWidth = 552
  KeyPreview = True
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 13
  inherited ShapeBase: TShape
    Width = 552
    Height = 573
  end
  object pnlMain: TPanel [1]
    Left = 0
    Top = 0
    Width = 552
    Height = 573
    Align = alClient
    Caption = ' '
    TabOrder = 0
    object xpPanel1: TxpPanel
      Left = 1
      Top = 1
      Width = 550
      Height = 88
      StartColor = 16511469
      EndColor = 16244694
      FillDirection = fdLeftToRight
      Caption = 'xpPanel'
      Title = #1580#1587#1578#1580#1608
      TitleFont.Charset = ARABIC_CHARSET
      TitleFont.Color = clNavy
      TitleFont.Height = -11
      TitleFont.Name = 'Tahoma'
      TitleFont.Style = []
      TitleAlignment = taRightJustify
      TitleStartColor = clWhite
      TitleEndColor = 15777194
      TitleColor = clWhite
      TitleFillDirect = fdVerticalFromCenter
      TitleImageAlign = tiaLeft
      TitleButtons = [tbMinimize]
      DefaultHeight = 88
      BorderColor = clWhite
      BGImageAlign = iaStretch
      Align = alTop
      TabOrder = 0
      DesignSize = (
        548
        56)
      object Label1: TLabel
        Left = 472
        Top = 8
        Width = 45
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1606#1575#1605' '#1705#1575#1605#1604' :'
        Transparent = True
      end
      object Label3: TLabel
        Left = 305
        Top = 8
        Width = 50
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1606#1575#1605' '#1705#1575#1585#1576#1585#1610':'
        Transparent = True
      end
      object Label6: TLabel
        Left = 141
        Top = 8
        Width = 42
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1606#1608#1593' '#1705#1575#1585#1576#1585':'
        Transparent = True
      end
      object SpeedButton1: TSpeedButton
        Left = 525
        Top = 3
        Width = 26
        Height = 24
        Hint = #1576#1575#1586#1711#1588#1578' '#1576#1607' '#1581#1575#1604#1578' '#1575#1608#1604#1610#1607
        Anchors = [akTop, akRight]
        Flat = True
        Glyph.Data = {
          
          D300C8D0D3003A01B1003A01B100545A4B00DAE3EB00C1C9CC00}
        ParentShowHint = False
        ShowHint = True
        OnClick = SpeedButton1Click
      end
      object edtUserName: TEdit
        Left = 187
        Top = 4
        Width = 115
        Height = 21
        TabOrder = 1
        OnChange = edtFullNameChange
      end
      object dblUserTypesrch: TDBLookupComboBox
        Left = 6
        Top = 4
        Width = 135
        Height = 21
        Anchors = [akTop, akRight]
        BiDiMode = bdLeftToRight
        ParentBiDiMode = False
        TabOrder = 2
        OnCloseUp = edtFullNameChange
      end
      object edtFullName: TEdit
        Left = 361
        Top = 4
        Width = 111
        Height = 21
        TabOrder = 0
        OnChange = edtFullNameChange
      end
    end
    object Panel2: TPanel
      Left = 1
      Top = 304
      Width = 550
      Height = 228
      Align = alBottom
      Color = 16244694
      TabOrder = 1
      DesignSize = (
        550
        228)
      object Label4: TLabel
        Left = 489
        Top = 41
        Width = 49
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1705#1604#1605#1607' '#1593#1576#1608#1585':'
      end
      object Label10: TLabel
        Left = 258
        Top = 41
        Width = 49
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1578#1705#1585#1575#1585'        :'
      end
      object Label2: TLabel
        Left = 489
        Top = 89
        Width = 55
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1603#1583' '#1576#1575#1586#1575#1585#1610#1575#1576#1610':'
      end
      object Label5: TLabel
        Left = 449
        Top = 112
        Width = 90
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1606#1608#1593' '#1705#1575#1585#1576#1585'                :'
      end
      object Label7: TLabel
        Left = 490
        Top = 17
        Width = 48
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1606#1575#1605' '#1705#1575#1605#1604'  :'
      end
      object Label8: TLabel
        Left = 258
        Top = 17
        Width = 50
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1606#1575#1605' '#1705#1575#1585#1576#1585#1610':'
      end
      object Label9: TLabel
        Left = 450
        Top = 65
        Width = 90
        Height = 13
        Anchors = [akTop, akRight]
        Caption = #1587#1591#1581' '#1583#1587#1578#1585#1587#1610'   :'
      end
      object Pass: TDBEdit
        Left = 354
        Top = 37
        Width = 135
        Height = 21
        Anchors = [akTop, akRight]
        DataField = 'NewPassWord'
        DataSource = DSForm
        PasswordChar = '*'
        TabOrder = 2
      end
      object ConfirmPass: TEdit
        Left = 122
        Top = 37
        Width = 135
        Height = 21
        Anchors = [akTop, akRight]
        PasswordChar = '*'
        TabOrder = 3
      end
      object DBLookupComboBox2: TDBLookupComboBox
        Left = 177
        Top = 85
        Width = 273
        Height = 21
        Anchors = [akTop, akRight]
        BiDiMode = bdLeftToRight
        DataField = 'MarketerID'
        DataSource = DSForm
        KeyField = 'MarketerID'
        ListField = 'MarketerTitle'
        ListSource = Dm.DMarketerALL
        ParentBiDiMode = False
        TabOrder = 6
      end
      object DBEdit3: TDBEdit
        Left = 454
        Top = 85
        Width = 35
        Height = 21
        Anchors = [akTop, akRight]
        DataField = 'MarketerID'
        DataSource = DSForm
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        TabOrder = 16
        OnEnter = TEditEnter
        OnExit = TEditExit
      end
      object dblUserType: TDBLookupComboBox
        Left = 176
        Top = 108
        Width = 273
        Height = 21
        Anchors = [akTop, akRight]
        BiDiMode = bdLeftToRight
        DataField = 'UserTypeId'
        DataSource = DSForm
        KeyField = 'UserTypeId'
        ListField = 'UserTypeTitle'
        ParentBiDiMode = False
        TabOrder = 7
        OnKeyDown = dblUserTypeKeyDown
      end
      object dbckIsActive: TDBCheckBox
        Left = 32
        Top = 18
        Width = 73
        Height = 17
        Caption = #1705#1575#1585#1576#1585' '#1601#1593#1575#1604
        DataField = 'IsActive'
        DataSource = DSForm
        TabOrder = 4
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object dbchPrintCustomerList: TDBCheckBox
        Left = 192
        Top = 153
        Width = 160
        Height = 17
        Anchors = [akTop, akRight]
        Caption = #1575#1610#1580#1575#1583' '#1582#1585#1608#1580#1610' '#1604#1610#1587#1578' '#1605#1588#1578#1585#1610#1575#1606
        DataField = 'ExportCustomerList'
        DataSource = DSForm
        TabOrder = 12
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox2: TDBCheckBox
        Left = 416
        Top = 135
        Width = 128
        Height = 17
        Anchors = [akTop, akRight]
        Caption = #1576#1578#1608#1575#1606#1583' '#1602#1585#1575#1585#1583#1575#1583#1607#1575' '#1585#1575' '#1576#1576#1610#1606#1583
        DataField = 'ShowContract'
        DataSource = DSForm
        TabOrder = 8
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox3: TDBCheckBox
        Left = 376
        Top = 153
        Width = 168
        Height = 17
        Anchors = [akTop, akRight]
        Caption = #1576#1578#1608#1575#1606#1583' '#1602#1585#1575#1585#1583#1575#1583' '#1608#1610#1585#1575#1610#1588'/'#1581#1584#1601' '#1705#1606#1583
        DataField = 'EditContract'
        DataSource = DSForm
        TabOrder = 9
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox4: TDBCheckBox
        Left = 18
        Top = 153
        Width = 150
        Height = 17
        Anchors = [akTop, akRight]
        Caption = #1605#1588#1575#1607#1583#1607' '#1578#1605#1575#1605' '#1605#1588#1578#1585#1610#1575#1606
        DataField = 'ShowAllCustomer'
        DataSource = DSForm
        TabOrder = 15
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox1: TDBCheckBox
        Left = 223
        Top = 135
        Width = 128
        Height = 17
        Anchors = [akTop, akRight]
        Caption = #1575#1610#1580#1575#1583' '#1582#1585#1608#1580#1610' '#1575#1602#1583#1575#1605#1575#1578
        DataField = 'ExportFollow'
        DataSource = DSForm
        TabOrder = 11
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox5: TDBCheckBox
        Left = 18
        Top = 137
        Width = 150
        Height = 17
        Anchors = [akTop, akRight]
        Caption = #1605#1588#1575#1607#1583#1607' '#1603#1575#1585#1607#1575#1610' '#1605#1606
        DataField = 'ShowMyAction'
        DataSource = DSForm
        TabOrder = 14
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox6: TDBCheckBox
        Left = 394
        Top = 169
        Width = 150
        Height = 17
        Anchors = [akTop, akRight]
        Caption = #1578#1605#1575#1605#1610' '#1575#1602#1583#1575#1605#1575#1578
        DataField = 'BtnAllAct'
        DataSource = DSForm
        TabOrder = 10
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox7: TDBCheckBox
        Left = 168
        Top = 169
        Width = 184
        Height = 17
        Anchors = [akTop, akRight]
        Caption = #1578#1605#1575#1605#1610' '#1602#1585#1575#1585#1583#1575#1583' '#1607#1575#1610' '#1578#1605#1575#1605#1610' '#1603#1575#1585#1576#1585#1575#1606
        DataField = 'BtnAllContract'
        DataSource = DSForm
        TabOrder = 13
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBEdit1: TDBEdit
        Left = 354
        Top = 13
        Width = 135
        Height = 21
        Anchors = [akTop, akRight]
        DataField = 'Title'
        DataSource = DSForm
        TabOrder = 0
      end
      object DBEdit2: TDBEdit
        Left = 122
        Top = 13
        Width = 135
        Height = 21
        Anchors = [akTop, akRight]
        DataField = 'UserName'
        DataSource = DSForm
        TabOrder = 1
      end
      object DBLookupComboBox1: TDBLookupComboBox
        Left = 177
        Top = 61
        Width = 273
        Height = 21
        Anchors = [akTop, akRight]
        BiDiMode = bdLeftToRight
        DataField = 'AccessTitle'
        DataSource = DSForm
        ParentBiDiMode = False
        TabOrder = 5
      end
      object DBCheckBox8: TDBCheckBox
        Left = 40
        Top = 170
        Width = 128
        Height = 17
        Anchors = [akTop, akRight]
        Caption = #1583#1575#1585#1575#1610' '#1608#1610#1606#1583#1608#1586' '#1587#1585#1608#1585
        DataField = 'ISWinServer'
        DataSource = DSForm
        TabOrder = 17
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox9: TDBCheckBox
        Left = 9
        Top = 38
        Width = 97
        Height = 17
        Caption = #1605#1583#1610#1585' '#1587#1610#1587#1578#1605
        DataField = 'IsAdmin'
        DataSource = DSForm
        TabOrder = 18
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox10: TDBCheckBox
        Left = 394
        Top = 186
        Width = 150
        Height = 17
        Anchors = [akTop, akRight]
        Caption = #1583#1587#1578#1585#1587#1610' '#1605#1606#1608' '#1605#1575#1604#1610
        DataField = 'AccAcess'
        DataSource = DSForm
        TabOrder = 19
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox11: TDBCheckBox
        Left = 169
        Top = 186
        Width = 184
        Height = 17
        Anchors = [akTop, akRight]
        Caption = #1583#1587#1578#1585#1587#1610' '#1575#1585#1586#1610#1575#1576#1610
        DataField = 'AssesstAccess'
        DataSource = DSForm
        TabOrder = 20
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox12: TDBCheckBox
        Left = 39
        Top = 187
        Width = 128
        Height = 17
        Anchors = [akTop, akRight]
        Caption = #1583#1587#1578#1585#1587#1610' '#1601#1575#1705#1578#1608#1585
        DataField = 'FactorAccess'
        DataSource = DSForm
        TabOrder = 21
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox13: TDBCheckBox
        Left = 394
        Top = 204
        Width = 150
        Height = 17
        Anchors = [akTop, akRight]
        Caption = #1583#1587#1578#1585#1587#1610' '#1578#1605#1575#1587
        DataField = 'CallAccess'
        DataSource = DSForm
        TabOrder = 22
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox14: TDBCheckBox
        Left = 18
        Top = 65
        Width = 150
        Height = 17
        Anchors = [akTop, akRight]
        Caption = #1605#1583#1610#1585' '#1601#1585#1608#1588
        DataField = 'ManagerSale'
        DataSource = DSForm
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 23
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox15: TDBCheckBox
        Left = 18
        Top = 81
        Width = 150
        Height = 17
        Anchors = [akTop, akRight]
        Caption = #1605#1583#1610#1585' '#1578#1608#1604#1610#1583
        DataField = 'ManagerProduction'
        DataSource = DSForm
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 24
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox16: TDBCheckBox
        Left = 18
        Top = 96
        Width = 150
        Height = 17
        Anchors = [akTop, akRight]
        Caption = #1605#1583#1610#1585' '#1662#1588#1578#1610#1576#1575#1606#1610
        DataField = 'ManagerSupport'
        DataSource = DSForm
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 25
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox17: TDBCheckBox
        Left = 18
        Top = 111
        Width = 150
        Height = 17
        Anchors = [akTop, akRight]
        Caption = #1605#1583#1610#1585' '#1575#1583#1575#1585#1610
        DataField = 'ManagerOfficial'
        DataSource = DSForm
        Font.Charset = ARABIC_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 26
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
    end
    object Panel1: TPanel
      Left = 1
      Top = 532
      Width = 550
      Height = 40
      Align = alBottom
      Color = 16244694
      TabOrder = 2
      DesignSize = (
        550
        40)
      object btnCancel: TBitBtn
        Left = 9
        Top = 8
        Width = 80
        Height = 28
        Anchors = [akTop]
        Caption = #1575#1606#1589#1585#1575#1601
        TabOrder = 0
        Kind = bkCancel
      end
      object btnDel: TBitBtn
        Left = 174
        Top = 8
        Width = 69
        Height = 28
        Anchors = [akTop]
        Caption = #1581#1584#1601
        TabOrder = 1
        OnClick = btnDelClick
        Glyph.Data = {
          
          9BED9B9BED9B9BEE9C9CEF9C9CEF9F9FEEA1A1EACCCCF1000000}
      end
      object btnSave: TBitBtn
        Left = 95
        Top = 8
        Width = 75
        Height = 28
        Anchors = [akTop]
        Caption = #1579#1576#1578
        Default = True
        TabOrder = 2
        OnClick = btnSaveClick
        Glyph.Data = {
          
          0000}
        NumGlyphs = 2
      end
      object btnAdd: TBitBtn
        Left = 245
        Top = 8
        Width = 75
        Height = 28
        Caption = #1575#1601#1586#1608#1583#1606
        TabOrder = 3
        OnClick = BitBtn6Click
        Glyph.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      end
    end
    object dbgUserList: TYDBGrid
      Left = 1
      Top = 89
      Width = 550
      Height = 215
      Align = alClient
      DataSource = DSForm
      Options = [dgAlwaysShowEditor, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
      TabOrder = 3
      TitleFont.Charset = ARABIC_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Tahoma'
      TitleFont.Style = []
      OnKeyDown = GotoNextGrid
      YeganehColor = False
      YeganehFinish = False
      PageNumber = False
      RecordNumber = False
      HasTime = False
      Oriention = poPortrait
      HasDate = False
      HasIndex = False
      InvertFarsiDate = True
      TitleSort = True
      AutoInsert = False
      FooterFields = 'Count'
      Columns = <
        item
          Expanded = False
          FieldName = 'Title'
          Title.Caption = #1606#1575#1605' '#1705#1575#1605#1604
          Width = 143
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'UserName'
          Title.Caption = #1606#1575#1605' '#1705#1575#1585#1576#1585#1610
          Width = 103
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'AccessTitle'
          Title.Caption = #1587#1591#1581' '#1583#1587#1578#1585#1587#1610
          Width = 208
          Visible = True
        end>
    end
  end
  inherited DSForm: TDataSource
    DataSet = User
    OnDataChange = DSFormDataChange
    Left = 51
    Top = 184
  end
  inherited ActionList: TActionList
    Left = 136
    Top = 184
  end
  object User: TADOTable
    Connection = Dm.YeganehConnection
    CursorType = ctStatic
    TableName = 'Users'
    Left = 49
    Top = 101
    object UserId: TIntegerField
      FieldName = 'Id'
    end
    object UserTitle: TWideStringField
      Alignment = taRightJustify
      FieldName = 'Title'
      Size = 50
    end
    object UserUserName: TWideStringField
      Alignment = taRightJustify
      FieldName = 'UserName'
      Size = 50
    end
    object UserPassWord: TWideStringField
      Alignment = taRightJustify
      FieldName = 'PassWord'
      Size = 50
    end
    object UserMarketerID: TWordField
      FieldName = 'MarketerID'
    end
    object UserDefaultProductID: TWordField
      FieldName = 'DefaultProductID'
    end
    object UserEmail: TStringField
      Alignment = taRightJustify
      FieldName = 'Email'
      Size = 50
    end
    object UserAccountNo: TStringField
      Alignment = taRightJustify
      FieldName = 'AccountNo'
      Size = 50
    end
    object UserAccessID: TIntegerField
      FieldName = 'AccessID'
    end
    object UserIsActive: TBooleanField
      Alignment = taRightJustify
      FieldName = 'IsActive'
    end
    object UserIsAdmin: TBooleanField
      FieldName = 'IsAdmin'
    end
    object UserUserTypeId: TWordField
      FieldName = 'UserTypeId'
    end
    object UserAccessTitle: TWideStringField
      FieldKind = fkLookup
      FieldName = 'AccessTitle'
      LookupDataSet = Dm.Accesses
      LookupKeyFields = 'ID'
      LookupResultField = 'Title'
      KeyFields = 'AccessID'
      Size = 1000
      Lookup = True
    end
    object UserShowContract: TBooleanField
      FieldName = 'ShowContract'
    end
    object UserEditContract: TBooleanField
      FieldName = 'EditContract'
    end
    object UserShowAllCustomer: TBooleanField
      FieldName = 'ShowAllCustomer'
    end
    object UserExportCustomerList: TBooleanField
      FieldName = 'ExportCustomerList'
    end
    object UserExportFollow: TBooleanField
      FieldName = 'ExportFollow'
    end
    object UserShowMyAction: TBooleanField
      FieldName = 'ShowMyAction'
    end
    object UserBtnAllContract: TBooleanField
      FieldName = 'BtnAllContract'
    end
    object UserBtnAllAct: TBooleanField
      FieldName = 'BtnAllAct'
    end
    object UserISWinServer: TBooleanField
      FieldName = 'ISWinServer'
    end
    object UserAccAcess: TBooleanField
      FieldName = 'AccAcess'
    end
    object UserCallAccess: TBooleanField
      FieldName = 'CallAccess'
    end
    object UserAssesstAccess: TBooleanField
      FieldName = 'AssesstAccess'
    end
    object UserFactorAccess: TBooleanField
      FieldName = 'FactorAccess'
    end
    object UserManagerSale: TBooleanField
      FieldName = 'ManagerSale'
    end
    object UserManagerProduction: TBooleanField
      FieldName = 'ManagerProduction'
    end
    object UserManagerSupport: TBooleanField
      FieldName = 'ManagerSupport'
    end
    object UserManagerOfficial: TBooleanField
      FieldName = 'ManagerOfficial'
    end
    object UserIsSystemUser: TBooleanField
      FieldName = 'IsSystemUser'
    end
    object UserNewPassWord: TStringField
      FieldName = 'NewPassWord'
      Size = 50
    end
  end
end
