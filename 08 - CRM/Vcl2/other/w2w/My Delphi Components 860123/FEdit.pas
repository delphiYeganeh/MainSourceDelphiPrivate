...
uses
  ...
  StdCtrls, DBCtrls, MyLibU;

type
  TFarsiEdit = class(TEdit)
  private
...
Procedure TFarsiEdit.SetFarsi(Val :Boolean);
Begin
  FFarsi := Val;
End;

Procedure TFarsiEdit.SetInpAlpha(Val :Boolean);
Begin
  FInputAlpha := Val;
End;

Procedure TFarsiEdit.SetInpDigit(Val :Boolean);
Begin
  FInputDigit := Val;
End;

Procedure TFarsiEdit.SetFocusedColor(CVal :TColor);
Begin
  FFocusedColor := CVal;
End;

procedure TFarsiEdit.DoEnter;
Begin
  inherited DoEnter;
  FirstColor:= Color;
  Color:= FocusedColor;
...
procedure TFarsiEdit.DoExit;
Begin
  inherited DoExit;
  Color:= FirstColor;
End;

procedure TFarsiEdit.KeyPress(var Key: Char);
var WFlag:Boolean;
...
constructor TFarsiEdit.Create(AOwner: TComponent);
begin
...
procedure TFarsiEdit.DefaultHandler(var Message);
begin
...
procedure Register;
begin
  RegisterComponents('Farsi', [TFarsiEdit]);
end;
