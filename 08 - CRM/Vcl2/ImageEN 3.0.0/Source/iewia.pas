...
<FM>Declaration<FC>
function GetItemProperty( PropId:TPropID; item:<A TIEWiaItem> ):Variant;
...
Xres := ImageEnView1.IO.WiaParams.GetItemProperty(WIA_IPS_XRES,nil) );
!!}
// if not connected, connect to first device
...
begin
  ...
  try
    ...
    else if (iewaList in attrib) and ((vPropVar.vt and VT_VECTOR) <> 0) and ((vPropVar.vt and VT_UI4) <> 0) then
    begin
      // list (integers)
      if vPropVar.caul.pElems <> nil then
      begin
    ...
    else if (iewaList in attrib) and ((vPropVar.vt and VT_VECTOR) <> 0) and ((vPropVar.vt and VT_CLSID) <> 0) then
    begin
      // list (GUIDs)
    ...
    else if (iewaFlag in attrib) and ((vPropVar.vt and VT_VECTOR) <> 0) and ((vPropVar.vt and VT_UI4) <> 0) then
    begin
...
<FM>Declaration<FC>
function TIEWia.SetItemProperty(PropId: dword; val: integer; item: <A TIEWiaItem>): boolean;
...
ImageEnView1.IO.WiaParams.SetItemProperty(WIA_IPS_XRES, 150 ,nil);
!!}
// This is important starting at Delphi 6 because it requires typed constants otherwise the variant in invalid!
...
<FM>Declaration<FC>
function SetItemPropertyVariant(PropId: dword; val: Variant; item: <A TIEWiaItem>): boolean;
...
begin
  ...
  if fIWiaDevMgr = nil then
    exit;
  ConnectToDefault;
  if (not GetDefaultItem(item,false)) and ((fTakePicture=false) or (fRoot=nil)) then  // 2.2.9
    exit;
  if MultiPage then
  begin
...
begin
  ...
  else if comparemem(@header.guidFormatID, @WiaImgFmt_BMP, sizeof(TGuid)) then
  begin
    // BMP
    BMPReadStream(iemem, ProcessingBitmap, 0, tmpparams, pr, false, false, tempAlphaChannel,true);
  end
  else if comparemem(@header.guidFormatID, @WiaImgFmt_JPEG, sizeof(TGuid)) then
  begin
    // JPEG
    ReadJpegStream(iemem, nil, ProcessingBitmap, tmpparams, pr, false, false, true, false, true,true,-1,tmpparams.IsNativePixelFormat);
    FullParamsAssign:=true; // this allows to load IPTC and EXIF
  end;
  if fSaveTransferBufferAs<>'' then
    iemem.SaveToFile(fSaveTransferBufferAs);
...
begin
  ...
  begin
    result := string(WIA.GetItemProperty(WIA_IPA_ITEM_NAME, item)) + ' ( ';
...
procedure AddChildsOf(WIA:TIEWIA; Items:TTreeNodes; ItemsParent: TIEWiaItem; TreeParent: TTreeNode; IncludeDescription:boolean);
var
  i: integer;
  node: TTreeNode;
begin
  if assigned(ItemsParent) then
    ...
    begin
      ...
      begin
        ...
        AddChildsOf(WIA, Items, ItemsParent.Children[i], node,IncludeDescription);
      end;
    end;
end;
...
begin
  ...
  begin
    ...
    node.Data := Device;
    AddChildsOf(self,Items,Device, node,IncludeDescription);
  end;
end;
...
begin
  ...
  begin
    ...
    else if comparemem(pEventGUID,@WIA_EVENT_ITEM_CREATED,sizeof(TGuid)) then
    begin
      (WIA as TIEWia).fLastEvent:=ieeItemCreated;
      (WIA as TIEWia).DoItemsTreeUpdated;
    end
    else if comparemem(pEventGUID,@WIA_EVENT_ITEM_DELETED,sizeof(TGuid)) then
    begin
