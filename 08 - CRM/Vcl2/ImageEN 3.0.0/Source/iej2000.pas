  ...
  case IOParams.J2000_ColorSpace of
    ...
    ioJ2000_RGB:
      begin
        ...
        if HasAlpha then
          jas_image_addcmpt(im, 3, @cmptparams);
        ...
        a:=nil;
        if HasAlpha then
          iejas_image_setcmpttype(im, 3, JAS_IMAGE_CT_COLOR(JAS_IMAGE_CT_OPACITY));
        for y := 0 to hh - 1 do
        begin
          row := Bitmap.Scanline[y];
          if HasAlpha then
          ...
          begin
            ...
            if HasAlpha then
            begin
              jas_image_writecmptsample(im, 3, x, y, trunc(a^ / 255 * colors));
              inc(a);
            end;
            inc(row);
          end;
