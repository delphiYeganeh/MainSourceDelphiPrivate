object Frmain: TFrmain
  Left = 330
  Top = 136
  Width = 691
  Height = 599
  BiDiMode = bdRightToLeft
  BorderIcons = [biSystemMenu]
  Caption = #1575#1606#1578#1602#1575#1604' '#1575#1591#1604#1575#1593#1575#1578' '#1576#1610#1606' '#1583#1576#1610#1585#1582#1575#1606#1607' '#1607#1575
  Color = clBtnFace
  Font.Charset = ARABIC_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  ParentBiDiMode = False
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object Number: TLabel
    Left = 8
    Top = 265
    Width = 6
    Height = 13
    Caption = '  '
  end
  object Label7: TLabel
    Left = 555
    Top = 496
    Width = 121
    Height = 13
    Caption = #1606#1575#1605#1607' '#1607#1575#1610' '#1579#1576#1578' '#1588#1583#1607' '#1575#1586' '#1578#1575#1585#1610#1582
  end
  object Label1: TLabel
    Left = 447
    Top = 495
    Width = 27
    Height = 13
    Caption = #1576#1607' '#1576#1593#1583
  end
  object FromDate: TShamsiDateEdit
    Left = 478
    Top = 493
    Width = 72
    Height = 21
    TabOrder = 0
    Text = #0
  end
  object ProgressBar1: TProgressBar
    Left = 0
    Top = 549
    Width = 683
    Height = 16
    Align = alBottom
    TabOrder = 1
    Visible = False
  end
  object YDBGrid2: TYDBGrid
    Left = 0
    Top = 0
    Width = 683
    Height = 165
    Cursor = crHandPoint
    Align = alTop
    Color = 16773345
    DataSource = DTransfer_Sent_RelatedSecs
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
    ReadOnly = True
    TabOrder = 2
    TitleFont.Charset = ARABIC_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    YeganehColor = True
    YeganehFinish = False
    PageNumber = False
    RecordNumber = False
    HasTime = False
    Oriention = poPortrait
    HasDate = False
    HasIndex = False
    PrintTitle = #1604#1610#1587#1578' '#1578#1607#1610#1607' '#1588#1583#1607' '#1578#1608#1587#1591' '#1606#1585#1605' '#1575#1601#1586#1575#1585' '#1610#1711#1575#1606#1607
    InvertFarsiDate = True
    TitleSort = True
    AutoInsert = False
    FooterFields = 'Count'
    Columns = <
      item
        Expanded = False
        FieldName = 'RelatedSecretariatTitle'
        Title.Caption = #1606#1575#1605' '#1583#1576#1610#1585#1582#1575#1606#1607
        Width = 158
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DestinationServerIP'
        Title.Caption = #1570#1583#1585#1587' '#1587#1585#1608#1585
        Width = 76
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DestinationServerDB'
        Title.Caption = #1606#1575#1605' '#1662#1575#1610#1711#1575#1607' '#1575#1591#1604#1575#1593#1575#1578#1610
        Width = 90
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DestinationSecID'
        Title.Caption = #1705#1583' '#1583#1576#1610#1585#1582#1575#1606#1607' '#1605#1602#1589#1583
        Width = 88
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Number_of_letters'
        Title.Caption = #1578#1593#1583#1575#1583' '#1606#1575#1605#1607' '#1570#1605#1575#1583#1607' '#1575#1606#1578#1602#1575#1604
        Width = 105
        Visible = True
      end>
  end
  object YDBGrid1: TYDBGrid
    Left = 0
    Top = 165
    Width = 683
    Height = 165
    Align = alTop
    Color = 16773345
    DataSource = Transfer_dm.DTransfer_Get_SentLetter
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
    ReadOnly = True
    TabOrder = 3
    TitleFont.Charset = ARABIC_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
    YeganehColor = True
    YeganehFinish = False
    PageNumber = False
    RecordNumber = False
    HasTime = False
    Oriention = poPortrait
    HasDate = False
    HasIndex = False
    PrintTitle = #1604#1610#1587#1578' '#1578#1607#1610#1607' '#1588#1583#1607' '#1578#1608#1587#1591' '#1606#1585#1605' '#1575#1601#1586#1575#1585' '#1610#1711#1575#1606#1607
    InvertFarsiDate = True
    TitleSort = True
    AutoInsert = False
    FooterFields = 'Count'
    Columns = <
      item
        Expanded = False
        FieldName = 'IndicatorID'
        Title.Caption = #1575#1606#1583#1610#1603#1575#1578#1608#1585
        Width = 61
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Memo'
        Title.Caption = #1605#1590#1605#1608#1606' '#1606#1575#1605#1607
        Width = 223
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'RegistrationDate'
        Title.Caption = #1578#1575#1585#1610#1582' '#1579#1576#1578
        Width = 100
        Visible = True
      end>
  end
  object Transfer: TButton
    Left = 93
    Top = 490
    Width = 76
    Height = 25
    Caption = #1578#1608#1602#1601' '#1575#1606#1578#1602#1575#1604
    TabOrder = 4
    OnClick = TransferClick
  end
  object ProgressBar2: TProgressBar
    Left = 0
    Top = 533
    Width = 683
    Height = 16
    Align = alBottom
    TabOrder = 5
    Visible = False
  end
  object Button1: TButton
    Left = 184
    Top = 490
    Width = 76
    Height = 25
    Caption = #1575#1580#1585#1575
    TabOrder = 6
    OnClick = Button1Click
  end
  object ErrorLog: TMemo
    Left = 0
    Top = 330
    Width = 683
    Height = 156
    Align = alTop
    BiDiMode = bdLeftToRight
    Lines.Strings = (
      'Errors:')
    ParentBiDiMode = False
    TabOrder = 7
  end
  object Button2: TButton
    Left = 6
    Top = 490
    Width = 83
    Height = 25
    Caption = #1584#1582#1610#1585#1607' '#1582#1591#1575#1607#1575
    TabOrder = 8
    OnClick = Button2Click
  end
  object Timer: TTimer
    Enabled = False
    Interval = 600000
    OnTimer = TimerTimer
    Left = 80
    Top = 112
  end
  object Transfer_Sent_RelatedSecs: TADOStoredProc
    Connection = Transfer_dm.ADOConnection1
    CursorType = ctStatic
    ProcedureName = 'Transfer_Sent_RelatedSecs;1'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
        Value = 0
      end>
    Left = 432
    Top = 216
    object Transfer_Sent_RelatedSecsRelatedSecretariatID: TAutoIncField
      FieldName = 'RelatedSecretariatID'
      ReadOnly = True
    end
    object Transfer_Sent_RelatedSecsRelatedSecretariatTitle: TWideStringField
      FieldName = 'RelatedSecretariatTitle'
      Size = 50
    end
    object Transfer_Sent_RelatedSecsDestinationOrgID_inSource: TIntegerField
      FieldName = 'DestinationOrgID_inSource'
    end
    object Transfer_Sent_RelatedSecsDestinationSecID: TIntegerField
      FieldName = 'DestinationSecID'
    end
    object Transfer_Sent_RelatedSecsDestinationServerIP: TStringField
      DisplayWidth = 30
      FieldName = 'DestinationServerIP'
      Size = 30
    end
    object Transfer_Sent_RelatedSecsDestinationServerDB: TStringField
      FieldName = 'DestinationServerDB'
      Size = 50
    end
    object Transfer_Sent_RelatedSecsNumber_of_letters: TIntegerField
      FieldName = 'Number_of_letters'
      ReadOnly = True
    end
  end
  object DTransfer_Sent_RelatedSecs: TDataSource
    DataSet = Transfer_Sent_RelatedSecs
    Left = 424
    Top = 264
  end
  object Tray: TPopupMenu
    Left = 176
    Top = 96
    object N1: TMenuItem
      Caption = #1588#1585#1608#1593' '#1575#1606#1578#1602#1575#1604
      OnClick = N1Click
    end
    object N2: TMenuItem
      Caption = #1578#1608#1602#1601' '#1575#1606#1578#1602#1575#1604
      OnClick = N2Click
    end
    object Show1: TMenuItem
      Bitmap.Data = {
        
        FFFFFFFFFFFFFFFFFF00}
      Caption = #1606#1605#1575#1610#1588' '#1608#1590#1593#1610#1578
      Default = True
      OnClick = Show1Click
    end
    object Exit1: TMenuItem
      Caption = #1578#1608#1602#1601' '#1608' '#1582#1585#1608#1580
      OnClick = Exit1Click
    end
  end
  object CiaTray1: TCiaTray
    PopupMenu = Tray
    Version = '1.00'
    Left = 8
    Top = 8
  end
  object SaveDialog: TSaveDialog
    Filter = 'text files|*.txt'
    Left = 128
    Top = 192
  end
end
