...
uses
  ...
  Menus,idglobal,IdCoderHeader, YeganehCheckMail, TntDialogs;
...
files:TStrings;
 SendHost :String;
   Username,Password,Subject_a,FromAddress_a,RecipientAddress_a,AttachFileName_a,SendHost_a :String;
   EmailBody_a:TStrings;
...
begin
 ...
 if DMEmails.SpEmails.State in [dsEdit,dsInsert] then
  ...
  Subject_a  :=  Trim(DMEmails.QrSaveEmailsSubject.AsString) ;
   ...
   if GetUserSetting('EmailAutenticationType') then at:=atLogin else at:=atNone;
