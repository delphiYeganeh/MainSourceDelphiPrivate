...
uses
  ...
  xpBitBtn, XPStyleActnCtrls, YDbgrid, ShowRouneveshtUnit, ImgList , U_SEE_PAYVAST_ERJA_FRM,word2000;
...
type
  TReCommite = class(TMBaseForm)
    ...
    Label2: TLabel;
    DBText2: TDBText;
    adoViewdateRecommite: TADOQuery;
    dsViewdateRecommite: TDataSource;
    adoViewdateRecommiteviewdate: TStringField;
...
var
   SelectedOrg, ParentID : integer; // Amin 1391/12/08
begin
   inherited;
...
begin
  inherited;
//   ShowMessage(IntToStr(QrRecommites.fieldbyname('recommiteid').AsInteger));
{   ShowMessage(dm.Select_LetterReCommiteRecommiteID.AsString);
   Abort;
   Exit;}
...
begin
   ...
   begin
      ...
      //---
      adoViewdateRecommite.Close;
      adoViewdateRecommite.Parameters.ParamByName('RecommiteID').Value := Dm.Select_LetterReCommiteRecommiteID.AsInteger;
      adoViewdateRecommite.Open;
   end;
...
VAR
SEE_PAYVASET : TSEE_PAYVAST_ERJA_FRM;
begin
{
  ...
  SEE_PAYVASET                := TSEE_PAYVAST_ERJA_FRM.Create(Self);
  SEE_PAYVASET.LETTERID       := IntToStr(Dm.Select_LetterReCommiteLetterID.AsInteger);
  SEE_PAYVASET.RecommitID     := IntToStr(Dm.Select_LetterReCommiteRecommiteID.AsInteger);
  SEE_PAYVASET.Letter_Or_Erja := 'Erja';
  SEE_PAYVASET.ShowModal;
  SEE_PAYVASET.FREE;
...
begin
    ...
    if (AaddLetterData.Visible) then AaddLetterData.Enabled:=False;
    if (BitBtn2.Visible) then BitBtn2.Enabled:=False;
end;
...
begin
    ...
    if (AaddLetterData.Visible) then AaddLetterData.Enabled:=True;
    if (BitBtn2.Visible) then BitBtn2.Enabled:=True;
end;
