...
{      with DM.Get_RecommitePayvast_by_RecommitePayvastID do
      begin
        Close;
        Parameters.ParamByName('@RecommitePayvastID').Value:=RecommitID;
        Open;
      end;}
end;
   ...
   RefreshData;
{
 ...
 // btnOpen.Visible:= not ReadOnly;
   ...
   if ShowReadonly then ShowReadOnlyForm else NotShowReadOnlyForm;

   if ADeleteAttachments.Enabled then
   begin
     ...
     //ShowMessage(CHK_USER_ACCESS_QRY.SQL.Text);
   //  CHK_USER_ACCESS_QRY.SQL.SaveToFile('c:\movahed.sql');
...
begin
  inherited;
   ...
   begin
     ...
     dm.Get_RecommitePayvast_by_RecommiteID.Post;
   end;
