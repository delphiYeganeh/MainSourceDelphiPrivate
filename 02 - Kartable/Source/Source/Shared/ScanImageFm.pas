...
Uses Dmu,  YShamsiDate , BusinessLayer, UMain, UPagePreview,
  ...
  WaitPleaseFM, PrintFm, ScanImageDM;
...
begin
   ...
   begin
      ...
      FitToPageClick(Self);
      YRotateLabel1Click(Self);
      YRotateLabel1Click(Self);
//      ImageEnDBView1.AutoFit := True;
//      ImageEnDBVect1.AutoFit := True;
   end;
end;
...
begin
  ...
  begin
     ...
     /////////////////////////////////////////////////////////////
        ...
        begin
            (*Commented By Hamed_Ansari_MRM_990716
            ...
            *)
           if (UpperCase(File_GetFileExt(OpenPictureDialog.Files[I]))='TIF')or(UpperCase(File_GetFileExt(OpenPictureDialog.Files[I]))='TIFF')then
           begin
                 ChangeMultyTifToTifsAndSave(OpenPictureDialog.Files[I]);  //›«Ì· TIFF œ— Â— ’Ê—  »’Ê—  TIFF –ŒÌ—Â ‘Êœ
                 FMWaitPlease.Refresh;
           end
...
begin
   ...
   begin
      ...
      Open;
      if not IsEmpty then
         ShowMessage(' ’«ÊÌ— œ«Œ· Å«Ìê«Â œ«œÂ »«Ìê«‰Ì ‰„«Ì‘ œ«œÂ ‰ŒÊ«Âœ ‘œ');
   end;
...
begin
   inherited;
   if QrLetterData.RecordCount = 0 then  // «ê—  ’ÊÌ— „ÊÃÊœ ‰»«‘œ
      Exit;
   ...
   if Source=Destination then  // «ê— ‘„«—Â „»œ« Ê „ﬁ’œ ÌòÌ »«‘œ
      exit;
   if (Destination<1)or(Destination>QrLetterData.RecordCount) then  // «ê— ‘„«—Â „ﬁ’œ Œ«—Ã «“ »«“Â »«‘œ
      exit;
...
//  if DMScanImage.ColorID=-1 then
  begin
      ...
      begin
         lblCount.text:=' ’›ÕÂ ' + IntToStr(QrLetterData.RecNo) +' «“ '+IntToStr(QrLetterData.RecordCount);
        // Edit1.Visible:=True;
        // Edit1.Text:='';
         try
...
//  ImageEnDBView1.AutoFit := True;
  LabelZoom .Caption :='»“—ê‰„«ÌÌ  :    '+ FloatToStr( Round(ImageEnDBView1.Zoom) )+'%';
//  ImageEnDBView1.FitToWidth;
//  LabelZoom .Caption :='»“—ê‰„«ÌÌ  :    '+ FloatToStr( Round(ImageEnView1.Zoom) )+'%';
end;
