...
begin
  ...
  begin
    ShowMessage('«» œ« ‘„«—Â Õ”«» œ—ŒÊ«”  ò‰‰œÂ —« Ê«—œ ‰„«ÌÌœ');
    SearchEdit.SetFocus;
  end;
end;
...
begin
  ...
  if Dm.select_LoanRequest.Active then
  begin
...
begin
  ...
  if Dm.select_LoanRequest.Active then
  begin
    if messageShowString('¬Ì« «“ Õ–› «Ì‰ œ—ŒÊ«”  „ÿ„∆‰ Â” Ìœ',True) then
    begin
...
begin
   ...
   if (RequestDate <> '14  /  /  ')And isValidDate(RequestDate) then
      if Where = '' then
   ...
   Number.Caption := ' ⁄œ«œ ' + IntToStr(dm.Report_LoanRequest.RecordCount);
end;
...
procedure TFrAddLoanRequest.Button6Click(Sender: TObject);
begin
  inherited;
  ...
  begin
    SearchEdit.Text := _FoundAcountNo;
    RefreshLoanRequest;
  end;
  if  (AccountID <= 0) then
   SearchEdit.Text := '' ;
