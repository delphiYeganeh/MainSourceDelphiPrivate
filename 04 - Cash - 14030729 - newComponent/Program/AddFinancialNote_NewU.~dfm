object FrAddFinancialNote_New: TFrAddFinancialNote_New
  Left = 492
  Top = 245
  Width = 1023
  Height = 507
  BiDiMode = bdRightToLeft
  BorderIcons = []
  Caption = #1608#1585#1608#1583' '#1575#1591#1604#1575#1593#1575#1578' '#1670#1603' '#1607#1575' '#1610#1575' '#1587#1601#1578#1607' '#1607#1575
  Color = clBtnFace
  Font.Charset = ARABIC_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  ParentBiDiMode = False
  Position = poScreenCenter
  Scaled = False
  OnCanResize = FormCanResize
  OnClose = FormClose
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 14
  object pnlMain: TPanel
    Left = 0
    Top = 0
    Width = 1007
    Height = 468
    Align = alClient
    ParentColor = True
    TabOrder = 0
    object Panel3: TPanel
      Left = 1
      Top = 426
      Width = 1005
      Height = 41
      Align = alBottom
      ParentColor = True
      TabOrder = 0
      DesignSize = (
        1005
        41)
      object BitBtn1: TAdvGlowButton
        Left = 922
        Top = 5
        Width = 75
        Height = 28
        Anchors = [akTop, akRight]
        Caption = #1578#1575#1610#1610#1583
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Tahoma'
        Font.Style = []
        ImageIndex = 59
        Images = dm.ImageList_MainNew
        ModalResult = 1
        NotesFont.Charset = DEFAULT_CHARSET
        NotesFont.Color = clWindowText
        NotesFont.Height = -11
        NotesFont.Name = 'Tahoma'
        NotesFont.Style = []
        ParentFont = False
        TabOrder = 0
        OnClick = BitBtn1Click
        Appearance.ColorChecked = 16111818
        Appearance.ColorCheckedTo = 16367008
        Appearance.ColorDisabled = 15921906
        Appearance.ColorDisabledTo = 15921906
        Appearance.ColorDown = 16111818
        Appearance.ColorDownTo = 16367008
        Appearance.ColorHot = 16117985
        Appearance.ColorHotTo = 16372402
        Appearance.ColorMirrorHot = 16107693
        Appearance.ColorMirrorHotTo = 16775412
        Appearance.ColorMirrorDown = 16102556
        Appearance.ColorMirrorDownTo = 16768988
        Appearance.ColorMirrorChecked = 16102556
        Appearance.ColorMirrorCheckedTo = 16768988
        Appearance.ColorMirrorDisabled = 11974326
        Appearance.ColorMirrorDisabledTo = 15921906
      end
      object Button4: TAdvGlowButton
        Left = 845
        Top = 5
        Width = 75
        Height = 28
        Anchors = [akTop, akRight]
        Caption = #1575#1606#1589#1585#1575#1601
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Tahoma'
        Font.Style = []
        ImageIndex = 1
        Images = dm.ImageList_MainNew
        ModalResult = 2
        NotesFont.Charset = DEFAULT_CHARSET
        NotesFont.Color = clWindowText
        NotesFont.Height = -11
        NotesFont.Name = 'Tahoma'
        NotesFont.Style = []
        ParentFont = False
        TabOrder = 1
        OnClick = Button4Click
        Appearance.ColorChecked = 16111818
        Appearance.ColorCheckedTo = 16367008
        Appearance.ColorDisabled = 15921906
        Appearance.ColorDisabledTo = 15921906
        Appearance.ColorDown = 16111818
        Appearance.ColorDownTo = 16367008
        Appearance.ColorHot = 16117985
        Appearance.ColorHotTo = 16372402
        Appearance.ColorMirrorHot = 16107693
        Appearance.ColorMirrorHotTo = 16775412
        Appearance.ColorMirrorDown = 16102556
        Appearance.ColorMirrorDownTo = 16768988
        Appearance.ColorMirrorChecked = 16102556
        Appearance.ColorMirrorCheckedTo = 16768988
        Appearance.ColorMirrorDisabled = 11974326
        Appearance.ColorMirrorDisabledTo = 15921906
      end
      object BitBtn13: TAdvGlowButton
        Left = 5
        Top = 6
        Width = 92
        Height = 28
        Caption = #1582#1585#1608#1580
        Font.Charset = ARABIC_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Tahoma'
        Font.Style = []
        ImageIndex = 84
        Images = dm.ImageList_MainNew
        NotesFont.Charset = DEFAULT_CHARSET
        NotesFont.Color = clWindowText
        NotesFont.Height = -11
        NotesFont.Name = 'Tahoma'
        NotesFont.Style = []
        ParentFont = False
        TabOrder = 2
        OnClick = BitBtn13Click
        Appearance.ColorChecked = 16111818
        Appearance.ColorCheckedTo = 16367008
        Appearance.ColorDisabled = 15921906
        Appearance.ColorDisabledTo = 15921906
        Appearance.ColorDown = 16111818
        Appearance.ColorDownTo = 16367008
        Appearance.ColorHot = 16117985
        Appearance.ColorHotTo = 16372402
        Appearance.ColorMirrorHot = 16107693
        Appearance.ColorMirrorHotTo = 16775412
        Appearance.ColorMirrorDown = 16102556
        Appearance.ColorMirrorDownTo = 16768988
        Appearance.ColorMirrorChecked = 16102556
        Appearance.ColorMirrorCheckedTo = 16768988
        Appearance.ColorMirrorDisabled = 11974326
        Appearance.ColorMirrorDisabledTo = 15921906
      end
    end
    object PageControl1: TPageControl
      Left = 1
      Top = 1
      Width = 1005
      Height = 425
      ActivePage = TabSheet2
      Align = alClient
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      Images = dm.ImageList_MainNew
      ParentFont = False
      TabHeight = 25
      TabOrder = 1
      TabWidth = 150
      OnChanging = PageControl1Changing
      object TabSheet1: TTabSheet
        Caption = #1604#1610#1587#1578
        ImageIndex = 118
        OnShow = TabSheet1Show
        DesignSize = (
          997
          390)
        object SpeedButton2: TAdvGlowButton
          Left = 597
          Top = 309
          Width = 121
          Height = 29
          Anchors = [akLeft, akBottom]
          Caption = #1583#1585' '#1580#1585#1610#1575#1606' '#1608#1589#1608#1604
          ImageIndex = 179
          Images = dm.LetterImages_New
          NotesFont.Charset = DEFAULT_CHARSET
          NotesFont.Color = clWindowText
          NotesFont.Height = -11
          NotesFont.Name = 'Tahoma'
          NotesFont.Style = []
          TabOrder = 3
          OnClick = SpeedButton2Click
          Appearance.ColorChecked = 16111818
          Appearance.ColorCheckedTo = 16367008
          Appearance.ColorDisabled = 15921906
          Appearance.ColorDisabledTo = 15921906
          Appearance.ColorDown = 16111818
          Appearance.ColorDownTo = 16367008
          Appearance.ColorHot = 16117985
          Appearance.ColorHotTo = 16372402
          Appearance.ColorMirrorHot = 16107693
          Appearance.ColorMirrorHotTo = 16775412
          Appearance.ColorMirrorDown = 16102556
          Appearance.ColorMirrorDownTo = 16768988
          Appearance.ColorMirrorChecked = 16102556
          Appearance.ColorMirrorCheckedTo = 16768988
          Appearance.ColorMirrorDisabled = 11974326
          Appearance.ColorMirrorDisabledTo = 15921906
        end
        object SpeedButton3: TAdvGlowButton
          Left = 474
          Top = 309
          Width = 121
          Height = 29
          Anchors = [akLeft, akBottom]
          Caption = #1608#1589#1608#1604' '#1588#1583
          ImageIndex = 214
          Images = dm.LetterImages_New
          NotesFont.Charset = DEFAULT_CHARSET
          NotesFont.Color = clWindowText
          NotesFont.Height = -11
          NotesFont.Name = 'Tahoma'
          NotesFont.Style = []
          TabOrder = 4
          OnClick = SpeedButton3Click
          Appearance.ColorChecked = 16111818
          Appearance.ColorCheckedTo = 16367008
          Appearance.ColorDisabled = 15921906
          Appearance.ColorDisabledTo = 15921906
          Appearance.ColorDown = 16111818
          Appearance.ColorDownTo = 16367008
          Appearance.ColorHot = 16117985
          Appearance.ColorHotTo = 16372402
          Appearance.ColorMirrorHot = 16107693
          Appearance.ColorMirrorHotTo = 16775412
          Appearance.ColorMirrorDown = 16102556
          Appearance.ColorMirrorDownTo = 16768988
          Appearance.ColorMirrorChecked = 16102556
          Appearance.ColorMirrorCheckedTo = 16768988
          Appearance.ColorMirrorDisabled = 11974326
          Appearance.ColorMirrorDisabledTo = 15921906
        end
        object SpeedButton4: TAdvGlowButton
          Left = 351
          Top = 309
          Width = 121
          Height = 29
          Anchors = [akLeft, akBottom]
          Caption = #1576#1585#1711#1588#1578
          ImageIndex = 181
          Images = dm.LetterImages_New
          NotesFont.Charset = DEFAULT_CHARSET
          NotesFont.Color = clWindowText
          NotesFont.Height = -11
          NotesFont.Name = 'Tahoma'
          NotesFont.Style = []
          TabOrder = 5
          OnClick = SpeedButton4Click
          Appearance.ColorChecked = 16111818
          Appearance.ColorCheckedTo = 16367008
          Appearance.ColorDisabled = 15921906
          Appearance.ColorDisabledTo = 15921906
          Appearance.ColorDown = 16111818
          Appearance.ColorDownTo = 16367008
          Appearance.ColorHot = 16117985
          Appearance.ColorHotTo = 16372402
          Appearance.ColorMirrorHot = 16107693
          Appearance.ColorMirrorHotTo = 16775412
          Appearance.ColorMirrorDown = 16102556
          Appearance.ColorMirrorDownTo = 16768988
          Appearance.ColorMirrorChecked = 16102556
          Appearance.ColorMirrorCheckedTo = 16768988
          Appearance.ColorMirrorDisabled = 11974326
          Appearance.ColorMirrorDisabledTo = 15921906
        end
        object SpeedButton5: TAdvGlowButton
          Left = 229
          Top = 309
          Width = 121
          Height = 29
          Anchors = [akLeft, akBottom]
          Caption = #1575#1576#1591#1575#1604
          ImageIndex = 240
          Images = dm.LetterImages_New
          NotesFont.Charset = DEFAULT_CHARSET
          NotesFont.Color = clWindowText
          NotesFont.Height = -11
          NotesFont.Name = 'Tahoma'
          NotesFont.Style = []
          TabOrder = 6
          OnClick = SpeedButton5Click
          Appearance.ColorChecked = 16111818
          Appearance.ColorCheckedTo = 16367008
          Appearance.ColorDisabled = 15921906
          Appearance.ColorDisabledTo = 15921906
          Appearance.ColorDown = 16111818
          Appearance.ColorDownTo = 16367008
          Appearance.ColorHot = 16117985
          Appearance.ColorHotTo = 16372402
          Appearance.ColorMirrorHot = 16107693
          Appearance.ColorMirrorHotTo = 16775412
          Appearance.ColorMirrorDown = 16102556
          Appearance.ColorMirrorDownTo = 16768988
          Appearance.ColorMirrorChecked = 16102556
          Appearance.ColorMirrorCheckedTo = 16768988
          Appearance.ColorMirrorDisabled = 11974326
          Appearance.ColorMirrorDisabledTo = 15921906
        end
        object Panel1: TPanel
          Left = 0
          Top = 349
          Width = 997
          Height = 41
          Align = alBottom
          ParentColor = True
          TabOrder = 0
          DesignSize = (
            997
            41)
          object xpBitBtn1: TAdvGlowButton
            Left = 6
            Top = 6
            Width = 95
            Height = 28
            Caption = #1575#1587#1705#1606' '#1578#1589#1575#1608#1610#1585
            ImageIndex = 182
            Images = dm.LetterImages_New
            NotesFont.Charset = DEFAULT_CHARSET
            NotesFont.Color = clWindowText
            NotesFont.Height = -11
            NotesFont.Name = 'Tahoma'
            NotesFont.Style = []
            TabOrder = 2
            OnClick = xpBitBtn1Click
            Appearance.ColorChecked = 16111818
            Appearance.ColorCheckedTo = 16367008
            Appearance.ColorDisabled = 15921906
            Appearance.ColorDisabledTo = 15921906
            Appearance.ColorDown = 16111818
            Appearance.ColorDownTo = 16367008
            Appearance.ColorHot = 16117985
            Appearance.ColorHotTo = 16372402
            Appearance.ColorMirrorHot = 16107693
            Appearance.ColorMirrorHotTo = 16775412
            Appearance.ColorMirrorDown = 16102556
            Appearance.ColorMirrorDownTo = 16768988
            Appearance.ColorMirrorChecked = 16102556
            Appearance.ColorMirrorCheckedTo = 16768988
            Appearance.ColorMirrorDisabled = 11974326
            Appearance.ColorMirrorDisabledTo = 15921906
          end
          object Button2: TAdvGlowButton
            Left = 103
            Top = 6
            Width = 95
            Height = 28
            Caption = #1670#1575#1662' '#1585#1587#1610#1583
            ImageIndex = 163
            Images = dm.LetterImages_New
            NotesFont.Charset = DEFAULT_CHARSET
            NotesFont.Color = clWindowText
            NotesFont.Height = -11
            NotesFont.Name = 'Tahoma'
            NotesFont.Style = []
            TabOrder = 3
            OnClick = Button2Click
            Appearance.ColorChecked = 16111818
            Appearance.ColorCheckedTo = 16367008
            Appearance.ColorDisabled = 15921906
            Appearance.ColorDisabledTo = 15921906
            Appearance.ColorDown = 16111818
            Appearance.ColorDownTo = 16367008
            Appearance.ColorHot = 16117985
            Appearance.ColorHotTo = 16372402
            Appearance.ColorMirrorHot = 16107693
            Appearance.ColorMirrorHotTo = 16775412
            Appearance.ColorMirrorDown = 16102556
            Appearance.ColorMirrorDownTo = 16768988
            Appearance.ColorMirrorChecked = 16102556
            Appearance.ColorMirrorCheckedTo = 16768988
            Appearance.ColorMirrorDisabled = 11974326
            Appearance.ColorMirrorDisabledTo = 15921906
          end
          object SpeedButton1: TAdvGlowButton
            Left = 200
            Top = 6
            Width = 95
            Height = 28
            Caption = #1670#1575#1662' '#1670#1705
            ImageIndex = 235
            Images = dm.LetterImages_New
            NotesFont.Charset = DEFAULT_CHARSET
            NotesFont.Color = clWindowText
            NotesFont.Height = -11
            NotesFont.Name = 'Tahoma'
            NotesFont.Style = []
            TabOrder = 4
            Visible = False
            Appearance.ColorChecked = 16111818
            Appearance.ColorCheckedTo = 16367008
            Appearance.ColorDisabled = 15921906
            Appearance.ColorDisabledTo = 15921906
            Appearance.ColorDown = 16111818
            Appearance.ColorDownTo = 16367008
            Appearance.ColorHot = 16117985
            Appearance.ColorHotTo = 16372402
            Appearance.ColorMirrorHot = 16107693
            Appearance.ColorMirrorHotTo = 16775412
            Appearance.ColorMirrorDown = 16102556
            Appearance.ColorMirrorDownTo = 16768988
            Appearance.ColorMirrorChecked = 16102556
            Appearance.ColorMirrorCheckedTo = 16768988
            Appearance.ColorMirrorDisabled = 11974326
            Appearance.ColorMirrorDisabledTo = 15921906
          end
          object xpBitBtn2: TAdvGlowButton
            Left = 768
            Top = 7
            Width = 115
            Height = 28
            Anchors = [akTop, akRight]
            Caption = #1608#1610#1585#1575#1610#1588'  '#1575#1591#1604#1575#1593#1575#1578
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clNavy
            Font.Height = -12
            Font.Name = 'Tahoma'
            Font.Style = []
            ImageIndex = 2
            Images = dm.ImageList_MainNew
            NotesFont.Charset = DEFAULT_CHARSET
            NotesFont.Color = clWindowText
            NotesFont.Height = -11
            NotesFont.Name = 'Tahoma'
            NotesFont.Style = []
            ParentFont = False
            TabOrder = 0
            OnClick = xpBitBtn2Click
            Appearance.ColorChecked = 16111818
            Appearance.ColorCheckedTo = 16367008
            Appearance.ColorDisabled = 15921906
            Appearance.ColorDisabledTo = 15921906
            Appearance.ColorDown = 16111818
            Appearance.ColorDownTo = 16367008
            Appearance.ColorHot = 16117985
            Appearance.ColorHotTo = 16372402
            Appearance.ColorMirrorHot = 16107693
            Appearance.ColorMirrorHotTo = 16775412
            Appearance.ColorMirrorDown = 16102556
            Appearance.ColorMirrorDownTo = 16768988
            Appearance.ColorMirrorChecked = 16102556
            Appearance.ColorMirrorCheckedTo = 16768988
            Appearance.ColorMirrorDisabled = 11974326
            Appearance.ColorMirrorDisabledTo = 15921906
          end
          object Button3: TAdvGlowButton
            Left = 887
            Top = 7
            Width = 102
            Height = 28
            Anchors = [akTop, akRight]
            Caption = #1580#1583#1610#1583
            ImageIndex = 60
            Images = dm.ImageList_MainNew
            NotesFont.Charset = DEFAULT_CHARSET
            NotesFont.Color = clWindowText
            NotesFont.Height = -11
            NotesFont.Name = 'Tahoma'
            NotesFont.Style = []
            TabOrder = 1
            OnClick = Button3Click
            Appearance.ColorChecked = 16111818
            Appearance.ColorCheckedTo = 16367008
            Appearance.ColorDisabled = 15921906
            Appearance.ColorDisabledTo = 15921906
            Appearance.ColorDown = 16111818
            Appearance.ColorDownTo = 16367008
            Appearance.ColorHot = 16117985
            Appearance.ColorHotTo = 16372402
            Appearance.ColorMirrorHot = 16107693
            Appearance.ColorMirrorHotTo = 16775412
            Appearance.ColorMirrorDown = 16102556
            Appearance.ColorMirrorDownTo = 16768988
            Appearance.ColorMirrorChecked = 16102556
            Appearance.ColorMirrorCheckedTo = 16768988
            Appearance.ColorMirrorDisabled = 11974326
            Appearance.ColorMirrorDisabledTo = 15921906
          end
        end
        object Panel2: TPanel
          Left = 0
          Top = 0
          Width = 997
          Height = 39
          Align = alTop
          ParentColor = True
          TabOrder = 1
          DesignSize = (
            997
            39)
          object Label10: TLabel
            Left = 954
            Top = 5
            Width = 38
            Height = 14
            Anchors = [akTop, akRight]
            Caption = #1580#1587#1578#1580#1608
          end
          object btnSearch: TAdvGlowButton
            Left = 147
            Top = 4
            Width = 123
            Height = 28
            Caption = #1606#1605#1575#1610#1588' '#1705#1604' '#1575#1591#1604#1575#1593#1575#1578
            ImageIndex = 41
            Images = dm.ImageList_MainNew
            NotesFont.Charset = DEFAULT_CHARSET
            NotesFont.Color = clWindowText
            NotesFont.Height = -11
            NotesFont.Name = 'Tahoma'
            NotesFont.Style = []
            TabOrder = 3
            OnClick = btnSearchClick
            Appearance.ColorChecked = 16111818
            Appearance.ColorCheckedTo = 16367008
            Appearance.ColorDisabled = 15921906
            Appearance.ColorDisabledTo = 15921906
            Appearance.ColorDown = 16111818
            Appearance.ColorDownTo = 16367008
            Appearance.ColorHot = 16117985
            Appearance.ColorHotTo = 16372402
            Appearance.ColorMirrorHot = 16107693
            Appearance.ColorMirrorHotTo = 16775412
            Appearance.ColorMirrorDown = 16102556
            Appearance.ColorMirrorDownTo = 16768988
            Appearance.ColorMirrorChecked = 16102556
            Appearance.ColorMirrorCheckedTo = 16768988
            Appearance.ColorMirrorDisabled = 11974326
            Appearance.ColorMirrorDisabledTo = 15921906
          end
          object SpeedButton6: TAdvGlowButton
            Left = 275
            Top = 4
            Width = 123
            Height = 28
            Caption = #1580#1587#1578#1580#1608
            ImageIndex = 70
            Images = dm.ImageList_MainNew
            NotesFont.Charset = DEFAULT_CHARSET
            NotesFont.Color = clWindowText
            NotesFont.Height = -11
            NotesFont.Name = 'Tahoma'
            NotesFont.Style = []
            TabOrder = 4
            OnClick = SpeedButton6Click
            Appearance.ColorChecked = 16111818
            Appearance.ColorCheckedTo = 16367008
            Appearance.ColorDisabled = 15921906
            Appearance.ColorDisabledTo = 15921906
            Appearance.ColorDown = 16111818
            Appearance.ColorDownTo = 16367008
            Appearance.ColorHot = 16117985
            Appearance.ColorHotTo = 16372402
            Appearance.ColorMirrorHot = 16107693
            Appearance.ColorMirrorHotTo = 16775412
            Appearance.ColorMirrorDown = 16102556
            Appearance.ColorMirrorDownTo = 16768988
            Appearance.ColorMirrorChecked = 16102556
            Appearance.ColorMirrorCheckedTo = 16768988
            Appearance.ColorMirrorDisabled = 11974326
            Appearance.ColorMirrorDisabledTo = 15921906
          end
          object SearchEdit: TEdit
            Left = 771
            Top = 0
            Width = 177
            Height = 22
            Anchors = [akTop, akRight]
            Font.Charset = ARABIC_CHARSET
            Font.Color = clWindowText
            Font.Height = -12
            Font.Name = 'Tahoma'
            Font.Style = []
            ParentFont = False
            TabOrder = 0
          end
          object FinancialNoteStatuse: TDBLookupComboBox
            Left = 515
            Top = 0
            Width = 140
            Height = 22
            Anchors = [akTop, akRight]
            BiDiMode = bdLeftToRight
            Font.Charset = ARABIC_CHARSET
            Font.Color = clWindowText
            Font.Height = -12
            Font.Name = 'Tahoma'
            Font.Style = []
            KeyField = 'FinancialNoteStatusID'
            ListField = 'FinancialNoteStatusDesc'
            ListSource = dm.DFinancialNoteStatus
            ParentBiDiMode = False
            ParentFont = False
            TabOrder = 1
            OnCloseUp = FinancialNoteStatuseCloseUp
          end
          object CbFinancialType: TCheckBox
            Left = 659
            Top = 4
            Width = 94
            Height = 17
            Anchors = [akTop, akRight]
            Caption = #1580#1587#1578#1580#1608' '#1601#1602#1591' '#1583#1585' '
            TabOrder = 2
          end
        end
        object YDBGrid2: TYDBGrid
          Left = 0
          Top = 39
          Width = 997
          Height = 266
          Cursor = crHandPoint
          Align = alTop
          Anchors = [akLeft, akTop, akRight, akBottom]
          DataSource = dm.DSelect_FinancialNote_Like_New
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
          TabOrder = 2
          TitleFont.Charset = ARABIC_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -12
          TitleFont.Name = 'Tahoma'
          TitleFont.Style = []
          OnDblClick = YDBGrid1EditButtonClick
          OnEditButtonClick = YDBGrid1EditButtonClick
          YeganehColor = False
          YeganehFinish = False
          PageNumber = False
          RecordNumber = False
          HasTime = False
          Oriention = poPortrait
          HasDate = False
          HasIndex = False
          InvertFarsiDate = True
          TitleSort = True
          AutoInsert = False
          FooterFields = 'Count'
          Columns = <
            item
              Expanded = False
              FieldName = 'NoteNo'
              Title.Alignment = taCenter
              Title.Caption = #1588#1605#1575#1585#1607
              Width = 54
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NoteNo'
              Title.Caption = #1588#1605#1575#1585#1607' '#1587#1606#1583
              Visible = False
            end
            item
              Expanded = False
              FieldName = 'BankTitle'
              Title.Caption = #1593#1604#1578
              Visible = False
            end
            item
              Expanded = False
              FieldName = 'Amount'
              Title.Alignment = taCenter
              Title.Caption = #1605#1576#1604#1594
              Width = 83
              Visible = True
            end
            item
              ButtonStyle = cbsEllipsis
              Expanded = False
              FieldName = 'ForcePaymentID'
              Title.Alignment = taCenter
              Title.Caption = #1588'.'#1602#1587#1591#1548#1608#1575#1605' '#1610#1575' '#1605#1575#1607#1610#1575#1606#1607
              Width = 130
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'FND_LoanNo'
              Title.Caption = #1588#1605#1575#1585#1607' '#1608#1575#1605
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NoteDate'
              Title.Alignment = taCenter
              Title.Caption = #1578#1575#1585#1610#1582' '
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NoteDescription'
              Title.Alignment = taCenter
              Title.Caption = #1588#1585#1581' '
              Width = 115
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'FinancialNoteStatusDesc'
              Title.Alignment = taCenter
              Title.Caption = #1608#1590#1593#1610#1578
              Width = 68
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'FinancialNoteTypeTitle'
              Title.Alignment = taCenter
              Title.Caption = #1606#1608#1593
              Width = 70
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'BankTitle'
              Title.Alignment = taCenter
              Title.Caption = #1576#1575#1606#1705
              Width = 91
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'BankAccountNo'
              Title.Alignment = taCenter
              Title.Caption = #1588'.'#1581#1587#1575#1576
              Width = 96
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'Branch'
              Title.Alignment = taCenter
              Title.Caption = #1588#1593#1576#1607
              Width = 59
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'BranchCode'
              Title.Alignment = taCenter
              Title.Caption = #1705#1583' '#1588#1593#1576#1607
              Visible = True
            end>
        end
      end
      object TabSheet2: TTabSheet
        Caption = #1608#1585#1608#1583' '#1575#1591#1604#1575#1593#1575#1578
        ImageIndex = 7
        DesignSize = (
          997
          390)
        object Shape1: TShape
          Left = 2
          Top = 3
          Width = 191
          Height = 50
          Brush.Style = bsClear
        end
        object Label3: TLabel
          Left = 138
          Top = 31
          Width = 44
          Height = 14
          Caption = #1578#1575#1585#1610#1582' '#1579#1576#1578
        end
        object Label9: TLabel
          Left = 944
          Top = 5
          Width = 42
          Height = 14
          Alignment = taRightJustify
          Anchors = [akTop, akRight]
          BiDiMode = bdLeftToRight
          Caption = #1606#1608#1593' '#1587#1606#1583
          ParentBiDiMode = False
        end
        object amount: TLabel
          Left = 75
          Top = 87
          Width = 538
          Height = 38
          Anchors = [akTop, akRight]
          AutoSize = False
          Caption = '-------------'
          WordWrap = True
        end
        object Label11: TLabel
          Left = 133
          Top = 10
          Width = 49
          Height = 14
          Caption = #1579#1576#1578' '#1705#1606#1606#1583#1607
        end
        object LBSbj: TLabel
          Left = 930
          Top = 45
          Width = 56
          Height = 14
          Alignment = taRightJustify
          Anchors = [akTop, akRight]
          BiDiMode = bdLeftToRight
          Caption = #1605#1608#1590#1608#1593' '#1670#1705
          ParentBiDiMode = False
        end
        object LbPrice: TLabel
          Left = 590
          Top = 45
          Width = 64
          Height = 14
          Alignment = taRightJustify
          Anchors = [akTop, akRight]
          BiDiMode = bdLeftToRight
          Caption = #1605#1576#1604#1594' '#1705#1604' '#1670#1705
          ParentBiDiMode = False
        end
        object LbResiver: TLabel
          Left = 566
          Top = 8
          Width = 88
          Height = 14
          Alignment = taRightJustify
          Anchors = [akTop, akRight]
          BiDiMode = bdLeftToRight
          Caption = #1583#1585#1610#1575#1601#1578' '#1705#1606#1606#1583#1607' '#1670#1705
          ParentBiDiMode = False
        end
        object LbNo: TLabel
          Left = 937
          Top = 81
          Width = 49
          Height = 14
          Alignment = taRightJustify
          Anchors = [akTop, akRight]
          BiDiMode = bdLeftToRight
          Caption = #1578#1593#1583#1575#1583' '#1670#1705
          ParentBiDiMode = False
        end
        object DBText1: TDBText
          Left = 4
          Top = 8
          Width = 107
          Height = 17
          DataField = 'UserConfirmer_Look'
          DataSource = dm.DFinancialNote_Master
        end
        object DBText2: TDBText
          Left = 4
          Top = 29
          Width = 107
          Height = 17
          DataField = 'MatureDate'
          DataSource = dm.DFinancialNote_Master
        end
        object DBLookupComboBox1: TDBLookupComboBox
          Left = 682
          Top = 0
          Width = 240
          Height = 22
          Anchors = [akTop, akRight]
          BiDiMode = bdLeftToRight
          DataField = 'FinancialNoteTypeID'
          DataSource = dm.DFinancialNote_Master
          Font.Charset = ARABIC_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          KeyField = 'FinancialNoteTypeID'
          ListField = 'FinancialNoteTypeTitle'
          ListSource = dm.DFinancialNoteType
          ParentBiDiMode = False
          ParentFont = False
          TabOrder = 1
        end
        object BitBtn2: TAdvGlowButton
          Left = 919
          Top = 138
          Width = 65
          Height = 28
          Action = DataSetInsert_Master
          Anchors = [akTop, akRight]
          Font.Charset = ARABIC_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ImageIndex = 60
          Images = dm.ImageList_MainNew
          NotesFont.Charset = DEFAULT_CHARSET
          NotesFont.Color = clWindowText
          NotesFont.Height = -11
          NotesFont.Name = 'Tahoma'
          NotesFont.Style = []
          ParentFont = False
          TabOrder = 0
          Appearance.ColorChecked = 16111818
          Appearance.ColorCheckedTo = 16367008
          Appearance.ColorDisabled = 15921906
          Appearance.ColorDisabledTo = 15921906
          Appearance.ColorDown = 16111818
          Appearance.ColorDownTo = 16367008
          Appearance.ColorHot = 16117985
          Appearance.ColorHotTo = 16372402
          Appearance.ColorMirrorHot = 16107693
          Appearance.ColorMirrorHotTo = 16775412
          Appearance.ColorMirrorDown = 16102556
          Appearance.ColorMirrorDownTo = 16768988
          Appearance.ColorMirrorChecked = 16102556
          Appearance.ColorMirrorCheckedTo = 16768988
          Appearance.ColorMirrorDisabled = 11974326
          Appearance.ColorMirrorDisabledTo = 15921906
        end
        object BitBtn3: TAdvGlowButton
          Left = 853
          Top = 138
          Width = 65
          Height = 28
          Action = DataSetEdit_Master
          Anchors = [akTop, akRight]
          Font.Charset = ARABIC_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ImageIndex = 2
          Images = dm.ImageList_MainNew
          NotesFont.Charset = DEFAULT_CHARSET
          NotesFont.Color = clWindowText
          NotesFont.Height = -11
          NotesFont.Name = 'Tahoma'
          NotesFont.Style = []
          ParentFont = False
          TabOrder = 6
          Appearance.ColorChecked = 16111818
          Appearance.ColorCheckedTo = 16367008
          Appearance.ColorDisabled = 15921906
          Appearance.ColorDisabledTo = 15921906
          Appearance.ColorDown = 16111818
          Appearance.ColorDownTo = 16367008
          Appearance.ColorHot = 16117985
          Appearance.ColorHotTo = 16372402
          Appearance.ColorMirrorHot = 16107693
          Appearance.ColorMirrorHotTo = 16775412
          Appearance.ColorMirrorDown = 16102556
          Appearance.ColorMirrorDownTo = 16768988
          Appearance.ColorMirrorChecked = 16102556
          Appearance.ColorMirrorCheckedTo = 16768988
          Appearance.ColorMirrorDisabled = 11974326
          Appearance.ColorMirrorDisabledTo = 15921906
        end
        object BitBtn4: TAdvGlowButton
          Left = 787
          Top = 138
          Width = 65
          Height = 28
          Action = DataSetPost_Master
          Anchors = [akTop, akRight]
          Font.Charset = ARABIC_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ImageIndex = 59
          Images = dm.ImageList_MainNew
          NotesFont.Charset = DEFAULT_CHARSET
          NotesFont.Color = clWindowText
          NotesFont.Height = -11
          NotesFont.Name = 'Tahoma'
          NotesFont.Style = []
          ParentFont = False
          TabOrder = 7
          Appearance.ColorChecked = 16111818
          Appearance.ColorCheckedTo = 16367008
          Appearance.ColorDisabled = 15921906
          Appearance.ColorDisabledTo = 15921906
          Appearance.ColorDown = 16111818
          Appearance.ColorDownTo = 16367008
          Appearance.ColorHot = 16117985
          Appearance.ColorHotTo = 16372402
          Appearance.ColorMirrorHot = 16107693
          Appearance.ColorMirrorHotTo = 16775412
          Appearance.ColorMirrorDown = 16102556
          Appearance.ColorMirrorDownTo = 16768988
          Appearance.ColorMirrorChecked = 16102556
          Appearance.ColorMirrorCheckedTo = 16768988
          Appearance.ColorMirrorDisabled = 11974326
          Appearance.ColorMirrorDisabledTo = 15921906
        end
        object BitBtn5: TAdvGlowButton
          Left = 721
          Top = 138
          Width = 65
          Height = 28
          Action = DataSetDelete_Master
          Anchors = [akTop, akRight]
          Font.Charset = ARABIC_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ImageIndex = 90
          Images = dm.ImageList_MainNew
          NotesFont.Charset = DEFAULT_CHARSET
          NotesFont.Color = clWindowText
          NotesFont.Height = -11
          NotesFont.Name = 'Tahoma'
          NotesFont.Style = []
          ParentFont = False
          TabOrder = 8
          Appearance.ColorChecked = 16111818
          Appearance.ColorCheckedTo = 16367008
          Appearance.ColorDisabled = 15921906
          Appearance.ColorDisabledTo = 15921906
          Appearance.ColorDown = 16111818
          Appearance.ColorDownTo = 16367008
          Appearance.ColorHot = 16117985
          Appearance.ColorHotTo = 16372402
          Appearance.ColorMirrorHot = 16107693
          Appearance.ColorMirrorHotTo = 16775412
          Appearance.ColorMirrorDown = 16102556
          Appearance.ColorMirrorDownTo = 16768988
          Appearance.ColorMirrorChecked = 16102556
          Appearance.ColorMirrorCheckedTo = 16768988
          Appearance.ColorMirrorDisabled = 11974326
          Appearance.ColorMirrorDisabledTo = 15921906
        end
        object BitBtn6: TAdvGlowButton
          Left = 655
          Top = 138
          Width = 65
          Height = 28
          Action = DataSetCancel_Master
          Anchors = [akTop, akRight]
          Font.Charset = ARABIC_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          ImageIndex = 1
          Images = dm.ImageList_MainNew
          NotesFont.Charset = DEFAULT_CHARSET
          NotesFont.Color = clWindowText
          NotesFont.Height = -11
          NotesFont.Name = 'Tahoma'
          NotesFont.Style = []
          ParentFont = False
          TabOrder = 9
          Appearance.ColorChecked = 16111818
          Appearance.ColorCheckedTo = 16367008
          Appearance.ColorDisabled = 15921906
          Appearance.ColorDisabledTo = 15921906
          Appearance.ColorDown = 16111818
          Appearance.ColorDownTo = 16367008
          Appearance.ColorHot = 16117985
          Appearance.ColorHotTo = 16372402
          Appearance.ColorMirrorHot = 16107693
          Appearance.ColorMirrorHotTo = 16775412
          Appearance.ColorMirrorDown = 16102556
          Appearance.ColorMirrorDownTo = 16768988
          Appearance.ColorMirrorChecked = 16102556
          Appearance.ColorMirrorCheckedTo = 16768988
          Appearance.ColorMirrorDisabled = 11974326
          Appearance.ColorMirrorDisabledTo = 15921906
        end
        object DBEdit6: TDBEdit
          Left = 682
          Top = 38
          Width = 240
          Height = 22
          Anchors = [akTop, akRight]
          DataField = 'NoteSubject'
          DataSource = dm.DFinancialNote_Master
          Font.Charset = ARABIC_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          TabOrder = 2
        end
        object Panel4: TPanel
          Left = 0
          Top = 170
          Width = 997
          Height = 220
          Align = alBottom
          Anchors = [akLeft, akTop, akRight, akBottom]
          ParentColor = True
          TabOrder = 10
          DesignSize = (
            997
            220)
          object Shape2: TShape
            Left = 216
            Top = 190
            Width = 289
            Height = 23
            Anchors = [akLeft, akBottom]
            Brush.Style = bsClear
          end
          object Label2: TLabel
            Left = 429
            Top = 195
            Width = 72
            Height = 14
            Anchors = [akLeft, akBottom]
            Caption = #1605#1576#1604#1594' '#1576#1575#1602#1610#1605#1575#1606#1583#1607':'
          end
          object LbRemain: TLabel
            Left = 224
            Top = 195
            Width = 209
            Height = 13
            Alignment = taCenter
            Anchors = [akLeft, akBottom]
            AutoSize = False
            Caption = '-'
          end
          object BitBtn7: TAdvGlowButton
            Left = 849
            Top = 187
            Width = 70
            Height = 28
            Action = DataSetEdit_Detail
            Anchors = [akLeft, akBottom]
            Font.Charset = ARABIC_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = []
            ImageIndex = 2
            Images = dm.ImageList_MainNew
            NotesFont.Charset = DEFAULT_CHARSET
            NotesFont.Color = clWindowText
            NotesFont.Height = -11
            NotesFont.Name = 'Tahoma'
            NotesFont.Style = []
            ParentFont = False
            TabOrder = 1
            Appearance.ColorChecked = 16111818
            Appearance.ColorCheckedTo = 16367008
            Appearance.ColorDisabled = 15921906
            Appearance.ColorDisabledTo = 15921906
            Appearance.ColorDown = 16111818
            Appearance.ColorDownTo = 16367008
            Appearance.ColorHot = 16117985
            Appearance.ColorHotTo = 16372402
            Appearance.ColorMirrorHot = 16107693
            Appearance.ColorMirrorHotTo = 16775412
            Appearance.ColorMirrorDown = 16102556
            Appearance.ColorMirrorDownTo = 16768988
            Appearance.ColorMirrorChecked = 16102556
            Appearance.ColorMirrorCheckedTo = 16768988
            Appearance.ColorMirrorDisabled = 11974326
            Appearance.ColorMirrorDisabledTo = 15921906
          end
          object BitBtn8: TAdvGlowButton
            Left = 920
            Top = 187
            Width = 70
            Height = 28
            Action = DataSetInsert_Detail
            Anchors = [akLeft, akBottom]
            Font.Charset = ARABIC_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = []
            ImageIndex = 60
            Images = dm.ImageList_MainNew
            NotesFont.Charset = DEFAULT_CHARSET
            NotesFont.Color = clWindowText
            NotesFont.Height = -11
            NotesFont.Name = 'Tahoma'
            NotesFont.Style = []
            ParentFont = False
            TabOrder = 0
            Appearance.ColorChecked = 16111818
            Appearance.ColorCheckedTo = 16367008
            Appearance.ColorDisabled = 15921906
            Appearance.ColorDisabledTo = 15921906
            Appearance.ColorDown = 16111818
            Appearance.ColorDownTo = 16367008
            Appearance.ColorHot = 16117985
            Appearance.ColorHotTo = 16372402
            Appearance.ColorMirrorHot = 16107693
            Appearance.ColorMirrorHotTo = 16775412
            Appearance.ColorMirrorDown = 16102556
            Appearance.ColorMirrorDownTo = 16768988
            Appearance.ColorMirrorChecked = 16102556
            Appearance.ColorMirrorCheckedTo = 16768988
            Appearance.ColorMirrorDisabled = 11974326
            Appearance.ColorMirrorDisabledTo = 15921906
          end
          object BitBtn9: TAdvGlowButton
            Left = 778
            Top = 187
            Width = 70
            Height = 28
            Action = DataSetPost_Detail
            Anchors = [akLeft, akBottom]
            Font.Charset = ARABIC_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = []
            ImageIndex = 59
            Images = dm.ImageList_MainNew
            NotesFont.Charset = DEFAULT_CHARSET
            NotesFont.Color = clWindowText
            NotesFont.Height = -11
            NotesFont.Name = 'Tahoma'
            NotesFont.Style = []
            ParentFont = False
            TabOrder = 2
            Appearance.ColorChecked = 16111818
            Appearance.ColorCheckedTo = 16367008
            Appearance.ColorDisabled = 15921906
            Appearance.ColorDisabledTo = 15921906
            Appearance.ColorDown = 16111818
            Appearance.ColorDownTo = 16367008
            Appearance.ColorHot = 16117985
            Appearance.ColorHotTo = 16372402
            Appearance.ColorMirrorHot = 16107693
            Appearance.ColorMirrorHotTo = 16775412
            Appearance.ColorMirrorDown = 16102556
            Appearance.ColorMirrorDownTo = 16768988
            Appearance.ColorMirrorChecked = 16102556
            Appearance.ColorMirrorCheckedTo = 16768988
            Appearance.ColorMirrorDisabled = 11974326
            Appearance.ColorMirrorDisabledTo = 15921906
          end
          object BitBtn10: TAdvGlowButton
            Left = 707
            Top = 187
            Width = 70
            Height = 28
            Action = DataSetDelete_Detail
            Anchors = [akLeft, akBottom]
            Font.Charset = ARABIC_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = []
            ImageIndex = 90
            Images = dm.ImageList_MainNew
            NotesFont.Charset = DEFAULT_CHARSET
            NotesFont.Color = clWindowText
            NotesFont.Height = -11
            NotesFont.Name = 'Tahoma'
            NotesFont.Style = []
            ParentFont = False
            TabOrder = 3
            Appearance.ColorChecked = 16111818
            Appearance.ColorCheckedTo = 16367008
            Appearance.ColorDisabled = 15921906
            Appearance.ColorDisabledTo = 15921906
            Appearance.ColorDown = 16111818
            Appearance.ColorDownTo = 16367008
            Appearance.ColorHot = 16117985
            Appearance.ColorHotTo = 16372402
            Appearance.ColorMirrorHot = 16107693
            Appearance.ColorMirrorHotTo = 16775412
            Appearance.ColorMirrorDown = 16102556
            Appearance.ColorMirrorDownTo = 16768988
            Appearance.ColorMirrorChecked = 16102556
            Appearance.ColorMirrorCheckedTo = 16768988
            Appearance.ColorMirrorDisabled = 11974326
            Appearance.ColorMirrorDisabledTo = 15921906
          end
          object BitBtn11: TAdvGlowButton
            Left = 636
            Top = 187
            Width = 70
            Height = 28
            Action = DataSetCancel_Detail
            Anchors = [akLeft, akBottom]
            Font.Charset = ARABIC_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'Tahoma'
            Font.Style = []
            ImageIndex = 1
            Images = dm.ImageList_MainNew
            NotesFont.Charset = DEFAULT_CHARSET
            NotesFont.Color = clWindowText
            NotesFont.Height = -11
            NotesFont.Name = 'Tahoma'
            NotesFont.Style = []
            ParentFont = False
            TabOrder = 4
            Appearance.ColorChecked = 16111818
            Appearance.ColorCheckedTo = 16367008
            Appearance.ColorDisabled = 15921906
            Appearance.ColorDisabledTo = 15921906
            Appearance.ColorDown = 16111818
            Appearance.ColorDownTo = 16367008
            Appearance.ColorHot = 16117985
            Appearance.ColorHotTo = 16372402
            Appearance.ColorMirrorHot = 16107693
            Appearance.ColorMirrorHotTo = 16775412
            Appearance.ColorMirrorDown = 16102556
            Appearance.ColorMirrorDownTo = 16768988
            Appearance.ColorMirrorChecked = 16102556
            Appearance.ColorMirrorCheckedTo = 16768988
            Appearance.ColorMirrorDisabled = 11974326
            Appearance.ColorMirrorDisabledTo = 15921906
          end
          object DBGrid1: TDBGrid
            Left = 1
            Top = 1
            Width = 995
            Height = 179
            Align = alTop
            Anchors = [akLeft, akTop, akRight, akBottom]
            DataSource = dm.DFinancialNote_Detail
            TabOrder = 5
            TitleFont.Charset = ARABIC_CHARSET
            TitleFont.Color = clWindowText
            TitleFont.Height = -12
            TitleFont.Name = 'Tahoma'
            TitleFont.Style = []
            OnEditButtonClick = DBGrid1EditButtonClick
            Columns = <
              item
                Expanded = False
                FieldName = 'NoteNo'
                Title.Alignment = taCenter
                Title.Caption = #1588#1605#1575#1585#1607' '#1670#1705
                Width = 94
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'Amount'
                Title.Alignment = taCenter
                Title.Caption = #1605#1576#1604#1594' '#1670#1705
                Visible = True
              end
              item
                ButtonStyle = cbsEllipsis
                Expanded = False
                FieldName = 'ForcePaymentID'
                Title.Alignment = taCenter
                Title.Caption = #1588'.'#1602#1587#1591' '#1610#1575' '#1605#1575#1607#1610#1575#1606#1607
                Width = 101
                Visible = True
              end
              item
                ButtonStyle = cbsEllipsis
                Expanded = False
                FieldName = 'FND_LoanNo'
                ReadOnly = True
                Title.Caption = #1588#1605#1575#1585#1607' '#1608#1575#1605
                Visible = True
              end
              item
                Alignment = taLeftJustify
                Expanded = False
                FieldName = 'NoteDate'
                Title.Alignment = taCenter
                Title.Caption = #1578#1575#1585#1610#1582' '#1670#1705
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'NoteDescription'
                Title.Alignment = taCenter
                Title.Caption = #1588#1585#1581' '#1670#1705
                Width = 200
                Visible = True
              end
              item
                ButtonStyle = cbsEllipsis
                Expanded = False
                FieldName = 'BankTitle'
                Title.Alignment = taCenter
                Title.Caption = #1606#1575#1605' '#1576#1575#1606#1705
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'BankAccountNo'
                Title.Alignment = taCenter
                Title.Caption = #1588'.'#1581#1587#1575#1576' '#1576#1575#1606#1705
                Width = 82
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'Branch'
                Title.Alignment = taCenter
                Title.Caption = #1588#1593#1576#1607
                Width = 81
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'BranchCode'
                Title.Alignment = taCenter
                Title.Caption = #1705#1583' '#1588#1593#1576#1607
                Width = 80
                Visible = True
              end>
          end
        end
        object YDBEdit1: TYDBEdit
          Left = 682
          Top = 76
          Width = 240
          Height = 22
          Anchors = [akTop, akRight]
          DataField = 'NoteCount'
          DataSource = dm.DFinancialNote_Master
          Font.Charset = ARABIC_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          TabOrder = 3
        end
        object YDBEdit2: TYDBEdit
          Left = 248
          Top = 38
          Width = 310
          Height = 22
          Anchors = [akTop, akRight]
          DataField = 'Amount'
          DataSource = dm.DFinancialNote_Master
          Font.Charset = ARABIC_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          TabOrder = 4
          OnChange = YDBEdit2Change
        end
        object YDBEdit3: TYDBEdit
          Left = 248
          Top = 0
          Width = 310
          Height = 22
          Anchors = [akTop, akRight]
          DataField = 'Reciver'
          DataSource = dm.DFinancialNote_Master
          Font.Charset = ARABIC_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          TabOrder = 5
        end
        object DBNavigator1: TDBNavigator
          Left = 8
          Top = 138
          Width = 365
          Height = 31
          DataSource = dm.DFinancialNote_Master
          VisibleButtons = [nbFirst, nbPrior, nbNext, nbLast, nbRefresh]
          Flat = True
          TabOrder = 11
          Visible = False
        end
        object BitBtn12: TAdvGlowButton
          Left = 496
          Top = 138
          Width = 149
          Height = 28
          Caption = #1576#1585#1585#1587#1610' '#1605#1576#1604#1594'/'#1578#1593#1583#1575#1583' '#1670#1705
          ImageIndex = 234
          Images = dm.LetterImages_New
          NotesFont.Charset = DEFAULT_CHARSET
          NotesFont.Color = clWindowText
          NotesFont.Height = -11
          NotesFont.Name = 'Tahoma'
          NotesFont.Style = []
          TabOrder = 12
          OnClick = BitBtn12Click
          Appearance.ColorChecked = 16111818
          Appearance.ColorCheckedTo = 16367008
          Appearance.ColorDisabled = 15921906
          Appearance.ColorDisabledTo = 15921906
          Appearance.ColorDown = 16111818
          Appearance.ColorDownTo = 16367008
          Appearance.ColorHot = 16117985
          Appearance.ColorHotTo = 16372402
          Appearance.ColorMirrorHot = 16107693
          Appearance.ColorMirrorHotTo = 16775412
          Appearance.ColorMirrorDown = 16102556
          Appearance.ColorMirrorDownTo = 16768988
          Appearance.ColorMirrorChecked = 16102556
          Appearance.ColorMirrorCheckedTo = 16768988
          Appearance.ColorMirrorDisabled = 11974326
          Appearance.ColorMirrorDisabledTo = 15921906
        end
      end
    end
  end
  object ActionManager_Detail: TActionManager
    ActionBars.SessionCount = 23
    ActionBars = <
      item
        Items = <
          item
            Action = DataSetInsert_Detail
            ImageIndex = 16
            ShortCut = 16429
          end
          item
            Action = DataSetDelete_Detail
            ImageIndex = 17
            ShortCut = 16430
          end
          item
            Action = DataSetEdit_Detail
            ImageIndex = 48
            ShortCut = 115
          end
          item
            Action = DataSetPost_Detail
            ImageIndex = 0
            ShortCut = 113
          end
          item
            Action = DataSetCancel_Detail
            ImageIndex = 56
            ShortCut = 32776
          end>
        AutoSize = False
      end>
    Left = 103
    Top = 352
    StyleName = 'XP Style'
    object DataSetNext_Detail: TDataSetNext
      Category = 'Dataset'
      Caption = #1576#1593#1583#1610
      Hint = 'Next'
      ImageIndex = 76
      ShortCut = 34
      DataSource = dm.DLoanType
    end
    object DataSetPrior_Detail: TDataSetPrior
      Category = 'Dataset'
      Caption = #1602#1576#1604#1610
      Hint = 'Prior'
      ImageIndex = 77
      ShortCut = 33
      DataSource = dm.DLoanType
    end
    object DataSetInsert_Detail: TDataSetInsert
      Caption = #1580#1583#1610#1583
      Hint = 'Insert'
      ImageIndex = 16
      ShortCut = 16429
      DataSource = dm.DFinancialNote_Detail
    end
    object DataSetDelete_Detail: TDataSetDelete
      Caption = #1581#1584#1601
      Hint = 'Delete'
      ImageIndex = 17
      ShortCut = 16430
      OnExecute = DataSetDelete_DetailExecute
      DataSource = dm.DFinancialNote_Detail
    end
    object DataSetEdit_Detail: TDataSetEdit
      Caption = #1608#1610#1585#1575#1610#1588
      Hint = 'Edit'
      ImageIndex = 48
      ShortCut = 115
      OnExecute = DataSetEdit_DetailExecute
      DataSource = dm.DFinancialNote_Detail
    end
    object DataSetPost_Detail: TDataSetPost
      Caption = #1584#1582#1610#1585#1607
      Hint = 'Post'
      ImageIndex = 0
      ShortCut = 113
      OnExecute = DataSetPost_DetailExecute
      DataSource = dm.DFinancialNote_Detail
    end
    object DataSetCancel_Detail: TDataSetCancel
      Caption = #1604#1594#1608
      Hint = 'Cancel'
      ImageIndex = 56
      ShortCut = 32776
      DataSource = dm.DFinancialNote_Detail
    end
    object Action_Detail: TAction
      Caption = #1580#1587#1578#1580#1608
      ShortCut = 114
    end
  end
  object ActionManager_Master: TActionManager
    ActionBars.SessionCount = 23
    ActionBars = <
      item
        Items = <
          item
            Action = DataSetInsert_Master
            ImageIndex = 16
            ShortCut = 16429
          end
          item
            Action = DataSetDelete_Master
            ImageIndex = 17
            ShortCut = 16430
          end
          item
            Action = DataSetEdit_Master
            ImageIndex = 48
            ShortCut = 115
          end
          item
            Action = DataSetPost_Master
            ImageIndex = 0
            ShortCut = 113
          end
          item
            Action = DataSetCancel_Master
            ImageIndex = 56
            ShortCut = 32776
          end>
        AutoSize = False
      end>
    Left = 119
    Top = 264
    StyleName = 'XP Style'
    object DataSetInsert_Master: TDataSetInsert
      Caption = #1580#1583#1610#1583
      Hint = 'Insert'
      ImageIndex = 16
      ShortCut = 16429
      DataSource = dm.DFinancialNote_Master
    end
    object DataSetDelete_Master: TDataSetDelete
      Caption = #1581#1584#1601
      Hint = 'Delete'
      ImageIndex = 17
      ShortCut = 16430
      DataSource = dm.DFinancialNote_Master
    end
    object DataSetEdit_Master: TDataSetEdit
      Caption = #1608#1610#1585#1575#1610#1588
      Hint = 'Edit'
      ImageIndex = 48
      ShortCut = 115
      DataSource = dm.DFinancialNote_Master
    end
    object DataSetPost_Master: TDataSetPost
      Caption = #1584#1582#1610#1585#1607
      Hint = 'Post'
      ImageIndex = 0
      ShortCut = 113
      DataSource = dm.DFinancialNote_Master
    end
    object DataSetCancel_Master: TDataSetCancel
      Caption = #1604#1594#1608
      Hint = 'Cancel'
      ImageIndex = 56
      ShortCut = 32776
      DataSource = dm.DFinancialNote_Master
    end
    object Action_Master: TAction
      Caption = #1580#1587#1578#1580#1608
      ShortCut = 114
    end
    object DataSetNext_Master: TDataSetNext
      Category = 'Dataset'
      Caption = #1576#1593#1583#1610
      Hint = 'Next'
      ImageIndex = 76
      ShortCut = 34
      DataSource = dm.DLoanType
    end
    object DataSetPrior_Master: TDataSetPrior
      Category = 'Dataset'
      Caption = #1602#1576#1604#1610
      Hint = 'Prior'
      ImageIndex = 77
      ShortCut = 33
      DataSource = dm.DLoanType
    end
  end
end
