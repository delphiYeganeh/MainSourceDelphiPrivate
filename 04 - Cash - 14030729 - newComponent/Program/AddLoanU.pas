...
uses
  ...
  XPStyleActnCtrls, Menus, SolarCalendarPackage, AdvGlowButton;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if FrAddLoan.pnlSearch.Enabled then
          SearchEdit.SetFocus;
         nex.Enabled:=False;
        exit;
      end
...
begin
  ...
  Self.Caption := ' «⁄ÿ«Ì Ê«„ --> „—Õ·Â' +' : '+PageControl.ActivePage.Caption ;
end;
...
{     IF NOT  IsValidAccountIdForLoan(Dm.Select_LoanAccountID.AsInteger,Dm.Select_LoanAmount.AsInteger)  THEN
    ...
    begin
      MessageShowString(' ﬂ„ —Ì‰ „ÊÃÊœÌ »—«Ì «Œ ’«’ «Ì‰ ‰Ê⁄ Ê«„ „»·€ '
                       ...
                       + Dm.Select_Account_DetailsExist.AsString + ' „Ì»«‘œ.·ÿ›« «’·«Õ ‰„«ÌÌœ ', False,'«ÿ·«⁄ —”«‰Ì');
      Exit;
    end;
    ...
    begin
      ...
      begin
        MessageShowString(IntToStr(PercentOfPaid)+' œ—’œ «“ «ﬁ”«ÿ Å—œ«Œ  ‘œÂ «” .œ— ’Ê— ÌòÂ Õœ«ﬁ·'+Dm.LoanTypeMinLastPartPercent.AsString+' œ—’œ «“ ò· ﬁ”ÿ »«Ìœ Å—œ«Œ  ‘Êœ ' , False,'«ÿ·«⁄ —”«‰Ì');
        Exit;
      end;
    end;
  end;
...
begin
  ...
  case NewPage of
    1: with dm do
        ...
        begin
          ...
          ShowMessage('«» œ« ‘„«—Â Õ”«» Ê«„ êÌ—‰œÂ —«  Ê”ÿ œò„Â Â«Ì "«‰ Œ«» «“ —ÊÌ «Ê·ÊÌ "  Ì«  "ﬁ—⁄Â ò‘Ì"  «‰ Œ«» ‰„«ÌÌœ');
          if FrAddLoan.pnlSearch.Enabled then
            SearchEdit.SetFocus;
          Result:=False;
        end
    ...
    3: with dm do
        begin
          ...
          if (Select_LoanAmount.AsLargeInt<>Get_SumPartAmount(Select_LoanLoanID.AsInteger)) and
            (YDBGrid1.DataSource.DataSet.RecordCount = 0) then
          begin
            ShowMessage('·ÿ›« ﬁ”ÿ »‰œÌ Ê«„ «‰Ã«„ œÂÌœ');
            Result:=false;
          end
          else
  ...
  begin
    ...
    if not prev.Visible then
      Nex.Left := prev.left
    else  Nex.Left := prev.left  - 90 ;
  end;
...
procedure TFrAddLoan.PageControlChange(Sender: TObject);
begin
  inherited;
//
  Self.Caption := ' «⁄ÿ«Ì Ê«„ --> „—Õ·Â' +' : '+PageControl.ActivePage.Caption ;
