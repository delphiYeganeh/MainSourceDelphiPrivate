...
uses
  ...
  ComCtrls, ExtCtrls, QuickRpt, Menus,QRCtrls, Grids;

type
  TFrForTest = class(TForm)
...
procedure TFrForTest.AExitExecute(Sender: TObject);
begin
   Close;
end;

procedure TFrForTest.FormClose(Sender: TObject; var Action: TCloseAction);
begin
   Action:=caFree;
end;

procedure TFrForTest.TEditExit(Sender: TObject);
Var
   D:String;
begin
...
procedure TFrForTest.CurrencyKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
   inherited;
...
procedure TFrForTest.FormCreate(Sender: TObject);
 var i:integer;
begin
 OpenSp_ForTest;
...
procedure TFrForTest.GotoNext(Sender: TObject; var key: Word; Shift: TShiftState);
var
   i: integer;
begin
...
procedure TFrForTest.TEditEnter(Sender: TObject);
begin
...
procedure TFrForTest.FormShow(Sender: TObject);
var i: integer;
begin
...
procedure TFrForTest.searchAccountExecute(Sender: TObject);
begin
...
procedure TFrForTest.AExportToExcelExecute(Sender: TObject);
begin
 if ActiveControl.ClassNameIs('TYDBGrid') then
   TYDBGrid(ActiveControl).ExportToExcel;
end;

procedure TFrForTest.SearchLoanExecute(Sender: TObject);
begin
...
procedure TFrForTest.ShowMsgString(AppMessage: string);
begin
...
Function TFrForTest.MessageShowString_WithExit(AppMessage: string):byte;
begin
  ShowMessageF:=TShowMessageF.Create(self);
  ShowMessageF.Label1.Caption:=AppMessage;
...
Function TFrForTest.MessageShowString(AppMessage: string;ShowCancel:boolean):boolean;
begin
  ShowMessageF:=TShowMessageF.Create(self);
  ShowMessageF.Label1.Caption:=AppMessage;
...
procedure TFrForTest.wordMenuItemClick(Sender: TObject);
begin
 if ActiveControl.ClassNameIs('TYDBGrid') then
...
procedure TFrForTest.ExcelMenuItemClick(Sender: TObject);
begin
 if ActiveControl.ClassNameIs('TYDBGrid') then
   TYDBGrid(ActiveControl).ExportToExcel;
...
procedure TFrForTest.TextMenuItemClick(Sender: TObject);
begin
 if ActiveControl.ClassNameIs('TYDBGrid') then
...
procedure TFrForTest.PrintMenuItemClick(Sender: TObject);
begin
 if ActiveControl.ClassNameIs('TYDBGrid') then
...
procedure TFrForTest.CustomMenuItemClick(Sender: TObject);
begin
 if ActiveControl.ClassNameIs('TYDBGrid') then
