...
uses
  ...
  ComCtrls, ADODB, Mask, AdvGlowButton;
...
type
  TFrSystemSetting = class(TYBaseForm)
    ...
    GroupBox7: TGroupBox;
    SBDBGridFont: TAdvGlowButton;
    ChBoEnterDataFont: TCheckBox;
    SBEnterDataFont: TAdvGlowButton;
...
begin
   ...
   Try
     if  Get_UserSetting(_Userid,'rgModeColor') = '' then
      rgModeColor.ItemIndex := 0
     else rgModeColor.ItemIndex := StrToInt(Get_UserSetting(_Userid,'rgModeColor'));
   Except
   End;
   //---
...
begin
  ...
  IF (Get_SystemSetting('EdtVosoulAccount') <>EdtVosoulAccount.Text) AND
   ...
   Set_UserSetting(_Userid,'rgModeColor',IntToStr( rgModeColor.itemindex) );
