...
var
  FrAddReportName: TFrAddReportName;
...
procedure TFrAddReportName.AExitExecute(Sender: TObject);
begin
   Close;
end;

procedure TFrAddReportName.FormClose(Sender: TObject; var Action: TCloseAction);
begin
   Action:=caFree;
end;

procedure TFrAddReportName.TEditExit(Sender: TObject);
Var
   D:String;
begin
...
procedure TFrAddReportName.CurrencyKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
   inherited;
...
procedure TFrAddReportName.FormCreate(Sender: TObject);
 var i:integer;
begin
...
procedure TFrAddReportName.GotoNext(Sender: TObject; var key: Word; Shift: TShiftState);
var
   i: integer;
begin
...
procedure TFrAddReportName.searchAccountExecute(Sender: TObject);
begin
...
procedure TFrAddReportName.AExportToExcelExecute(Sender: TObject);
begin
 if ActiveControl.ClassNameIs('TYDBGrid') then
   TYDBGrid(ActiveControl).ExportToExcel;
end;

procedure TFrAddReportName.SearchLoanExecute(Sender: TObject);
begin
...
procedure TFrAddReportName.ShowMsgString(AppMessage: string);
begin
...
Function TFrAddReportName.MessageShowString_WithExit(AppMessage: string):byte;
begin
  ShowMessageF:=TShowMessageF.Create(self);
  ShowMessageF.Label1.Caption:=AppMessage;
...
Function TFrAddReportName.MessageShowString(AppMessage: string;ShowCancel:boolean):boolean;
begin
  ShowMessageF:=TShowMessageF.Create(self);
  ShowMessageF.Label1.Caption:=AppMessage;
...
procedure TFrAddReportName.wordMenuItemClick(Sender: TObject);
begin
 if ActiveControl.ClassNameIs('TYDBGrid') then
...
procedure TFrAddReportName.ExcelMenuItemClick(Sender: TObject);
begin
 if ActiveControl.ClassNameIs('TYDBGrid') then
   TYDBGrid(ActiveControl).ExportToExcel;
...
procedure TFrAddReportName.TextMenuItemClick(Sender: TObject);
begin
 if ActiveControl.ClassNameIs('TYDBGrid') then
...
procedure TFrAddReportName.PrintMenuItemClick(Sender: TObject);
begin
 if ActiveControl.ClassNameIs('TYDBGrid') then
...
procedure TFrAddReportName.CustomMenuItemClick(Sender: TObject);
begin
 if ActiveControl.ClassNameIs('TYDBGrid') then
...
procedure TFrAddReportName.DBGrid1EditButtonClick(Sender: TObject);
begin
    ...
    Begin
       FrListofObjects:=TFrListofObjects.Create(NIL);
       FrListofObjects.ShowModal;
       IF FrListofObjects.ModalResult=mrok then
         Dm.ReportsNameObjectName.AsString:=dm.ObjectsObjectName.AsString
       ELSE
          Dm.ReportsNameObjectName.AsString:='';
    END;
    ...
    Begin
          if OpenDialog.Execute then
             dm.ReportsNameReportName.AsString:= ExtractFileName(OpenDialog.FileName);
    END;
end;
...
procedure TFrAddReportName.FormCanResize(Sender: TObject; var NewWidth,
  NewHeight: Integer; var Resize: Boolean);
begin
  { TODO -oparsa : 14030628- bug369 }
