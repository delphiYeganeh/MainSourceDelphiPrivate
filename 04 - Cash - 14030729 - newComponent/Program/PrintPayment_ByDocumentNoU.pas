...
begin
   ...
   begin
      ...
      Open;
      pnlDesc.caption := ' ⁄œ«œ «”‰«œ = '+ inttostr(dm.Report_Payment_ByDocumentNo.RecordCount ) ;
   end;
...
begin
  inherited;
 // if ( not YDBGrid.DataSource.DataSet.Active) or (YDBGrid.DataSource.DataSet.RecordCount = 0) then
  if ( not dm.Report_Payment_ByDocumentNo.Active) or (dm.Report_Payment_ByDocumentNo.RecordCount = 0) then
    Exit;
...
begin
   ...
   if not Date_CheckTwoDate(FromDate.Text,ToDate.Text) then
      Exit;
...
begin
  ...
  _ListDeletedPaymentId := '' ;
  if YDBGrid.SelectedRows.Count>0 then
    msgstr  := '¬Ì« «“ À»  ”‰œ „⁄òÊ” '+ IntToStr(YDBGrid.SelectedRows.Count) + ' ”‰œ «‰ Œ«»Ì „ÿ„⁄‰ „Ì »«‘Ìœ ø '
  else  msgstr := ' ¬Ì« «“ À»  ”‰œ „⁄òÊ” »—«Ì ”‰œ Ã«—Ì „ÿ„⁄‰ „Ì »«‘Ìœø' ;
    ...
    begin
      ...
      begin
          ...
          begin
            ...
            _ListDeletedPaymentId:=_ListDeletedPaymentId+inttostr(_PaymentId)+',';
          end  
          else
    ...
    begin
      ...
      begin
       ...
       // ShowMessage('Õ–› »« „Ê›ﬁÌ  «‰Ã«„ ‘œ');
        ShowMessage('”‰œ „⁄òÊ” »« „Ê›ﬁÌ  ’«œ— ‘œ');
      end
      else
        ShowMessage('”‰œ „⁄òÊ” ﬁ»·« ’«œ— ‘œÂ «”  «„ò«‰ Õ–› ÊÃÊœ ‰œ«—œ.');
    end;

    if (trim(_ListNotDeletedPaymentId)<>'') and (trim(_ListDeletedPaymentId)<>'') then
      ShowMessage('”‰œ „⁄òÊ” »—«Ì òœ Â«Ì '+_ListNotDeletedPaymentId+' ’«œ— ‘œÂ «”  «„ò«‰ Õ–› ÊÃÊœ ‰œ«—œ.' +'('+' ”‰œ „⁄òÊ”  »—«Ì òœÂ«Ì '+_ListDeletedPaymentId+' «‰Ã«„ ‘œ. '+')' )
    else
    if (trim(_ListNotDeletedPaymentId)<>'') and (trim(_ListDeletedPaymentId)='') then
      ShowMessage('”‰œ „⁄òÊ” »—«Ì òœ Â«Ì '+_ListNotDeletedPaymentId+' ’«œ— ‘œÂ «”  «„ò«‰ Õ–› ÊÃÊœ ‰œ«—œ.')
    else
    if (trim(_ListNotDeletedPaymentId)='') and (trim(_ListDeletedPaymentId)<>'') then
      ShowMessage(' ”‰œ „⁄òÊ”  »—«Ì òœÂ«Ì '+_ListDeletedPaymentId+' «‰Ã«„ ‘œ. ');
...
begin
  ...
  if (messageShowString('¬Ì« «“ Õ–› «”‰«œ Ã„⁄Ì Ê Ê«»” Â »Â ”‰œ Ã«—Ì „ÿ„⁄‰ „Ì »«‘Ìœø',true, '«Œÿ«—')) then
  begin
    //Hamed_Ansari_990623_S
    //_PaymentId:=DM.Report_Payment_ByDocumentNo.FieldByName('PaymentID').AsInteger;
    //_DelDate:=DM.Report_Payment_ByDocumentNo.FieldByName('Date').AsString;
    ...
    _DelDate:=ADOQDoclist.FieldByName('Date').AsString;
    //Hamed_Ansari_990623_E
    if (messageShowString('  ⁄œ«œ «”‰«œ Ê«»” Â  '+INTTOSTR((Count_PAYMENT(_PaymentId,_DelDate)))+ '  « «” .¬Ì«  „«Ì· »Â Õ–› œ«—Ìœø ',true, '«ÿ·«⁄ —”«‰Ì'))  THEN
