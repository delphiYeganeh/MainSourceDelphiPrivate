...
type
 ...
 TDblclickAction=(viewLetter,ViewJpg,ViewWord,ViewPdf);
  TMainForm = class(TMBaseForm)
    ...
    Action22: TAction;
    Action23: TAction;
    zist_Archive: TAction;
    zist_Archive2: TAction;
...
Procedure SetShowFollowUp(Value: boolean);
    ...
    procedure zist_ArchiveExecute(Sender: TObject);
    procedure zist_Archive2Execute(Sender: TObject);
...
uses Dmu,  FixedTableF, EnteredLetterInputF, ExitedLetterInputF,
  ...
  UFollowUp, DateUtils, FrReport_scan, FieldAccessU, FrCity, FrEmployer,
  FrFileKeeping, FrResearch;
   ...
   for i:=0 to Count-1 do
      if UpperCase(Columns[i].FieldName)='SENDERTITLE'  then
       case value of
         ...
         2:Columns[i].Title.Caption:='êÌ—‰œÂ';
         3:Columns[i].Visible:=false;
       end
     else
      if UpperCase(Columns[i].FieldName)='DELIVERTITLE' then 
       case value of
         1:Columns[i].Title.Caption:='êÌ—‰œÂ';
         2:Columns[i].Title.Caption:='›—” ‰œÂ';
         3:Columns[i].Title.Caption:='«ÌÃ«œ ò‰‰œÂ'
       end;
 ...
 if not _AccessToClassifieldLetter then
   if w<>'' then
     ...
     if SearchI_All_Year_and_Sec then
      dm.ExecGet_All_Letter(ArchiveStatus, archiveFolderID,Letter_Type,LetterFormat,0,0,strtoint(SpinEdit.Text),w)
     else
      dm.ExecGet_All_Letter(ArchiveStatus, archiveFolderID,Letter_Type,LetterFormat,dm.CurrentMYear,dm.SecID,strtoint(SpinEdit.Text),w);
...
begin
  ...
  if r=3 then DblclickAction:=ViewPdf ;
end;
  ...
  if dm.Get_All_LetterArchived.AsBoolean then
   begin
    if messageShowString('¬Ì« «“  «‰ ﬁ«· ‰«„Â »Â ÅÊ‘Â ‰«„Â Â«Ì Ã«—Ì „ÿ„∆‰ Â” Ìœ',True) then
     Exec_Update_Archived_letter(dm.Get_All_LetterLetterID.AsInteger,1)
   end
  else
   begin
    ...
    if _selectedLettersID.Count>0  then
     for i:=0 to _selectedLettersID.Count-1 do
       Exec_Update_Archived_letter(strtoint(_selectedLettersID[i]),4)
      ELSE
       Exec_Update_Archived_letter(dm.Get_All_LetterLetterID.AsInteger,4);
     end ;
  RefreshQuery;
end;
...
begin
  inherited;
CityFrm:=TCityFrm.Create(Application);
CityFrm.ShowModal;
end;
...
begin
  inherited;
  EmployerFrm:=TEmployerFrm.Create(Application);
  EmployerFrm.Selectbtn.Hide;
  EmployerFrm.ShowModal;
end;

procedure TMainForm.Action22Execute(Sender: TObject);
begin
  inherited;
FileKeepingFrm:=TFileKeepingFrm.Create(Application);
FileKeepingFrm.Selectbtn.Hide;
FileKeepingFrm.ShowModal;
end;

procedure TMainForm.Action23Execute(Sender: TObject);
begin
  inherited;
ResearchFrm:=TResearchFrm.Create(Application);
ResearchFrm.ShowModal;
end;

procedure TMainForm.zist_ArchiveExecute(Sender: TObject);
begin
  inherited;
  FileKeepingFrm:=TFileKeepingFrm.Create(Application);
  FileKeepingFrm.LetterID:=dm.Get_All_LetterLetterID.AsInteger;
  FileKeepingFrm.ShowModal;
  ArchiveFolders.Text:=Exec_sp_ArchivePlaces(dm.Get_All_LetterLetterID.AsInteger,0);

end;

procedure TMainForm.zist_Archive2Execute(Sender: TObject);
begin
  inherited;
  EmployerFrm:=TEmployerFrm.Create(Application);
  EmployerFrm.LetterID:=dm.Get_All_LetterLetterID.AsInteger;
  EmployerFrm.ShowModal;
  ArchiveFolders.Text:=Exec_sp_ArchivePlaces(dm.Get_All_LetterLetterID.AsInteger,0);
