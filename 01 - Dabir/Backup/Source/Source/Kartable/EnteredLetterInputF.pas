...
uses  YShamsiDate, FromOrgU,   QuickSearch, UMain,
  ...
  UInputUserTable, ShowSaderehUnit;
...
begin
   ...
   begin
      ShowMyMessage('íÛÇã','ßÇÑÈÑ ÑÇãí ¡ áØÝÇ ÇÈÊÏÇ ÇØáÇÚÇÊ ÎæÏ ÑÇ ÐÎíÑå äãÇííÏ',[mbOK],mtInformation);
      Exit;
   end;
...
begin
   ...
   begin
      MBaseForm.MessageShowString(' åí ãÞÏÇÑí ÏÑ ÞÓãÊ ' + dm.UserMemoDisplay +' íÇÝÊ äÔÏ  ', False);
      Exit;
   end;
...
begin
   ...
   begin
      messageShowString(' ÞÈáÇ Èå Çíä äÇãå ÝÑã íæÓÊ ÔÏå ÇÓÊ ',false);
      Exit;
   end;
...
begin
   ...
   ExactCopyF.TypeId := 1;//ÑæäæÔÊ
   ExactCopyF.Caption := 'ÑæäæÔÊ';
...
begin
   ...
   begin
       ...
       begin
         ...
         ShowMsgString('ÈÑÇí Çíä ãæÑÏ - ãÔÊÑí ËÈÊ äÔÏå ÇÓÊ');

       frmCustomerInfo.Free;
       frmCustomerInfo:=nil;
   end
   else
       ShowMessage('äÇã ÈÇä˜ ÇØáÇÚÇÊí ÓíÓÊã ãÏíÑíÊ ÑæÇÈØ ãÔÊÑí ãÔÎÕ äÔÏå ÇÓÊ');
...
begin
  ...
  if Select_Letter.State in [dsInsert,dsEdit] then
     ...
     ShowMsgString('ÈÇíÏ ÏÑ ÍÇáÊ æíÑÇíÔ íÇ ÏÑÌ ÇØáÇÚÇÊ ÈÇÔíÏ');
...
begin
  inherited;
  if messageShowString('ÂíÇ ÇÒ ÍÐÝ ÝÑã ãØãíä åÓÊíÏ¿',true) then
  begin
      QDelForms.Close;
      ...
      QAddedForms.Open;
      ShowCount;
      ShowMsgString('ÚãáíÇÊ ÈÇ ãæÝÞíÊ ÇäÌÇã ÔÏ');
  end;
end;
...
begin
  ...
  begin
      ...
      begin
        ...
        lblHasForms.Caption:='Çíä äÇãå ÏÇÑÇí ÝÑã íæÓÊ ÔÏå ãí ÈÇÔÏ';
      end
      else
      begin
        lblHasForms.Font.Color:=clMaroon;
        lblHasForms.Caption:='Çíä äÇãå ÏÇÑÇí ÝÑã íæÓÊ ÔÏå äãí ÈÇÔÏ';
      end;
  end;
end;
...
begin
 lblCount.Caption:=' ÊÚÏÇÏ ' +IntToStr(YDBGrid1.DataSource.DataSet.RecordCount)+' ãæÑÏ íÇÝÊ ÔÏ ';
end;
