inherited FrLetterNoSetting: TFrLetterNoSetting
  Left = 907
  Top = 158
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = #1578#1606#1592#1610#1605' '#1588#1605#1575#1585#1607' '#1606#1575#1605#1607' '#1607#1575
  ClientHeight = 349
  ClientWidth = 270
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel [0]
    Left = 0
    Top = 308
    Width = 270
    Height = 41
    Align = alBottom
    TabOrder = 0
    DesignSize = (
      270
      41)
    object BitBtn2: TBitBtn
      Left = 104
      Top = 8
      Width = 75
      Height = 25
      Cursor = crHandPoint
      Anchors = [akTop, akRight]
      Cancel = True
      Caption = #1575#1606#1589#1585#1575#1601
      Font.Charset = ARABIC_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      OnClick = BitBtn2Click
      Glyph.Data = {
        
        8000808000808000808000808000}
    end
    object BitBtn1: TBitBtn
      Left = 184
      Top = 8
      Width = 75
      Height = 25
      Cursor = crHandPoint
      Anchors = [akTop, akRight]
      Caption = #1578#1575#1610#1610#1583
      Default = True
      Font.Charset = ARABIC_CHARSET
      Font.Color = clNavy
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      OnClick = BitBtn1Click
      Glyph.Data = {
        
        8000808000FF00008080}
    end
  end
  object GroupBox1: TGroupBox [1]
    Left = 0
    Top = 0
    Width = 270
    Height = 308
    Align = alClient
    Caption = #1578#1593#1610#1610#1606' '#1588#1605#1575#1585#1607' '#1606#1575#1605#1607
    TabOrder = 1
    DesignSize = (
      270
      308)
    object Label1: TLabel
      Left = 172
      Top = 99
      Width = 59
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1602#1587#1605#1578' '#1575#1608#1604' :'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label2: TLabel
      Left = 172
      Top = 127
      Width = 61
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1602#1587#1605#1578' '#1583#1608#1605' :'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label3: TLabel
      Left = 172
      Top = 155
      Width = 67
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1602#1587#1605#1578' '#1587#1608#1605' :'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label4: TLabel
      Left = 172
      Top = 184
      Width = 71
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1602#1587#1605#1578' '#1670#1607#1575#1585#1605' :'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label5: TLabel
      Left = 172
      Top = 212
      Width = 66
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1602#1587#1605#1578' '#1662#1606#1580#1605' :'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label6: TLabel
      Left = 172
      Top = 241
      Width = 75
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1602#1587#1605#1578' '#1588#1588#1605' :'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label7: TLabel
      Left = 172
      Top = 272
      Width = 87
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1580#1583#1575#1705#1606#1606#1583#1607' '#1602#1587#1605#1578#1607#1575' :'
      Font.Charset = ARABIC_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
    end
    object Label8: TLabel
      Left = 173
      Top = 69
      Width = 43
      Height = 13
      Caption = #1583#1576#1610#1585#1582#1575#1606#1607' :'
    end
    object Indicator2: TComboBox
      Left = 23
      Top = 124
      Width = 145
      Height = 21
      Anchors = [akTop, akRight]
      BiDiMode = bdLeftToRight
      ItemHeight = 13
      ParentBiDiMode = False
      TabOrder = 0
      Text = #1582#1575#1604#1610
    end
    object Indicator3: TComboBox
      Left = 23
      Top = 152
      Width = 145
      Height = 21
      Anchors = [akTop, akRight]
      BiDiMode = bdLeftToRight
      ItemHeight = 13
      ParentBiDiMode = False
      TabOrder = 1
      Text = #1582#1575#1604#1610
    end
    object Indicator4: TComboBox
      Left = 23
      Top = 181
      Width = 145
      Height = 21
      Anchors = [akTop, akRight]
      BiDiMode = bdLeftToRight
      ItemHeight = 13
      ParentBiDiMode = False
      TabOrder = 2
      Text = #1582#1575#1604#1610
    end
    object Indicator5: TComboBox
      Left = 23
      Top = 209
      Width = 145
      Height = 21
      Anchors = [akTop, akRight]
      BiDiMode = bdLeftToRight
      ItemHeight = 13
      ParentBiDiMode = False
      TabOrder = 3
      Text = #1582#1575#1604#1610
    end
    object Indicator6: TComboBox
      Left = 23
      Top = 238
      Width = 145
      Height = 21
      Anchors = [akTop, akRight]
      BiDiMode = bdLeftToRight
      ItemHeight = 13
      ParentBiDiMode = False
      TabOrder = 4
      Text = #1582#1575#1604#1610
    end
    object Indicator1: TComboBox
      Left = 23
      Top = 96
      Width = 145
      Height = 21
      Anchors = [akTop, akRight]
      BiDiMode = bdLeftToRight
      ItemHeight = 13
      ParentBiDiMode = False
      TabOrder = 5
      Text = #1582#1575#1604#1610
    end
    object MaskEdit1: TEdit
      Left = 132
      Top = 269
      Width = 36
      Height = 21
      Anchors = [akTop, akRight]
      MaxLength = 1
      TabOrder = 6
      Text = '|'
      OnKeyPress = MaskEdit1KeyPress
    end
    object DBLookupComboBox1: TDBLookupComboBox
      Left = 23
      Top = 66
      Width = 144
      Height = 21
      BiDiMode = bdLeftToRight
      KeyField = 'SecId'
      ListField = 'SecTitle'
      ListSource = dUserSecs
      ParentBiDiMode = False
      TabOrder = 7
    end
    object RadioGroup1: TRadioGroup
      Left = 16
      Top = 16
      Width = 241
      Height = 41
      Caption = #1606#1608#1593' '#1606#1575#1605#1607
      Columns = 5
      Items.Strings = (
        #1607#1605#1607
        #1608#1575#1585#1583#1607
        #1589#1575#1583#1585#1607
        #1583#1575#1582#1604#1610
        #1605#1583#1585#1705)
      TabOrder = 8
      OnClick = RadioGroup1Click
    end
  end
  inherited DSForm: TDataSource
    Left = 8
    Top = 296
  end
  inherited ActionList: TActionList
    Left = 8
    Top = 264
  end
  inherited Grid_PopupMenu: TPopupMenu
    Left = 68
    Top = 296
  end
  inherited Timer1sssss: TTimer
    Left = 104
    Top = 264
  end
  object dUserSecs: TDataSource
    DataSet = Dm.UserSecs
    OnDataChange = dUserSecsDataChange
    Left = 72
    Top = 264
  end
  object QUpdLetterFormula: TADOQuery
    Connection = Dm.YeganehConnection
    Parameters = <>
    Left = 40
    Top = 296
  end
  object QReadLetterFormula: TADOQuery
    Connection = Dm.YeganehConnection
    Parameters = <
      item
        Name = 'SecID'
        Attributes = [paSigned, paNullable]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = 0
      end
      item
        Name = 'letterTypeID'
        DataType = ftInteger
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'SELECT * FROM LetterFormula'
      'WHERE SecID=:SecID'
      'AND letterTypeID=:letterTypeID')
    Left = 40
    Top = 264
    object QReadLetterFormulaID: TAutoIncField
      FieldName = 'ID'
      ReadOnly = True
    end
    object QReadLetterFormulaSecID: TIntegerField
      FieldName = 'SecID'
    end
    object QReadLetterFormulaLetterFormulaPart1: TStringField
      FieldName = 'LetterFormulaPart1'
      Size = 50
    end
    object QReadLetterFormulaLetterFormulaPart2: TStringField
      FieldName = 'LetterFormulaPart2'
      Size = 50
    end
    object QReadLetterFormulaLetterFormulaPart3: TStringField
      FieldName = 'LetterFormulaPart3'
      Size = 50
    end
    object QReadLetterFormulaLetterFormulaPart4: TStringField
      FieldName = 'LetterFormulaPart4'
      Size = 50
    end
    object QReadLetterFormulaLetterFormulaPart5: TStringField
      FieldName = 'LetterFormulaPart5'
      Size = 50
    end
    object QReadLetterFormulaLetterFormulaPart6: TStringField
      FieldName = 'LetterFormulaPart6'
      Size = 50
    end
    object QReadLetterFormulaLetterFormulaSeperator: TStringField
      FieldName = 'LetterFormulaSeperator'
      Size = 50
    end
  end
  object ado_Count: TADOQuery
    Connection = Dm.YeganehConnection
    Parameters = <
      item
        Name = 'SecID'
        Attributes = [paSigned, paNullable]
        DataType = ftInteger
        Precision = 10
        Size = 4
        Value = 0
      end
      item
        Name = 'letterTypeID'
        DataType = ftInteger
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'SELECT COUNT(*) AS Cnt '
      'FROM LetterFormula'
      'WHERE SecID=:SecID'
      'AND letterTypeID=:letterTypeID')
    Left = 200
    Top = 264
    object ado_CountCnt: TIntegerField
      FieldName = 'Cnt'
    end
  end
  object LetterFormula_inser: TADOStoredProc
    Connection = Dm.YeganehConnection
    CommandTimeout = 0
    ProcedureName = 'PRC_LetterFormula_Insert'
    Parameters = <
      item
        Name = '@SecID'
        DataType = ftInteger
        Value = Null
      end
      item
        Name = '@LetterFormulaPart1'
        DataType = ftString
        Size = 3
        Value = Null
      end
      item
        Name = '@LetterFormulaPart2'
        DataType = ftString
        Size = 3
        Value = Null
      end
      item
        Name = '@LetterFormulaPart3'
        DataType = ftString
        Size = 3
        Value = Null
      end
      item
        Name = '@LetterFormulaPart4'
        DataType = ftString
        Size = 3
        Value = Null
      end
      item
        Name = '@LetterFormulaPart5'
        DataType = ftString
        Size = 3
        Value = Null
      end
      item
        Name = '@LetterFormulaPart6'
        DataType = ftString
        Size = 3
        Value = Null
      end
      item
        Name = '@LetterFormulaSeperator'
        DataType = ftString
        Size = 3
        Value = Null
      end
      item
        Name = '@UseForWord'
        DataType = ftString
        Size = 3
        Value = Null
      end
      item
        Name = '@letterTypeID'
        DataType = ftInteger
        Value = Null
      end>
    Left = 200
    Top = 232
    object LetterFormula_inserID: TIntegerField
      FieldName = 'ID'
    end
    object LetterFormula_inserSecID: TIntegerField
      FieldName = 'SecID'
    end
    object LetterFormula_inserLetterFormulaPart1: TStringField
      FieldName = 'LetterFormulaPart1'
    end
    object LetterFormula_inserLetterFormulaPart2: TStringField
      FieldName = 'LetterFormulaPart2'
    end
    object LetterFormula_inserLetterFormulaPart3: TStringField
      FieldName = 'LetterFormulaPart3'
    end
    object LetterFormula_inserLetterFormulaPart4: TStringField
      FieldName = 'LetterFormulaPart4'
      Size = 0
    end
    object LetterFormula_inserLetterFormulaPart5: TStringField
      FieldName = 'LetterFormulaPart5'
    end
    object LetterFormula_inserLetterFormulaPart6: TStringField
      FieldName = 'LetterFormulaPart6'
    end
    object LetterFormula_inserLetterFormulaSeperator: TStringField
      FieldName = 'LetterFormulaSeperator'
    end
    object LetterFormula_inserUseForWord: TStringField
      FieldName = 'UseForWord'
    end
    object LetterFormula_inserletterTypeID: TIntegerField
      FieldName = 'letterTypeID'
    end
  end
end
