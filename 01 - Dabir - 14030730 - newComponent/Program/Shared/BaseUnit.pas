...
Uses
  Dmu,UsearchStno,BusinessLayer,UsearchShortCut,ShowmessageU
...
begin
  ...
  if Sender.ClassNameIs('TMemo') then
  begin
...
begin
  ...
  begin
    ...
    else if Components[i].InheritsFrom(TCheckBox) then
    begin
      TCheckBox(Components[i]).Enabled := dm.GetActionAccess(TCheckBox(Components[i]).Name, Self.Tag);
    end;
  end;
end;
...
begin
   ...
   begin
    //if Components[I] is TADOQuery then
     //TADOQuery(Components[I]).CommandTimeout := 0
    //else
    //if Components[I] is TADOTable then
     //TADOTable(Components[I]).CommandTimeout := 0
    //else
    //if Components[I] is TADOStoredProc then
     //TADOStoredProc(Components[I]).CommandTimeout := 0
   //else
...
//   Exec_delete_Actions_ByFormID(self.Tag);
  ...
  begin
    ...
    begin
      ...
      begin
        Exec_insert_Actions(TCheckBox(Components[i]).Name,self.Tag,TCheckBox(Components[i]).Caption,Components[i].tag);
      end;
    end;
  end;

{    if Components[i].InheritsFrom(TAction) then//or Components[i].InheritsFrom(TCheckBox) then
      if Components[i].tag>=0 then
      begin
        if Components[i].ClassNameIs('TCheckBox') then
        begin
          if (TAction(Components[i]).Name = '') then
                   lkkl
        end;
