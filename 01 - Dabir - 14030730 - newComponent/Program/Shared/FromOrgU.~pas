...
begin
  ...
  begin
    ShowMyMessage('ÅÌ€«„','ﬁ«»·Ì  œ—Ã «Ì‰ ”ÿÕ ÊÃÊœ ‰œ«—œ',[mbOK],mtInformation);
    Exit;
  end;
...
begin
  ...
  { TODO -oparsa : 14030627 }
  if PageControl.ActivePageIndex = 1 then
    SelectedNodeID := FromOrgID.AsInteger
  else
  { TODO -oparsa : 14030627 }
  ...
  begin
    ShowMyMessage('ÅÌ€«„','”«“„«‰ „Ê—œ ‰Ÿ— ﬁ«»·Ì  Ã«»Â Ã«ÌÌ ‰œ«—œ',[mbOK],mtInformation);
    exit;
  end;
  ...
  { TODO -oparsa : 14030627 }
  if PageControl.ActivePageIndex = 1 then
    SelectedNodeTitle := FromOrgTitle.AsString
  else
  { TODO -oparsa : 14030627 }
...
begin
  ...
  if PageControl.ActivePageIndex = 1 then
  begin
    if RBInnerOrg.Checked then
      SelectedNodeID := 2 ; //  ç«—  ”«“„«‰Ì

    if RBOuterOrg.Checked then
      SelectedNodeID := 4 ;//œ»Ì—Œ«‰Â Â«Ì œÌê—
  end
  else
  { TODO -oparsa : 14030627 }
...
begin
  if PageControl.ActivePageIndex = 0 then  //œ—Œ Ì
  begin
