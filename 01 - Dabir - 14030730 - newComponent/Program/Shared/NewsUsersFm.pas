...
begin
  ...
  begin
     ...
     begin
        ...
        MBaseForm.messageShowString('Èå Çíä ÎÈÑ åí ÝÇíáí íæÓÊ äÔÏå ÇÓÊ', False);
end;
...
begin
  Result := False;
  {
  if Pos('ÏÚæÊ Èå ÌáÓå ÈÇ ˜Ï' ,DMNewsUsers.QrNewsMemo.AsString)>0 then Result:=True;
  if Pos('ÏÚæÊ Èå ãáÇÞÇÊ ÈÇ ˜Ï' ,DMNewsUsers.QrNewsMemo.AsString)>0 then Result:=True;
  if Pos('ÏÚæÊ Èå ÇäÌÇã ˜ÇÑ ÈÇ ˜Ï' ,DMNewsUsers.QrNewsMemo.AsString)>0 then Result:=True;
  }
  if (Pos('ÏÚæÊ Èå ÌáÓå ÈÇ ˜Ï' ,DMNewsUsers.QrNewsMemo.AsString)>0) or
     (Pos('ÏÚæÊ Èå ãáÇÞÇÊ ÈÇ ˜Ï' ,DMNewsUsers.QrNewsMemo.AsString)>0) or
     (Pos('ÏÚæÊ Èå ÇäÌÇã ˜ÇÑ ÈÇ ˜Ï' ,DMNewsUsers.QrNewsMemo.AsString)>0) then
    Result:=True;
