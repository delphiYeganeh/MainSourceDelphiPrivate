...
begin
 ...
 // OrgTree.Height := 0;
 // Panel4.Height := Panel4.Height - 170 ;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        //œ” —”Ì »Â ‰«„Â Â«Ì „Õ—„«‰Â   œ— ›—„ ÌÊ“—
        blnGetHasSecureLetterAccess := GetHasSecureLetterAccess(Dm.Select_LetterReCommiteOrgID.AsInteger);

        { TODO -oparsa : 14030721 }
        CheckWorkGroupAccess :=  CheckWorkGroup(Dm.Select_LetterReCommiteOrgID.AsInteger) ;
        DescriptionStr := '';
        //if ( not CheckWorkGroup(Dm.Select_LetterReCommiteOrgID.AsInteger) ) or
        if ( not CheckWorkGroupAccess ) or
        { TODO -oparsa : 14030721 }
           //(not GetHasSecureLetterAccess(Dm.Select_LetterReCommiteOrgID.AsInteger) and
           ( not blnGetHasSecureLetterAccess and (dm.Get_All_LetterClassificationID.Value <> 1)) then
        begin
          if ( not CheckWorkGroupAccess ) then
            DescriptionStr := ' - '+ ' êÌ—‰œÂ œ— ê—ÊÂ ò«—Ì À»  ò‰‰œÂ ﬁ—«— ‰œ«—œ'
          else if (not blnGetHasSecureLetterAccess) then
           DescriptionStr := ' - ' + ' êÌ—‰œÂ œ” —”Ì »Â ‰«„Â Â«Ì „Õ—„«‰Â ‰œ«—œ ' ;

          MessageShowString('„ÃÊ“ «—Ã«⁄ »Â «Ì‰ ‘Œ’ —« ‰œ«—Ìœ' + DescriptionStr ,false);
          Exit;
        end;
      ...
      if MoreOrg.Checked then    //«—Ã«⁄ ‘Ê‰œê«‰
      begin
        tmp:=OrgTree.Items[0];
        ...
        begin
          ...
          { TODO -oparsa : 14030718 }
           ...
           { TODO -oparsa : 14030718 }
            ...
            CheckWorkGroupAccess :=  CheckWorkGroup(integer(tmp.Data)) ;
            DescriptionStr := '';
            if ( not CheckWorkGroupAccess ) or
            { TODO -oparsa : 14030721 }
            { TODO -oparsa : 14030718 }
            ...
            begin
              if ( not CheckWorkGroupAccess ) then
               ...
               DescriptionStr := ' - ' + ' êÌ—‰œÂ œ” —”Ì »Â ‰«„Â Â«Ì „Õ—„«‰Â ‰œ«—œ ' ;
              MessageShowString('„ÃÊ“ «—Ã«⁄ »Â «Ì‰ ‘Œ’ —« ‰œ«—Ìœ '+ '  ' + '('+tmp.Text +')'+ DescriptionStr ,false);
              Exit;
            end;
