...
uses
  ...
  AdvObj, BaseGrid, AdvGrid, DBAdvGrid, Wwdbigrd, Wwdbgrid, TntDBGrids;
...
{
procedure TFWorkSheetSelection.MssDBGrid1CellClick(Column: TColumn);
var
  Pt: TPoint;
  Coord: TGridCoord;
  ClickRow: Integer;
begin
  ...
  Pt := MssDBGrid1.ScreenToClient(Mouse.CursorPos);
  Coord := MssDBGrid1.MouseCoord(Pt.X, Pt.Y);
  ClickRow := Coord.Y;
  //QSessions.RecNo := ClickRow;
  MssDBGrid1.SelectedIndex := ClickRow;
  //ShowMessage('You clicked column ' + IntToStr(ClickRow));
end;
}
procedure TFWorkSheetSelection.dbgMainDblClick(Sender: TObject);
begin
  inherited;
  Close;
end;
