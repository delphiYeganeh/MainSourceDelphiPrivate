...
uses
  ...
  oxmldom,WordXp, AppEvnts, LetterEmails, xpPanel, AdvGlowButton;
...
type
  TSentLetterInputForm = class(TMBaseForm)
    ...
    Label10: TLabel;
    Image1: TImage;
    btninflection: TAdvGlowButton;
    BtnFollower: TAdvGlowButton;
    ...
    SBDelDefualtSSigner: TAdvGlowButton;
    N3: TMenuItem;
    N4: TMenuItem;
    N5: TMenuItem;
    N2: TMenuItem;
    ...
    btnFollowRetroactionLetter: TAdvGlowButton;
    ...
    GroupBox4: TGroupBox;
    lblHasForms: TLabel;
    DBEIndicatorInnerNo_old: TDBEdit;
    ...
    procedure btninflectionClick(Sender: TObject);
...
uses YShamsiDate, FromOrgU,  FixedTableF, QuickSearch, UMain,
  ...
  EmailsDM, untFollowRetroactionLetter, USubject;
...
begin
  ...
  if  pnlDetail.Minimized then
    SetUserSetting('ExitLetterpnlDetailMinimize', 1)
  else SetUserSetting('ExitLetterpnlDetailMinimize', 0) ;
...
begin
   ...
   begin
      ...
      //---
         FieldByName('SignerID').AsInteger := DefualtSSigner;//»œ”  ¬„œÂ «“ ›—„  ‰ŸÌ„«  /‰«„Â ’«œ—Â/«„÷«Ì ﬂ‰‰œÂ
   end;
   {Ranjbar 89.09.17 ID=243}
  ...
  begin
    ...
    begin
      if not IsAnswer then
        ...
        {Ranjbar   ﬁ—«— êÌ—œ LetterID »«Ìœ „ﬁ«œÌ— ›Ì·œ SendLetterID œ— ›Ì·œ}
        {else
    ...
    if _HasReplicate then // ‰ŸÌ„«  : ﬁ«»·Ì  «” ›«œÂ «“ «‰ ﬁ«· ‰«„Â
...
//      if not Dm.ExecGet_LetterWordFile(dm.Get_All_LetterLetterID.AsInteger,false,True) then
  ...
  if OldIndicID <> DBEIndicatorID.Text then
   Exec_insert_UserLog(Self.Tag,'‘„«—Â «‰œÌò« Ê— ‰«„Â ’«œ—Â ' + OldIndicID + ' »Â ' + DBEIndicatorID.Text + '  €ÌÌ— Ì«›  ',select_LetterLetterID.AsInteger);
...
begin
  ...
  for i:=0 to ComponentCount-1 do
    if Components[i].InheritsFrom(TAdvGlowButton) then
      TAdvGlowButton(Components[i]).Visible := Dm.GetActionAccess(TAdvGlowButton(Components[i]).name,MainForm.Tag);

  btninflection.Visible    := False ;
  BtnFollower.Visible      := False ;
  SBSimpleEditor.Visible   := False ;
...
begin
  inherited;
  if LetterFormat = 2 then  //‰«„Â ÅÌ‘ ‰ÊÌ” ’«œ—Â
  ...
  begin
   ...
   Keyboard_Farsi;
   DBText1.Font.Name := 'Tahoma';
   //DBText1.OnKeyPress := DBEIncommingNOKeyPress;
   DBText1.BiDiMode := bdRightToLeft;
...
begin
  ...
  if FromOrgForm.Done then
  begin
    DSForm.DataSet.FieldByName('SignerID').AsInteger := _ResultOrgID;//»œ”  ¬„œÂ «“ ›—„ ç«—  ”«“„«‰Ì
    EdtSignerCode.Text := Dm.SignersCode.AsString;
  end;
      {Ranjbar}
      {else
...
procedure TSentLetterInputForm.BtnFollowerClick(Sender: TObject);
var
  rr : Boolean;
  LetterID, mYear : Integer;
begin
  inherited;
  if (DSForm.DataSet.State in [dsInsert,dsEdit]) then
  begin
...
procedure TSentLetterInputForm.btninflectionClick(Sender: TObject);
var
  LetterID:integer;
  rr :Boolean;
...
begin
  ...
  if RetroactionYear.Value = 0 then
    mYear := _Year
  else
    mYear := RetroactionYear.Value;    // Amin 1391/12/05
...
begin
  ...
  //œ— «— »«ÿ »« ›—„ "«— »«ÿ »« ‰—„ «›“«—Â«Ì œÌê— / „œÌ—Ì  «ÿ·«⁄«  ›—«ŒÊ«‰Ì ‘œÂ" „Ì»«‘œ
...
begin
  ...
  if not(select_Letter.State in [dsInsert,dsEdit]) then
    Exit;
  Select_LetterMemo.AsString := MainForm.FillBlobField(Self,DBMemo2.Text,'„ ‰ „÷„Ê‰ ‰«„Â');
  //---
end;
...
begin
  ...
  if Not(Select_Letter.State in [dsEdit,dsInsert]) then
  begin
    ShowMyMessage('ÅÌ€«„','«Ì‰ ‰«„Â œ— Õ«·  ÊÌ—«Ì‘ ‰„Ì »«‘œ',[mbOK],mtInformation);
    Exit;
  end;
  Select_LetterMemo.AsString := select_LetterMemo.AsString +#13+ '--------- ‰«„Â Œ«·Ì —“—Ê ‘œÂ -------- ';
...
var
  iXml: IDOMDocument;
  iRoot, iProtocol, iSoftware, iSender, iAttribute, iNode, iReceiver, iLetterNo,
  iLetterDateTime, iOrigins, iOrigin, iSubject, iPriority, iClassification,
  iAttachments,iAttachment: IDOMNode;
  XMLText, emailBody : TStringList;
  FileName,Extention,strGUID,encoded,XMLFileName,SenderEmail :string;
  ms : TMemoryStream;
  parameters : pAnsiChar;
  gregorianDate, sbj, pdfFile, inputFile:string;
  strTo, strCC, strBCC : String;
  CheckSSL : Boolean;
  I : Integer;
begin
  inherited;
...
//  ShowMessage(Dm.PathOfECE+XMLFileName+'.xml');
  ...
  if Trim(SenderEmail)='' then
      ShowMessage('«Ì„Ì· ‘„« œ— ÃœÊ·  ⁄—Ì› ò«—»—  ‰ŸÌ„ ‰‘œÂ «” ')
  else
  begin
     { strReceiverEmail := InputBox('','·ÿ›« ¬œ—” «Ì„Ì· êÌ—‰œÂ —« Ê«—œ ò‰Ìœ','');  //'target@GMAIL.COM';//
      try
      ...
      with frmLetterEmails do
       begin
        frmLetterEmails := TfrmLetterEmails.Create(Application);
        ShowModal;
        CheckSSL := chBoxSSL.Checked;
        for I := 0 to ToList.Count - 1 do
         begin
          if strTo = '' then
           strTo := ToList.Strings[I]
          else
           strTo := strTo + ';' + ToList.Strings[I];
         end;
        for I := 0 to CCList.Count - 1 do
         begin
          if strCC = '' then
           strCC := CCList.Strings[I]
          else
           strCC := strCC + ';' + CCList.Strings[I];
         end;
        for I := 0 to BCCList.Count - 1 do
         begin
          if strBCC = '' then
           strBCC := BCCList.Strings[I]
          else
           strBCC := strBCC + ';' + BCCList.Strings[I];
         end;
...
procedure TSentLetterInputForm.QAddedFormsAfterScroll(DataSet: TDataSet);
begin
...
procedure TSentLetterInputForm.hasformDescription;
begin
  if not Select_Letter.IsEmpty then
  begin
...
procedure TSentLetterInputForm.btnSubjectClick(Sender: TObject);
begin
  inherited;
