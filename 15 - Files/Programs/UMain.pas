...
begin
  ...
  begin
    ...
    StatusBar.panels[2].text :=  ' ‰«„ ò«—»— : ' + DM.UsersUserName.AsString  ;
  ...
  begin
   ...
   edtPrinterName2.Text := UDM._UserPrinterName;
    ...
    StatusBar.panels[2].text :=  ' ‰«„ ò«—»— : ' + ' „Êﬁ  ò«— »« «ò”· ' ;
  ...
  begin
   ...
   { if ( Oldsheet = newsheet  ) and
    ...
    begin }
      ...
      else ShowMessage('›«Ì· „Ê—œ ‰Ÿ— Ì«›  ‰‘œ ·ÿ›« œÊ»«—Â „”Ì— ›«Ì· „‘Œ’ ‰„«ÌÌœ');
   // end;
  end
...
begin
  ...
  begin
    ...
    begin
      ...
      ShowMessage('  ⁄œ«œ ' + inttostr( countRecord) + ' ⁄œœ Å—Ì‰  »« „Ê›ﬁÌ  «—”«·  ‘œ ' );
...
begin
  ...
  begin
    ShowMessage('  ⁄œ«œ ' + ' Ìò ' + ' ⁄œœ Å—Ì‰  »« „Ê›ﬁÌ  «—”«·  ‘œ ' );
    RecordRefresh (gridmode,_UserId);
  end;  
...
begin
  ...
  pnlMainForm.Title := 'Å—Ê‰œÂ Â«Ì Å—Ì‰  ‘œÂ' ;
  mainPageControl.ActivePageIndex:= 0 ;
  gridmode := 1;
  ContactGrid.Columns[1].Visible := False;
    //
...
begin
  ...
  if Trim(newTo) = 'N' then
    msgStr  := '¬Ì« «“ «‰ ﬁ«· ”ÿ—'+ '"' +Trim(newFrom)  +'"'+ '  « ”ÿ— ' + '"¬Œ—"' + ' ›«Ì· «‰ Œ«»Ì „ÿ„⁄‰ „Ì »«‘Ìœ ø '
  else
    msgStr  := '¬Ì« «“ «‰ ﬁ«· ”ÿ—'+'"' + Trim(newFrom)  +'"' + '  « ”ÿ— ' +'"' + Trim(newTo) +'"' + ' ›«Ì· «‰ Œ«»Ì „ÿ„⁄‰ „Ì »«‘Ìœ ø ' ;
  ...
  begin
    ...
    try
      ...
      lblProc.Visible := False;
      ProgressBar1.Position := ProgressBar1.Max ;
      ShowMessage(' «‰ ﬁ«·  '+ IntToStr(intCount) +' —òÊ—œ »« „Ê›ﬁÌ  «‰Ã«„ ‘œ ' ) ;
...
begin
 ...
 // SpprintableData.UpdateFooter;
    ...
    StatusBar.panels[3].text :=  '  ⁄œ«œ —òÊ—œ : ' + IntToStr(ContactGrid.DataSource.DataSet.recordcount ) ;
  end
  else
  begin
      StatusBar.panels[3].text :=  '  ⁄œ«œ —òÊ—œ : ' + IntToStr(GridTemp.DataSource.DataSet.recordcount ) ;
  end;
...
begin
  inherited;
   {
  try
   // Printer.PrinterIndex := Dm.GetValue(2100); //ç«Åê—
  except
  end;
  }
 ...
 // Dm.SetValue(2100,Printer.PrinterIndex); //ç«Åê—
...
begin
  ...
  RecordRefresh (2,_UserId);
    pnlMainForm.Title := 'Â„Â Å—Ê‰œÂ Â«' ;  
  mainPageControl.ActivePageIndex:= 0 ;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Memo <> nil then
      begin
        Memo.Memo.Text := SpprintableDataColumF2.AsString;// ò·«”Â Å—Ê‰œÂ  —òÌ»Ì
      end;

      Memo := TfrxMemoView(MainReport.FindObject('MemDesc0')); //C
      if Memo <> nil then
        Memo.Memo.Text := SpprintableDataColumF3.AsString ; //«ÿ·«⁄«  ò·Ì Å—Ê‰œÂ

      Memo := TfrxMemoView(MainReport.FindObject('MemDesc2')); //G
      if Memo <> nil then
        Memo.Memo.Text := SpprintableDataColumF7.AsString ; //‘„«—Â Ã·œ Å—Ê‰œÂ

      Memo := TfrxMemoView(MainReport.FindObject('MemDesc4')); //E
      if Memo <> nil then
        Memo.Memo.Text := SpprintableDataColumF5.AsString ; //‰«„ ÿ—Õ

      Memo := TfrxMemoView(MainReport.FindObject('MemDesc5')); //I
      if Memo <> nil then
        Memo.Memo.Text := SpprintableDataColumF9.AsString ; //‰«„ Ê ‰«„ Œ«‰Ê«œêÌ

      Memo := TfrxMemoView(MainReport.FindObject('MemDesc63')); //L
      if Memo <> nil then
        Memo.Memo.Text := SpprintableDataColumF12.AsString ; //»Œ‘

      Memo := TfrxMemoView(MainReport.FindObject('MemDesc64')); //J
      if Memo <> nil then
        Memo.Memo.Text := SpprintableDataColumF10.AsString ; //Å·«ò «’·Ì

      Memo := TfrxMemoView(MainReport.FindObject('MemDesc65')); //K
      if Memo <> nil then
        Memo.Memo.Text := SpprintableDataColumF11.AsString ; //Å·«ò ›—⁄Ì


      Memo := TfrxMemoView(MainReport.FindObject('MemDesc7')); //N
      if Memo <> nil then
      begin
        Memo.Memo.Text := SpprintableDataColumF14.AsString ; //¬œ—” ÊﬁÊ⁄ „·ò
        if Memo.Tag = 0 then
      ...
      if Memo <> nil then
        Memo.Memo.Text := SpprintableDataColumF13.AsString ; //òœ ‰Ê”«“Ì
  ...
  begin
    ...
    begin
      ...
      if Memo <> nil then
      begin
        Memo.Memo.Text := DM.CDTempF1.AsString;// ò·«”Â Å—Ê‰œÂ  —òÌ»Ì
        if Memo.Tag = 0 then
          Memo.Tag := Memo.Font.Size ;
      ...
      if Memo <> nil then
      begin
        Memo.Memo.Text := DM.CDTempF2.AsString ; //«ÿ·«⁄«  ò·Ì Å—Ê‰œÂ
        if Memo.Tag = 0 then
          Memo.Tag := Memo.Font.Size ;
      ...
      if Memo <> nil then
      begin
        Memo.Memo.Text := DM.CDTempF3.AsString ; //‘„«—Â Ã·œ Å—Ê‰œÂ
        if Memo.Tag = 0 then
          Memo.Tag := Memo.Font.Size ;
        if Length(Memo.Memo.Text) > 55 then
          Memo.Font.Size := Memo.Tag - 2
      ...
      if Memo <> nil then
      begin
        Memo.Memo.Text := DM.CDTempF4.AsString ; //‰«„ ÿ—Õ
        if Memo.Tag = 0 then
          Memo.Tag := Memo.Font.Size ;
      ...
      if Memo <> nil then
      Begin
        Memo.Memo.Text := DM.CDTempF5.AsString ; //‰«„ Ê ‰«„ Œ«‰Ê«œêÌ
        if Memo.Tag = 0 then
          Memo.Tag := Memo.Font.Size ;
      ...
      if Memo <> nil then
      begin
        Memo.Memo.Text := DM.CDTempF9.AsString ; //»Œ‘
        if Memo.Tag = 0 then
          Memo.Tag := Memo.Font.Size ;
      ...
      if Memo <> nil then
      begin
        Memo.Memo.Text := DM.CDTempF10.AsString ; //Å·«ò «’·Ì
        if Memo.Tag = 0 then
          Memo.Tag := Memo.Font.Size ;
      ...
      if Memo <> nil then
      begin
        Memo.Memo.Text := DM.CDTempF11.AsString ; //Å·«ò ›—⁄Ì
        if Memo.Tag = 0 then
          Memo.Tag := Memo.Font.Size ;
        if Length(Memo.Memo.Text) > 20 then
          Memo.Font.Size := Memo.Tag - 2
      ...
      if Memo <> nil then
      begin
        Memo.Memo.Text := DM.CDTempF12.AsString ; //¬œ—” ÊﬁÊ⁄ „·ò  ///////////////////////////////////
        if Memo.Tag = 0 then
          Memo.Tag := Memo.Font.Size ;
        if Length(Memo.Memo.Text) > 140 then
          Memo.Font.Size := Memo.Tag - 2
      ...
      if Memo <> nil then
      begin
        Memo.Memo.Text := DM.CDTempF8.AsString ; //òœ ‰Ê”«“Ì
        if Memo.Tag = 0 then
          Memo.Tag := Memo.Font.Size ;
        if Length(Memo.Memo.Text) > 140 then
          Memo.Font.Size := Memo.Tag - 2
...
begin
  ...
  if SpprintableDataIsPrint.AsBoolean then
    msgStr :=  '¬Ì« «“  €ÌÌ— Ê÷⁄Ì  «“ Å—Ì‰  ‘œÂ »Â Å—Ì‰  ‰‘œÂ „ÿ„⁄‰ „Ì »«‘Ìœø'
  else  msgStr :=  '¬Ì« «“  €ÌÌ— Ê÷⁄Ì  «“ Å—Ì‰  ‰‘œÂ »Â Å—Ì‰  ‘œÂ „ÿ„⁄‰ „Ì »«‘Ìœø'  ;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        lblProcTemp.Caption := 'œ— Õ«· Å—œ«“‘ ›«Ì· «ò”·';
        Application.ProcessMessages;
        ...
        begin
          ...
          // œ—Ã —òÊ—œ ÃœÌœ
          DM.CDTemp.Append; // Ì« ClientDataSet1.Insert;
          ...
          intCount := intCount + 1;
          lblProcTemp.Caption := 'œ— Õ«· Å—œ«“‘  ' + IntToStr(intCount)  + ' —òÊ—œ';
          Application.ProcessMessages;
   ...
   //   dm.Secretariats.Locate('SecId',_DefaultSec,[]);
    ...
    try
      lblProcTemp.Visible := True;
      lblProcTemp.Caption := 'œ— Õ«· «‰ ﬁ«· œ«œÂ ...';
      Application.ProcessMessages;
...
begin
 ...
 { try
  ...
  begin
    ShowMessage('ÅÊ‘Â „Ê—œ ‰Ÿ— Ì«›  ‰‘œ!');
    exit ;
  end;

  if rbPicReverce.ItemIndex = 0 then
    tempStr := '¬Ì« «“  €ÌÌ— ‰«„ ⁄ò”Â« „ÿ„⁄‰ „Ì »«‘Ìœø'
  else if rbPicReverce.ItemIndex = 1 then
    tempStr := '¬Ì« «“  €ÌÌ— ‰«„ ⁄ò”Â« »Â Â„—«Â òÅÌ œÊ»«—Â ⁄ò” «Ê· „ÿ„⁄‰ „Ì »«‘Ìœø'
  else if rbPicReverce.ItemIndex = 2 then
    tempStr := '¬Ì« «“ òÅÌ œÊ»«—Â ⁄ò” «Ê· „ÿ„⁄‰ „Ì »«‘Ìœø'
  else if rbPicReverce.ItemIndex = 3 then
    tempStr := '¬Ì« «“ Õ–› ¬Œ—Ì‰ ⁄ò” Â— ›Ê·œ— „ÿ„⁄‰ „Ì »«‘Ìœø' ;

  //memo1.Lines.Add ( 'Ã” ÃÊ  « ”Â ”ÿÕ ›Ê·œ— «‰Ã«„ „Ì ‘Êœ ' ) ;
  memo1.Lines.Add ( 'Ã” ÃÊ  « çÂ«— ”ÿÕ ›Ê·œ— «‰Ã«„ „Ì ‘Êœ ' ) ;
...
begin
  ...
  begin
     if MessageDlg('›«Ì· œ— „”Ì— ÊÃÊœ œ«—œ ¬Ì« „Ì ŒÊ«ÂÌœ Õ–› ‘Êœø',mtWarning,[mbyes,mbno],0)=mryes then
       deleteFile(FolderPath) ;
  end;
  
  memo1.Lines.Add ( '  MA_    Ã” ÃÊ »—«Ì ›«Ì·Â«Ì »« ÅÌ‘Ê‰œ ' ) ;
  memo1.Lines.Add ( '  ' ) ;
    ...
    begin
      ...
      begin
        ...
        begin
          {
          Path := ShellTreeView2.Path+'\'+ShellTreeView2.Selected.Item[j].Text;
          memo1.Lines.Add (  '  „”Ì—   ' + Path   ) ;
          memo1.Lines.Add (  ' ... œ— Õ«· ‘„«—‘  '   ) ;
          Counttif := DM.CountFilesInFolder( Path, Prefix  );
           memo1.Lines.Add (  '  '+  IntToStr(Counttif) + '  =  ⁄œ«œ ›«Ì· Ì«›  ‘œÂ  ') ;
          ...
          begin
            ...
            begin
              ...
              memo1.Lines.Add (  '  '+ IntToStr(Counttif) + '  =  ⁄œ«œ ò· ›«Ì·Â«Ì Ì«›  ‘œÂ  ') ;
  ...
  memo1.Lines.Add ( '”«Œ  ›«Ì· «ò”· »« „Ê›ﬁÌ  œ— „”Ì— »«·« «‰Ã«„ ‘œ' ) ;
...
begin
  ...
  SelectedNode := ShellTreeView4.Selected; // ê—Â «‰ Œ«»ù‘œÂ
  ParentNode  := SelectedNode.Parent; // ê—Â Ê«·œ

  // „”Ì— ÅÊ‘Âù«Ì òÂ „ÌùŒÊ«ÂÌœ «ÌÃ«œ ò‰Ìœ
    ...
    try
      try
       ...
       // ShellTreeView4.Repaint;
        ShowMessage('ÅÊ‘Â Â« »« „Ê›ﬁÌ  òÅÌ ‘œ‰œ' + '  '+   IntToStr(countFolder ));
      end;
    except

       ShowMessage('⁄„·Ì«  òÅÌ ›Ê·œ—Â« »« Œÿ« „Ê«ÃÂ ‘œ ·ÿ›« œÊ»«—Â «„ Õ«‰ ò‰Ìœ');
    end;
  end ;
  {
  else
  begin
    ShowMessage('ÅÊ‘Â «“ ﬁ»· ÊÃÊœ œ«—œ.');
  end;
   }
end;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        lblProcTempExcel.Caption := 'œ— Õ«· Å—œ«“‘ ›«Ì· «ò”·';
        Application.ProcessMessages;
        ...
        //   end;
          ...
          //intCount := intCount + 1;
          lblProcTempExcel.Caption := 'œ— Õ«· Å—œ«“‘  ' + IntToStr(intCount)  + ' —òÊ—œ';
          Application.ProcessMessages;
   ...
   //   dm.Secretariats.Locate('SecId',_DefaultSec,[]);
    ...
    try
      lblProcTempExcel.Visible := True;
      lblProcTempExcel.Caption := 'œ— Õ«· «‰ ﬁ«· œ«œÂ ...';
      Application.ProcessMessages;
