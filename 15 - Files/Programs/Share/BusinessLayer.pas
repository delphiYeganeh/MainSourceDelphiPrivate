...
Procedure Calculate_Wage_Interest(LoanAmount : int64;LoanTypeid:integer;var WageAmount,InterestAmount:Int64);             overload;
Procedure Calculate_Wage_Interest(LoanAmount : int64;PartNumber,LoanTypeid:integer;var WageAmount,InterestAmount:Int64);  overload;
...
Procedure Open_Report_Payment_ByDocumentNo(documentNo:string;aYear: Integer);
Procedure Open_REPORT_PAYMENT_ForPrint(DocumentNoFrom,DocumentNoTo,DateFrom,DateTo:string);
Procedure Open_Report_DeletedPayment_ByDocumentNo(documentNo:string;aYear: Integer);
...
Function Get_SumPartAmount(LoanID:integer):Largeint;
...
function get_RemainOfLoan_byLoanID(loanId:integer):Int64;
...
uses dmu, YShamsiDate,  MainU, AddLoanU, AddReportNameU,
  ComObj, Classes;
...
Procedure Calculate_Wage_Interest(LoanAmount : int64;PartNumber,LoanTypeid:integer;var WageAmount,InterestAmount:Int64);
var
   ADOSP:TADOStoredProc;
begin
  ADOSP:=TADOStoredProc.create(nil);
...
Procedure  Calculate_Wage_Interest(LoanAmount : int64;LoanTypeid:integer;var WageAmount,InterestAmount:int64);
var
  ADOSP:TADOStoredProc;
begin
  ADOSP:=TADOStoredProc.create(nil);
...
Begin
  ...
  if (ADOSP.Parameters.ParamByname('@Value').value = '') then
    Result:= '0'
  else
    Result:= ADOSP.Parameters.ParamByname('@Value').value;
End;
...
{  IF (FrAddPayment<>NIL) AND
   ...
   BEGIN
            ...
            _DEBATOR  :=Crtr_DetAccountID;
   END;}
    if (_DEBATOR <> 0) and (_CREDITOR <> 0) then
    begin
        {       // Amin 1391/11/08-2
...
{ ATAIE1400
  ...
  BEGIN
    ...
    EXCEPT
    _YearTmp:=strtoint(copy(DateTmp,3,2));
    END
  END;
}
  with ADOSP.Parameters.AddParameter do
...
procedure UpdateFinancialNote_Detail(FinancialNote_DeatilID,StatusId:string);
Var
  _D,_Cr,_Db,_Crtr_DetAccountID,_m:INTEGER;
...
begin
  ...
  ADOSP.Connection:=dm.YeganehConnection;
  ADOSP.ExecProc;
  if ADOSP.Parameters.ParamByname('@RandACCOUNTID').value <> null then
...
begin
  ...
  With Dm.AccDailyReport,parameters do
   begin
    Close;
    ParamByName('@_Date').Value := QuotedStr(_RepDate);
    Open;
   end;
  *)
  dm.AccDailyReport.Close;
  dm.AccDailyReport.SQL.Text :=
   ' SELECT   Payment.documentno, Payment.Comment, Account.AccountTitle, Account.AccountNo, PayType.PayTypeTitle, ' +
	 '          Payment.AMOUNT As Amount_Crt,0 As Amount_Dbt, Payment.[Date], Payment.[Time] ' +
	 ' FROM     Payment ' +
   ...
   ' WHERE Payment.[Date]=' + QuotedStr(_RepDate) +
   ' UNION ALL ' +
   ' SELECT   Payment.DocumentNo, Payment.Comment, Account.AccountTitle, Account.AccountNo, PayType.PayTypeTitle, ' +
	 '          0 AS Amount_Crt,Payment.AMOUNT AS Amount_Dbt,Payment.[Date],Payment.[Time] ' +
   ' FROM     Payment ' +
   ' INNER JOIN Account ON Payment.DebtorAccountID = Account.AccountID ' +
   ' INNER JOIN PayType ON Payment.PayTypeID = PayType.PayTypeID ' +
   ' WHERE      Payment.[Date]=' + QuotedStr(_RepDate) +
   ' ORDER BY   Payment.DocumentNo,Payment.[Time], Payment.[Date] ';
  dm.AccDailyReport.Open;
