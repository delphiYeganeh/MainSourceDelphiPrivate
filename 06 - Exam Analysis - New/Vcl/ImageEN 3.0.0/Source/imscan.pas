...
begin
  ...
  begin
    ...
    else if grec.TransferMode <> tmFile then
      grec.TransferMode := tmNative;
    //
    ...
    // before of 2.2.3 we first call ICAP_ORIENTATION then ICAP_SUPPORTEDSIZE
...
begin
  ...
  if not assigned(grec) then
    LogWrite('TIEProxyWin.WndProc. Msg=' + inttostr(msg.message) + ' grec=' + inttostr(integer(grec)) + ' grec no assigned!')
  else
    LogWrite('TIEProxyWin.WndProc. Msg=' + inttostr(msg.message) + ' grec=' + inttostr(integer(grec)) + ' state=' + inttostr(grec^.nstate));
...
begin
  ...
  result := grec;
  LogWrite('IETWAINAcquireOpen : result=  ' + inttostr(integer(result)));
  finally
    //windows.SetActiveWindow(grec.actwnd);
  end;
end;
