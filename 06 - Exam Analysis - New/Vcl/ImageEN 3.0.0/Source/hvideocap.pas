...
begin
  ...
  if (lpVHdr^.dwFlags and 1 = 0) or (lpVHdr^.dwFlags and 2 = 0) then
    exit;
  if SendMessageTimeOut(hWnd, WM_CAP_GET_USER_DATA, 0, 0, SMTO_ABORTIFHUNG or SMTO_BLOCK, 100, i) <> 0 then
  begin
    pobj := pointer(i);
...
function CallBackStatusFunc(hWnd: HWND; nID: integer; lpsz: pchar): LRESULT;
var
  pobj: pointer;
  obj: TImageEnVideoCap;
{$IFDEF IEDELPHI3}
