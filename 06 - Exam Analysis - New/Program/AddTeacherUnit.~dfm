inherited FAddTeacher: TFAddTeacher
  Left = 714
  Top = 284
  Width = 311
  Height = 281
  Caption = #1575#1601#1586#1608#1583#1606' '#1575#1587#1578#1575#1583
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel [0]
    Left = 0
    Top = 201
    Width = 295
    Height = 41
    Align = alBottom
    TabOrder = 0
    object SpeedButton1: TSpeedButton
      Left = 80
      Top = 8
      Width = 65
      Height = 22
      Caption = #1578#1575#1610#1610#1583
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFCAE9CA6AC56A85CE85F1F7F1}
      OnClick = SpeedButton1Click
    end
    object SpeedButton2: TSpeedButton
      Left = 11
      Top = 8
      Width = 65
      Height = 22
      Caption = #1575#1606#1589#1585#1575#1601
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFBFBFED6D7F8}
      OnClick = SpeedButton2Click
    end
  end
  object Panel2: TPanel [1]
    Left = 0
    Top = 0
    Width = 295
    Height = 201
    Align = alClient
    TabOrder = 1
    DesignSize = (
      295
      201)
    object Label1: TLabel
      Left = 269
      Top = 18
      Width = 13
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1606#1575#1605
    end
    object Label2: TLabel
      Left = 271
      Top = 130
      Width = 11
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1705#1583
    end
    object Label3: TLabel
      Left = 261
      Top = 168
      Width = 21
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1578#1604#1601#1606
    end
    object Label4: TLabel
      Left = 224
      Top = 58
      Width = 58
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1606#1575#1605' '#1582#1575#1606#1608#1575#1583#1711#1610
    end
    object Label5: TLabel
      Left = 256
      Top = 96
      Width = 26
      Height = 13
      Anchors = [akTop, akRight]
      Caption = #1593#1606#1608#1575#1606
    end
    object edtName: TEdit
      Left = 16
      Top = 16
      Width = 200
      Height = 21
      Anchors = [akLeft, akTop, akRight]
      TabOrder = 0
    end
    object edtCode: TEdit
      Left = 16
      Top = 130
      Width = 200
      Height = 21
      Anchors = [akLeft, akTop, akRight]
      TabOrder = 3
    end
    object edtTitle: TEdit
      Left = 16
      Top = 92
      Width = 200
      Height = 21
      Anchors = [akLeft, akTop, akRight]
      TabOrder = 2
    end
    object edtFamily: TEdit
      Left = 16
      Top = 54
      Width = 200
      Height = 21
      Anchors = [akLeft, akTop, akRight]
      TabOrder = 1
    end
    object edtPhone: TEdit
      Left = 16
      Top = 168
      Width = 200
      Height = 21
      Anchors = [akLeft, akTop, akRight]
      TabOrder = 4
    end
  end
  inherited ActionList: TActionList
    Left = 135
    Top = 128
  end
  inherited xpWindow1: TxpWindow
    Top = 128
  end
  object QAddTeacher: TADOQuery
    Connection = Dm.YeganehConnection
    Parameters = <>
    SQL.Strings = (
      'SELECT TeacherCode,FullName FROM Teacher')
    Left = 52
    Top = 111
    object QAddTeacherTeacherCode: TStringField
      FieldName = 'TeacherCode'
    end
    object QAddTeacherFullName: TWideStringField
      FieldName = 'FullName'
      Size = 80
    end
  end
end
