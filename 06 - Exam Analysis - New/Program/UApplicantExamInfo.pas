...
begin
  ...
  begin
    ...
    //decimalPart := Abs(Select_Applicant_by_ExamidmaxMark.AsFloat - Trunc(Select_Applicant_by_ExamidmaxMark.AsFloat));

    //tempStr     := FloatToStr(temp) ;
    tempStr     := Select_Applicant_by_ExamidmaxMark.AsString ;
    ...
    if  TempLenFrac > 10  then
      TempLenFrac := 2 ;

    ADOQuery1.Close;
    ADOQuery1.SQL.Clear;
    ADOQuery1.SQL.Add('SELECT ROND FROM dbo.ROND_INF WHERE EXAMID = ''' + IntToStr(dm.Browse_ExamExamID.AsInteger) + '''');
    ADOQuery1.Open;

    if ADOQuery1.RecordCount>0 then
      TempLenFrac := ADOQuery1.Fields.Fields[0].AsInteger ;
    end ;
