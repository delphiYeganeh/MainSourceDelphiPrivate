...
type
  TFInputExam = class(TMBaseForm)
    ...
    Select_Exam_ByExamIDUTID: TIntegerField;
...
begin
  ...
  Select_Exam_ByExamIDUTID.AsInteger := _UTId ;
end;
...
begin
  inherited;
   ...
   qryTeacher.SQL.Add('SELECT TeacherID, FullName FROM Teacher WHERE UTID = '+INTTOSTR(_UTId) );

   qryTeacher.Close ;
   qryTeacher.Open ;

   qryEducationGroup.SQL.Clear;
   qryEducationGroup.SQL.Add('SELECT EducationGroupID,EducationGroupTitle FROM EducationGroup WHERE UTID = '+INTTOSTR(_UTId) );

   qryEducationGroup.Close;
   qryEducationGroup.Open ;
...
begin
  ...
  dm.qGroupExam.Close;
  dm.qGroupExam.SQL.Text :=' select * from GroupExam  where UTID = '+IntToStr(_UTId)  ;
  dm.qGroupExam.open;
  { TODO -oparsa : 14031207 }
end;
...
begin
  ...
  qryTeacher.SQL.Clear;
  if Edit1.Text <> '' then
     qryTeacher.SQL.Add('SELECT TeacherID, FullName FROM Teacher WHERE UTID = '+INTTOSTR(_UTId)+' AND  TeacherCode = ''' + Edit1.Text + '''')
     ELSE
     qryTeacher.SQL.Add('SELECT TeacherID, FullName FROM Teacher WHERE UTID = '+INTTOSTR(_UTId) );
  qryTeacher.Open;
  { TODO -oparsa : 14031207 }
