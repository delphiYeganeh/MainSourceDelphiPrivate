...
type
  TMainForm = class(TMBaseForm)
    ...
    xpBitBtn69: TxpBitBtn;
    AddUniversity: TAction;
    ...
    procedure AddUniversityExecute(Sender: TObject);
  private
...
uses Dmu,FrInputFix, backupF,
  ...
  UApplicantImportNew, UntSearchDataSet, UUniversity;
...
begin
   ...
   ExamWhere := ' (isnull(Exam.UTID,0) = '+inttostr(_UTId)+' )';
   QuestionWhere :='' ;
   QuestionBodyWhere :='';
   ApplicantWhere := ' ( isnull(Applicant.UTID,0) = '+IntToStr( _UTID ) +' ) ';
   TeacherWhere   := ' (isnull(teacher.UTID,0) = '+inttostr(_UTID)+')';
   TrainingCourseWhere := ' ( isnull(Course.UTID,0) = '+inttostr(_UTId)+')';
   { TODO -oparsa : 14031207 }
   LoadGrids;
   CustomFieldsRefresh;
   ...
   //⁄œ„ œ—Ã ’ÕÌÕ  ⁄œ«œ —òÊ—œ œ— ’›ÕÂ «’·Ì
   //xpBitBtn28Click(self);
{
   xpTabSheet1.ImageIndex := 76 ;
   xpTabSheet2.ImageIndex := 75 ;
   xpTabSheet3.ImageIndex := 74 ;
   xpTabSheet4.ImageIndex := 133 ;
   xpTabSheet5.ImageIndex := 168 ;
   xpTabSheet6.ImageIndex := 106 ;
   xpTabSheet7.ImageIndex := 75 ;
   }
   xpTabSheet1.ImageIndex := 173 ;
   xpTabSheet2.ImageIndex := 178 ;
   xpTabSheet3.ImageIndex := 176 ;
   xpTabSheet4.ImageIndex := 177 ;
   xpTabSheet5.ImageIndex := 174 ;
   xpTabSheet6.ImageIndex := 175 ;
   xpTabSheet7.ImageIndex := 178 ;
end;
...
begin
  ...
  QuestionGrid.PrintTitle:='«ÿ·«⁄«   Õ·Ì·Ì ”∆Ê«·«  ¬“„Ê‰ '+dm.Browse_ExamExamTitle.AsString;
end;
...
begin
  ...
  //ApplicantWhere:='( isnull(Applicant.UTID,0) = '+IntToStr( _UTID ) + ' and  (Applicant.ApplicantID in (SELECT  ApplicantID FROM  ApplicantExam WHERE  ExamID='+Dm.Browse_ExamExamID.AsString+')))';

  if (Dm.Browse_ExamExamID.AsInteger >  0) then
    ApplicantWhere:= ' ( isnull(Applicant.UTID,0) =  '+IntToStr( _UTID ) + ' and  ( Applicant.ApplicantID in (SELECT  ApplicantID FROM  ApplicantExam WHERE  ExamID = '+ IntToStr(Dm.Browse_ExamExamID.asinteger)+')))'
  else
    ApplicantWhere:= ' ( isnull(Applicant.UTID,0) = '+IntToStr( _UTID ) +' ) ';
...
begin
  if ExamGrid.DataSource.DataSet.RecordCount > 0 then
  begin
...
begin
 // Application.BiDiKeyboard:= '00020409';//00000429 ›«—”Ì
...
begin
   ...
   if (dm.Browse_ExamExamID.AsInteger >  0) then
     TeacherWhere:=' ( isnull(teacher.UTID,0) = '+inttostr(_UTID)+' and   TeacherID in (select distinct DesignerTeacherID from Question where ExamID='+dm.Browse_ExamExamID.AsString+'))'
   else   TeacherWhere:=' ( isnull(teacher.UTID,0) = '+inttostr(_UTID)+' ) ' ;
...
begin
  ...
  begin
   ...
   { TODO -oparsa : 14031207 }
   //TrainingCourseWhere:= ' TrainingCourse.ExamID='+Dm.Browse_ExamExamID.AsString;
   TrainingCourseWhere:= ' ( isnull(Course.UTID,0) = '+inttostr(_UTId)+' and  TrainingCourse.ExamID='+Dm.Browse_ExamExamID.AsString + ')';
   { TODO -oparsa : 14031207 }
  end
...
begin
  IF (dm.Select_TrainingCourse_By_WhereTrainingCourseID.AsInteger > 0) THEN
  begin
...
begin
  ...
  begin
    ...
    ApplicantWhere:= ' ( isnull(Applicant.UTID,0) = '+IntToStr( _UTID ) + ' and  (ApplicantID in (SELECT  ApplicantID  FROM  ApplicantCourse WHERE   TrainingCourseID = '+
    Dm.Select_TrainingCourse_By_WhereTrainingCourseID.AsString+'))) ';
    cb_ApplicantCurrentExam.Checked:=true;
    ArefreshApplicant.Execute;
    mainPageControl.ActivePageIndex:=2;
...
begin
  ...
  if dm.Browse_ExamExamID.AsInteger = 0 then
    exit;
  if  messageShowString('¬Ì« «Ì‰ '+_ApplicentEntityCaption+'  —« »Â ¬“„Ê‰ Ã«—Ì «Œ ’«’ „Ì œÂÌœ',true) then
  begin
     with dm.Select_Applicant_by_Where do
     begin
        Next;
...
begin
  ...
  xpBitBtn50.Caption  := '«ÌÃ«œ' + ' ' + _ApplicentEntityCaption;
  btnGetStno.Caption  := 'Å«”Œ Â«Ì' + ' ' + _ApplicentEntityCaption;
  Label10.Caption := _ApplicentCodeEntityCaption;
  cb_ApplicantCurrentExam.Caption := '›ﬁÿ '+_ApplicentEntityPlural+'  ¬“„Ê‰/œ—” Ã«—Ì';
...
procedure TMainForm.AddUniversityExecute(Sender: TObject);
begin
  inherited;
  FUniversity:=TFUniversity.Create(Application);
  FUniversity.ShowModal
end;
