...
uses Shellapi,forms, BaseUnit, ExtCtrls, Menus, ExtActns, BandActn, Classes,StdCtrls,Dialogs,
     ...
     IdCoder, IdCoder3to4, IdCoderMIME, RichEdit, BusinessSkinForm, MssDBGrid , U_SORT_SETTING_FRM , U_COMPANY_NAME_SETTING_FRM , U_VERSION_INFO_FRM ,U_SEARCH_SHARHE_PEYVAST_FRM , U_SEARCH_IN_PDF_FRM,
  FnpNumericEdit,BaseUnits, IdComponent, IdTCPConnection, IdTCPClient,
  IdFTP;
...
Type
  ...
  TMainForm = class(TMBaseForm)
    ...
    procedure MeSDateKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    ...
    procedure REdtMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
...
begin
   ...
   //if FDisplayMode = Search then
      //QuickSearchF.Close;
  if value=Notany then value:=Search;
...
begin
   inherited;
   if (MeSDate.Text <> '˛13  /  /  ')And(MeEDate.Text = '˛13  /  /  ') then   //#254 + MeEDate.Text
      MeEDate.Text := MeSDate.Text;
end;
...
begin
  ...
  QTemp.Open;
  if not QTemp.IsEmpty then Result:=QTemp.FieldValues['VALUE'];
end;
...
var
   _Wrap,_Text,_False,_True,_Zero, _wdWord:OleVariant;
   i , index: integer;
begin
...
begin
 ...
 begin
    if Dm.Get_All_LetterHasLetterAlerm.AsInteger=2 then Column.Font.Color:=clMaroon
    else if Dm.Get_All_LetterHasLetterAlerm.AsInteger=1 then Column.Font.Color:=clGreen
    else Column.Font.Color:=clBlack;
 end;
