...
type
    ...
    Report_ForcePayment2AMERCE: TIntegerField;
    ...
    spGet_Farsi_Message: TADOStoredProc;
    spInsert_ErrorMessage: TADOStoredProc;
    spupdate_UserLoginLogout: TADOStoredProc;
  ...
  public
    ...
    { TODO -oparsa : 14030108 }
    function dmGet_Farsi_Message(ErrorMsg:string;var ShowMessage :boolean):string;
    Procedure  dmInsert_ErrorMessage(ErrorMessage:string;UserID:integer);
    Procedure dmupdate_UserLoginLogout(UserLoginLogoutID:integer;isSafeLogout:boolean);
  end;
...
Const
   _SoftVerLastUpdate = '1403/08/05';//'1396/07/17';
...
begin
  ...
  farsiMessage := dmGet_Farsi_Message(e.Message,ShowMsg);
  ...
  begin
    ...
    dmInsert_ErrorMessage(E.Message,_Userid);
...
function Tdm.dmGet_Farsi_Message(ErrorMsg: string; var ShowMessage: boolean): string;
begin

  with DM.spGet_Farsi_Message do
  begin
    Close;
    Parameters.ParamByName('@ERRORMSG').Value:= ErrorMsg ;
    try
      Open;
    except
    end;
  ...
  Result:= '' ;
  if spGet_Farsi_Message.Parameters.ParamByname('@FARSIMESSAGE').value <> '' then
     Result:= spGet_Farsi_Message.Parameters.ParamByname('@FARSIMESSAGE').value;
  ShowMessage:= spGet_Farsi_Message.Parameters.ParamByname('@SHOWMSG').value;

end;



procedure Tdm.dmInsert_ErrorMessage(ErrorMessage: string; UserID: integer);
begin
  with DM.spInsert_ErrorMessage do
  begin
    Close;
    Parameters.ParamByName('@ERRORMESSAGE').Value:= ErrorMessage ;
...
procedure Tdm.dmupdate_UserLoginLogout(UserLoginLogoutID: integer; isSafeLogout: boolean);
begin
  with DM.spupdate_UserLoginLogout do
  begin
    Close;
    Parameters.ParamByName('@USERLOGINLOGOUTID').Value:= UserLoginLogoutID ;
    Parameters.ParamByName('@ISSAFELOGOUT').Value:= isSafeLogout ;
    try
      Open;
    except
    end;
