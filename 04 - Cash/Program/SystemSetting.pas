...
begin
   ...
   Old2 := Trim(Dm.GetSystemValue(52));
...
//      SQL.Text := 'UPDATE sys_AppMessage SET FarsiMessage = replace(replace(replace(FarsiMessage, '''+'‰«„Â'+''', '''+'”‰œ'+'''), '''+'”‰œ'+''' , '''+Old2+'''),'''+Old2+''','''+EdtDocument.Text+''')';
//      ExecSQL;
//   end;

   //MainForm.lettersDbGrid »—œ«‘ ‰ Ê œÊ»«—Â –ŒÌ—Â ﬂ—œ‰  ‰ŸÌ„«  ” Ê‰Â«Ì ﬂ‰ —·
   if (Old1 <> ChBoTitle.Checked) or (Old2 <> Trim(EdtDocument.Text))then
