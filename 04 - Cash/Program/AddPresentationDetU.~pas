...
type
  TFrAddPresentationDet = class(TForm)
...
procedure TFrAddPresentationDet.AExitExecute(Sender: TObject);
begin
   Close;
end;

procedure TFrAddPresentationDet.FormClose(Sender: TObject; var Action: TCloseAction);
begin
   Action:=caFree;
   dm.AccountPresenter.Cancel;
end;

procedure TFrAddPresentationDet.TEditExit(Sender: TObject);
Var
   D:String;
begin
...
procedure TFrAddPresentationDet.CurrencyKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
   inherited;
...
procedure TFrAddPresentationDet.FormCreate(Sender: TObject);
 var i:integer;
begin
...
procedure TFrAddPresentationDet.GotoNext(Sender: TObject; var key: Word; Shift: TShiftState);
var
   i: integer;
begin
...
procedure TFrAddPresentationDet.TEditEnter(Sender: TObject);
begin
...
procedure TFrAddPresentationDet.FormShow(Sender: TObject);
var i: integer;
begin
//   AccountID := Get_AccountID_ByAccountNo(SearchEdit.Text);
//   AccountID := Get_AccountID_ByAccountNo();
   Dm.AccountPresenter.Open;
...
{   AccountID := Get_AccountID_ByAccountNo(SearchEdit.Text);
   ...
   BEGIN
     ...
     _AccountPresenterAccountID:= Get_AccountID_ByAccountNo(SearchEdit.Text);
   END;
...
procedure TFrAddPresentationDet.searchAccountExecute(Sender: TObject);
begin
...
procedure TFrAddPresentationDet.AExportToExcelExecute(Sender: TObject);
begin
 if ActiveControl.ClassNameIs('TYDBGrid') then
   TYDBGrid(ActiveControl).ExportToExcel;
end;

procedure TFrAddPresentationDet.SearchLoanExecute(Sender: TObject);
begin
...
procedure TFrAddPresentationDet.ShowMsgString(AppMessage: string);
begin
...
Function TFrAddPresentationDet.MessageShowString_WithExit(AppMessage: string):byte;
begin
  ShowMessageF:=TShowMessageF.Create(self);
  ShowMessageF.Label1.Caption:=AppMessage;
...
Function TFrAddPresentationDet.MessageShowString(AppMessage: string;ShowCancel:boolean):boolean;
begin
  ShowMessageF:=TShowMessageF.Create(self);
  ShowMessageF.Label1.Caption:=AppMessage;
...
procedure TFrAddPresentationDet.wordMenuItemClick(Sender: TObject);
begin
 if ActiveControl.ClassNameIs('TYDBGrid') then
...
procedure TFrAddPresentationDet.ExcelMenuItemClick(Sender: TObject);
begin
 if ActiveControl.ClassNameIs('TYDBGrid') then
   TYDBGrid(ActiveControl).ExportToExcel;
...
procedure TFrAddPresentationDet.TextMenuItemClick(Sender: TObject);
begin
 if ActiveControl.ClassNameIs('TYDBGrid') then
...
procedure TFrAddPresentationDet.PrintMenuItemClick(Sender: TObject);
begin
 if ActiveControl.ClassNameIs('TYDBGrid') then
...
procedure TFrAddPresentationDet.CustomMenuItemClick(Sender: TObject);
begin
 if ActiveControl.ClassNameIs('TYDBGrid') then
...
procedure TFrAddPresentationDet.Button1Click(Sender: TObject);
begin
     //if not dm.DAccountPresentation.DataSet.State in [dsEditModes] then
     dm.DAccountPresentation.DataSet.Insert;
   FrListofContacts:=TFrListofContacts.Create(NIL);
   FrListofContacts.ShowModal;
   IF FrListofContacts.ModalResult=mrOk THEN
   BEGIN
      dm.AccountPresenterContactId.AsInteger:=Dm.ContactQContactID.AsInteger;
      dm.DAccountPresentation.DataSet.post;
   END
   ELSE
   BEGIN
     // dm.AccountPresenterContactId.AsInteger:=-1;
     dm.DAccountPresentation.DataSet.cancel;
   END;

{   Dm.SearchTable(dm.Contact,'ContactID','ContactTitle');
   if Dm.SearchResult>0 then
   begin
      dm.AccountPresenterContactId.AsInteger:=dm.SearchResult;
...
procedure TFrAddPresentationDet.Button2Click(Sender: TObject);
begin
if MessageDlg('«“ Õ–› „⁄—› ›Êﬁ „ÿ„∆‰ Â” Ìœ ø',mtConfirmation,[mbYes,mbNo],0)=mrYES then
begin
if dm.DAccountPresentation.DataSet.RecordCount>0 then dm.DAccountPresentation.DataSet.delete;
end;
end;
